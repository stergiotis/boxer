...
fn run(seed: u64, natural: bool) {
    ...
    let base = if natural {
        LumaFrame::synthetic_natural(fw, fh, seed).unwrap()
    } else {
        LumaFrame::filled(fw, fh, 120.0).unwrap()
    };
    let payload = Payload([0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88]);
    let full = encode_frame(&base, &payload, &spec).unwrap();
    ...
    for _ in 0..200 {
        ...
        let crop = full.crop(ox, oy, cw, ch).unwrap();
