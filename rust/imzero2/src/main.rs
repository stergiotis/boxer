...
fn setup_tracing() {
    ...
    // install the bridge. The case that surfaced it was the since-retired
    // walkers tile loader (ADR-0204 M4), where a tile server that was down,
    // rate-limiting, or serving garbage produced a grey map and complete
    // silence; egui, wgpu and the vendored software rasterizer report through
    // `log` as well.
    //
    // Bridged at Info, not Trace: a per-item `debug!`/`trace!` from a busy
    // dependency (the tile loader logged one per tile) is noise on the hot
    // path, and every record below the cap is converted before it can be
    // filtered. Warnings and errors — the ones worth waking up for — arrive.
