...
self.apply_widget(w,u,f,Some(i));
            ...
            FuncProcId::EndETable => {
                ...
                if u.is_some() {
                    ...
                    impl<'sa, 'sb, 'sc, SR: std::io::BufRead, SW: std::io::Write>
                        egui_table::TableDelegate for EtStripedDelegate<'sa, 'sb, 'sc, SR, SW>
                    {
...
egui::Panel::top(i);
            ...
            FuncProcId::Plot => {
                ...
                loop {
                    ...
                    match m {
                        ...
                        PlotBuilderMethodId::AllowDoubleClickReset => {
                            #[cfg(feature = "puffin")]
                            puffin::profile_scope!(
                                "match PlotBuilderMethodId::AllowDoubleClickReset"
                            );
                            #[allow(unused_mut)]
                            let mut val = self.io.read_plain_b()?;
                        ...
                        PlotBuilderMethodId::YGridMarks => {
                            ...
                            y_grid_values = values;
                            y_grid_labels = labels;
                        }
                ...
                if u.is_some() {
                    ...
                    if let Some(w) = plot_width {
                        plot = plot.width(w);
                    }
                    if let Some(h) = plot_height {
                        plot = plot.height(h);
                    }
                    if let Some(a) = view_aspect {
                        plot = plot.view_aspect(a);
                    }
                    if let Some(a) = data_aspect {
                        plot = plot.data_aspect(a);
                    }
                    if let Some(ref l) = x_axis_label {
                        plot = plot.x_axis_label(l.as_str());
                    }
                    if let Some(ref l) = y_axis_label {
                        plot = plot.y_axis_label(l.as_str());
                    }
                    if show_legend {
                    ...
                    if !y_grid_values.is_empty() {
                        ...
                        let step = if positions.len() >= 2 {
                            (positions[1] - positions[0]).abs()
                        } else {
                            1.0
                        };
                        let positions_for_spacer = positions.clone();
                        plot = plot.y_grid_spacer(
                            move |_input: egui_plot::GridInput| -> Vec<egui_plot::GridMark> {
                        ...
                        plot = plot.y_axis_formatter(
                            move |gm: egui_plot::GridMark,
                    ...
                    if x_auto_ticks {
                        plot = plot.x_grid_spacer(
                            |input: egui_plot::GridInput| -> Vec<egui_plot::GridMark> {
                                ...
                                if !(range > 0.0)
                                    || !range.is_finite()
                                    || input.base_step_size.abs() < f64::EPSILON
                                {
                                    return Vec::new();
                                }
                    ...
                    let polygons_series: Vec<PlotPolygonData> =
                        self.plot_polygons.drain(..).collect();
                    let plot_response = plot.show(ui, |plot_ui| {
                        ...
                        if clamp_x.is_some() || clamp_y.is_some() {
                            ...
                            if let Some((lo, hi)) = clamp_y {
                                ...
                                if changed_y && nylo > nyhi {
                                    nylo = lo;
                                    nyhi = hi;
                                }
                            }
                            if changed_x {
                                plot_ui.set_plot_bounds_x(nxlo..=nxhi);
                            }
                            if changed_y {
                                plot_ui.set_plot_bounds_y(nylo..=nyhi);
                            }
                        }
                        ...
                        for pg in &polygons_series {
                            ...
                            if pg.stroke_width > 0.0 {
                                poly = poly.stroke(egui::Stroke::new(
                                    pg.stroke_width,
                                    color32_from_rgba_u32(pg.stroke_color),
                                ));
                            } else {
                                poly = poly.stroke(egui::Stroke::NONE);
                            }
                            if pg.highlight {
                                poly = poly.highlight(true);
                            }
                        ...
                        for ld in &lines {
                            ...
                            if let Some(f) = ld.fill {
                                line = line.fill(f as f32);
                            }
                            plot_ui.line(line);
                        }
                        for sd in &scatters {
                            ...
                            let shape = match sd.shape {
                                ...
                                2 => egui_plot::MarkerShape::Square,
                                3 => egui_plot::MarkerShape::Cross,
                                4 => egui_plot::MarkerShape::Plus,
                                5 => egui_plot::MarkerShape::Up,
                                6 => egui_plot::MarkerShape::Down,
                                7 => egui_plot::MarkerShape::Left,
                                8 => egui_plot::MarkerShape::Right,
                                9 => egui_plot::MarkerShape::Asterisk,
                                _ => egui_plot::MarkerShape::Circle,
                            };
                            let mut p = egui_plot::Points::new(&sd.name, pts)
                                .radius(sd.radius)
                                .shape(shape)
                                .filled(sd.filled);
                            if let Some(c) = sd.color {
                                p = p.color(c);
                            }
                            if sd.highlight {
                                p = p.highlight(true);
                        ...
                        for t in &texts {
                            let mut txt = egui_plot::Text::new(
                                &t.name,
                                egui_plot::PlotPoint::new(t.x, t.y),
                                &t.text,
                            );
                        ...
                        for bs in &boxes_series {
                            let elems: Vec<egui_plot::BoxElem> = bs
                                .boxes
                                .iter()
                                .map(|b| {
            ...
            FuncProcId::PlotBars => {
                ...
                for i in 0..n {
                    bars.push((arguments[i], values[i]));
                }
                let mut color: Option<egui::Color32> = None;
                ...
                // apply
                self.plot_bars.push(PlotBarsData {
                    name,
                    bars,
                    color,
                    width,
            ...
            FuncProcId::PlotBoxes => {
                ...
                let n = arguments
                    ...
                    .min(medians.len())
                    .min(q3s.len())
                    .min(whisker_mins.len())
                    .min(whisker_maxes.len())
                    .min(box_widths.len())
                    .min(fill_colors.len())
                    .min(stroke_colors.len())
                    .min(stroke_widths.len());
                ...
                loop {
                    ...
                    match m {
                        ...
                        PlotBoxesBuilderMethodId::SuppressElementText => {
                            #[cfg(feature = "puffin")]
                            puffin::profile_scope!(
                                "match PlotBoxesBuilderMethodId::SuppressElementText"
                            );
                ...
                // apply
                self.plot_boxes.push(PlotBoxesData {
                    name,
                    boxes,
                    horizontal,
                    highlight,
                    allow_hover,
                    suppress_element_text,
                });
            }
            FuncProcId::PlotHLine => {
                ...
                // apply
                self.plot_hlines.push(PlotHLineData {
                    name,
                    y: yy,
                    color,
                    width,
            ...
            FuncProcId::PlotLine => {
                ...
                // apply
                self.plot_lines.push(PlotLineData {
                    name,
                    points,
                    color,
                    width,
            ...
            FuncProcId::PlotPolygon => {
                ...
                // apply
                self.plot_polygons.push(PlotPolygonData {
                    name,
                    points,
                    fill_color,
                    stroke_color,
            ...
            FuncProcId::PlotScatter => {
                ...
                // apply
                self.plot_scatters.push(PlotScatterData {
                    name,
                    points,
                    color,
                    radius,
            ...
            FuncProcId::PlotText => {
                ...
                // apply
                self.plot_texts.push(PlotTextData {
                    name,
                    x: px,
                    y: py,
                    text,
            ...
            FuncProcId::PlotVLine => {
                ...
                // apply
                self.plot_vlines.push(PlotVLineData {
                    name,
                    x: xx,
                    color,
                    width,
