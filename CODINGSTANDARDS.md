...
*   **Record the review in the package's marker** (once built). Set `packageprops.Review` in `package_props.go`: the gofmt-normalized source digest the review covered, the reviewer (`code-review@<tier>` or `ultra`), the commit, the date, and a reference to the findings sidecar. The marker is the summary; the findings themselves — repros, dispositions — live in the sidecar, keeping `Props` a clean vocabulary ([ADR-0080](./doc/adr/0080-packageprops-per-package-declarations.md) §SD5).
*   **Re-review is triggered by drift** (once built). A review-aware `props verify` will recompute each marked package's normalized digest and flag the packages that have changed since their recorded review — that mismatch is the re-review signal. It is to be advisory while its false-positive rate is being bounded, and to graduate to a CI gate per the calibration lifecycle in ADR-0131. Whitespace-only churn will not trigger it; whether comment-only churn does depends on the normalization, one of the ADR's open questions.
