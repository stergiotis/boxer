...
func TestDefaultTabsEnumeration(t *testing.T) {
	...
	wantDockID := map[string]uint64{
		...
		"treemap": dockTabTreemap, "chart": dockTabChart,
		"graph":  dockTabGraph,
...
func TestTabRegistryMutationAndFreeze(t *testing.T) {
	...
	assert.Equal(t, TabZoneBody, reg.all()[25].Zone, "embedder tabs default to the body zone")
	...
	assert.Equal(t, "y", reg.all()[25].ID)
