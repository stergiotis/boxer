...
func TestDefaultTabsEnumeration(t *testing.T) {
	...
	wantDockID := map[string]uint64{
		...
		"kanban": dockTabKanban, "chat": dockTabChat, "network": dockTabNetwork, "graphview": dockTabGraphview,
	...
	assert.ElementsMatch(t, []string{"table", "projection", "timeline", "world", "kanban", "chat", "network", "graphview",
	...
	assert.Equal(t, []uint64{dockTabTable, dockTabProjection, dockTabTimeline,
		dockTabMap, dockTabWorld, dockTabKanban, dockTabChat, dockTabNetwork, dockTabGraphview, dockTabSankey, dockTabDist, dockTabIcicle,
...
func TestTabRegistryMutationAndFreeze(t *testing.T) {
	...
	assert.Equal(t, TabZoneBody, reg.all()[31].Zone, "embedder tabs default to the body zone")
	...
	assert.Equal(t, "y", reg.all()[31].ID)
