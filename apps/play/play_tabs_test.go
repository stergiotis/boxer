...
func TestDefaultTabsEnumeration(t *testing.T) {
	...
	wantDockID := map[string]uint64{
		...
		"dist": dockTabDist, "icicle": dockTabIcicle, "graph": dockTabGraph,
...
func TestTabRegistryMutationAndFreeze(t *testing.T) {
	...
	assert.Equal(t, TabZoneBody, reg.all()[22].Zone, "embedder tabs default to the body zone")
	...
	assert.Equal(t, "y", reg.all()[22].ID)
