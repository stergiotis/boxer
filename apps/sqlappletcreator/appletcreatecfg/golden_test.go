...
var updateGolden = flag.Bool("update", false, "rewrite appletcreatecfg.out.go with the current generator output")

// TestGeneratorMatchesCheckedInOutput runs the keelson codec generator on
// appletcreatecfg.go and asserts the bytes match the committed
// appletcreatecfg.out.go. Rerun with `-update` after intentional generator
// or DTO changes.
func TestGeneratorMatchesCheckedInOutput(t *testing.T) {
	_, thisFile, _, _ := runtime.Caller(0)
	dir := filepath.Dir(thisFile)
	inputPath := filepath.Join(dir, "appletcreatecfg.go")
	outputPath := filepath.Join(dir, "appletcreatecfg.out.go")
