...
//
// PresenceStore composes the generated facts building blocks per ADR-0100:
// append-only primitives and iterator query verbs; batched
// cached retrieval is the attachable PresenceCache view. Reads see only
...
import (
	...
	"github.com/stergiotis/boxer/public/keelson/runtime/watchbill/watchbillpresence/internal/lowlevel"
...
// PresenceTableName is the ClickHouse table this store binds — database-
// qualified ("<db>.<table>") when a Database was set at generation.
const PresenceTableName = "boxer.facts"
...
const (
	PresenceColKey   = `"id:id:u64:47::0:"`
	PresenceColOrder = `"ts:ts:z64:47::0:"`
)
...
// PresenceMembershipIds is the membership-id assignment this store was
...
var PresenceMembershipIds = map[string]map[string]uint64{
	"WorkerPresence": {
		"runtimeKindWatchbillWorker": 9223372049739677791,
		"watchbillWorkerRunId":       9223372049739677792,
		"watchbillWorkerHost":        9223372049739677793,
		"watchbillWorkerPhase":       9223372049739677794,
		"watchbillWorkerKinds":       9223372049739677795,
		"watchbillWorkerQueues":      9223372049739677796,
		"watchbillWorkerMaxWorkers":  9223372049739677797,
	},
}

// PresenceEnvelope carries the pass-through backbone columns — every plain
...
// PresenceEntity is the entity bag (ADR-0100 SD5): the envelope plus one option
...
type PresenceEntity struct {
	ID uint64
	Ts time.Time
	// PresenceEnvelope is embedded: its pass-through columns read as
	// promoted entity fields.
	PresenceEnvelope
	WorkerPresence option.Option[WorkerPresence]
}

// Archetype reports which components the entity carries, in schema order.
func (inst *PresenceEntity) Archetype() (a []string) {
	if inst.WorkerPresence.Has {
		a = append(a, "workerPresence")
	}
	return
}

type PresenceStoreConfig struct {
	// Table overrides the ClickHouse table this store binds — the baked
	// PresenceTableName — for every statement it issues (DDL, DESCRIBE, INSERT,
...
// PresenceStore is single-goroutine, like every part it composes. Batched
// cached retrieval is not built in — attach a PresenceCache view.
type PresenceStore struct {
	exec     recordstore.ExecutorI
	alloc    memory.Allocator
	cfg      PresenceStoreConfig
	...
	// views (NewPresenceCache registers one pair per view): onWrite populates
	...
	onWrite []func(uint64, *PresenceEntity)
	onFlush []func(uint64)
	// stampers is cfg.Stampers, consulted per Begin (ADR-0112 M1).
	stampers []recordstore.ReferenceStamper
}

// NewPresenceStore wires the store. A nil alloc selects the Go allocator.
func NewPresenceStore(exec recordstore.ExecutorI, alloc memory.Allocator, cfg PresenceStoreConfig) (inst *PresenceStore) {
	if cfg.Table != "" {
		if terr := recordstore.CheckTableRef(cfg.Table); terr != nil {
			panic("PresenceStore: " + terr.Error())
		}
	}
	if alloc == nil {
		alloc = memory.NewGoAllocator()
	}
	inst = &PresenceStore{exec: exec, alloc: alloc, cfg: cfg, dml: lowlevel.NewInEntityFactsTable(alloc, 64), dirty: make(map[uint64]struct{}), stampers: cfg.Stampers}
	return
}

// tableName is the table reference every statement uses: the configured
// override when set, else the baked PresenceTableName.
func (inst *PresenceStore) tableName() string {
	if inst.cfg.Table != "" {
...
func (inst *PresenceStore) applyStampers() (pushed int) {
...
// the key instead.
func (inst *PresenceStore) notifyWrite(key uint64, ent *PresenceEntity) {
...
func (inst *PresenceStore) notifyFlush(key uint64) {
...
// PresenceMembershipIds against the writer's assignment when pointing this
...
func (inst *PresenceStore) VerifySchema(ctx context.Context) (err error) {
...
// OpenPresenceStore is NewPresenceStore plus the startup check: it runs VerifySchema and
...
// caller's layer (ADR-0105 D1). NewPresenceStore panics on a malformed config
// and so does this. A caller that cannot afford the DESCRIBE at startup
// keeps using NewPresenceStore.
func OpenPresenceStore(ctx context.Context, exec recordstore.ExecutorI, alloc memory.Allocator, cfg PresenceStoreConfig) (inst *PresenceStore, err error) {
	st := NewPresenceStore(exec, alloc, cfg)
	err = st.VerifySchema(ctx)
	if err != nil {
		st.Close()
		err = eh.Errorf("open presenceStore: %w", err)
		return
	}
	inst = st
	return
...
// PresenceEntityBuilder assembles one entity: envelope from Begin, components
// via Add*, direct attribute manipulation via Raw, then Commit.
type PresenceEntityBuilder struct {
	store *PresenceStore
	key   uint64
...
func (inst *PresenceEntityBuilder) endSection(section string) error {
	switch section {
	case "symbol":
...
func (inst *PresenceStore) Begin(id uint64, ts time.Time, env PresenceEnvelope) *PresenceEntityBuilder {
	...
	b := &PresenceEntityBuilder{store: inst, key: id, ent: PresenceEntity{ID: id, Ts: ts, PresenceEnvelope: env}}
	b.pushed = inst.applyStampers()
	return b
}

// AddWorkerPresence contributes the WorkerPresence component to the open entity.
//
...
func (inst *PresenceEntityBuilder) AddWorkerPresence(row WorkerPresence) *PresenceEntityBuilder {
	if err := inst.buf.StartKind("WorkerPresence"); err != nil {
		inst.store.dml.AppendError(err)
		return inst
	}
	inst.buf.Enqueue("symbol", "WorkerPresence", func() error {
		return workerPresenceEmitSectionSymbol(inst.store.dml.GetSectionSymbol(), row)
	})
	inst.buf.Enqueue("symbolArray", "WorkerPresence", func() error {
		return workerPresenceEmitSectionSymbolArray(inst.store.dml.GetSectionSymbolArray(), row)
	})
	inst.buf.Enqueue("u32Array", "WorkerPresence", func() error {
		return workerPresenceEmitSectionU32Array(inst.store.dml.GetSectionU32Array(), row)
	})
	inst.ent.WorkerPresence = option.Some(row)
	return inst
}
...
func (inst *PresenceEntityBuilder) Raw() *lowlevel.InEntityFactsTable {
...
func (inst *PresenceEntityBuilder) Commit() (err error) {
...
func (inst *PresenceEntityBuilder) Rollback() (err error) {
...
// them.
func (inst *PresenceStore) IngestWorkerPresence(ts time.Time, rows []WorkerPresence) (err error) {
	seen := make(map[uint64]struct{}, len(rows))
	for i := range rows {
		if _, dup := seen[rows[i].Id]; dup {
			err = eh.Errorf("ingest workerPresence row %d: %w: key %v", i, recordstore.ErrDuplicateIngestKey, rows[i].Id)
			return
		}
...
func (inst *PresenceStore) Buffered() int { return inst.buffered }
...
func (inst *PresenceStore) Flush(ctx context.Context) (n int, err error) {
...
func (inst *PresenceStore) FlushIfDue(ctx context.Context) (n int, err error) {
...
func (inst *PresenceStore) DiscardPending() {
...
func (inst *PresenceStore) Close() {
	inst.DiscardPending()
	lowlevel.InEntityFactsTableReleaseBuilder(inst.dml)
}

// PresenceCacheConfig parameterizes an attached read-through cache view.
...
// PresenceCache is the batched read-through, write-through KV view over a
// PresenceStore (ADR-0100 SD5): misses queue under work items and flush
...
type PresenceCache[W comparable] struct {
	st    *PresenceStore
	cfg   PresenceCacheConfig
	cache *caching.ReadThroughCache[uint64, *PresenceEntity, W]
}

// NewPresenceCache attaches a read-through, write-through cache view to st,
// registering its write-through and flush hooks with the store. Views
// attach for the store's lifetime — there is no detach.
func NewPresenceCache[W comparable](st *PresenceStore, cfg PresenceCacheConfig) (inst *PresenceCache[W]) {
	if cfg.Capacity <= 0 {
		cfg.Capacity = 1024
	}
	inst = &PresenceCache[W]{st: st, cfg: cfg}
	inst.rebuild()
	// The hooks close over the view, not the cache instance, keeping
	// them correct if the cache is ever swapped out.
	st.onWrite = append(st.onWrite, func(k uint64, ent *PresenceEntity) {
		if ent == nil {
...
func (inst *PresenceCache[W]) rebuild() {
	opts := []caching.CacheOption[uint64, *PresenceEntity, W]{
		// Admission mirrors the table's newest-row-per-key semantics:
		// the Order timestamp is the entity's monotonic version.
		caching.WithVersioning[uint64, *PresenceEntity, W](func(e *PresenceEntity) int64 { return e.Ts.UnixNano() }),
	}
	if inst.cfg.FreshnessTTL > 0 {
		opts = append(opts, caching.WithFreshnessTTL[uint64, *PresenceEntity, W](inst.cfg.FreshnessTTL))
	}
	if inst.cfg.NegativeTTL > 0 {
		opts = append(opts, caching.WithNegativeCaching[uint64, *PresenceEntity, W](inst.cfg.NegativeTTL))
	}
	inst.cache = caching.NewReadThroughCache[uint64, *PresenceEntity, W](inst.cfg.Capacity, &factsFetcher{st: inst.st}, inst.cfg.FetchCriteria, opts...)
}
...
func (inst *PresenceCache[W]) Get(key uint64) (ent *PresenceEntity, found bool) {
...
func (inst *PresenceCache[W]) GetFetch(ctx context.Context, key uint64) (ent *PresenceEntity, found bool, err error) {
...
func (inst *PresenceCache[W]) WorkItem(w W) iter.Seq[functional.NilIteratorValueType] {
...
func (inst *PresenceCache[W]) IterateReadyWorkItems(ctx context.Context) iter.Seq[W] {
...
func (inst *PresenceCache[W]) IterateRestWorkItems(ctx context.Context) iter.Seq[W] {
...
func (inst *PresenceCache[W]) AdvanceEpoch() {
...
func (inst *PresenceCache[W]) MarkStale(key uint64) {
...
func (inst *PresenceCache[W]) MarkStaleIfOlder(key uint64, order time.Time) {
	inst.cache.MarkAsStaleIfOlder(key, order.UnixNano())
}

// Invalidate drops the key's cached entry (L1 and stash).
func (inst *PresenceCache[W]) Invalidate(key uint64) {
...
func (inst *PresenceStore) fetchLatestSQL(keys []uint64) string {
	...
	sb.WriteString(") ORDER BY " + PresenceColOrder + " DESC LIMIT 1 BY " + PresenceColKey)
...
// public method set.
type factsFetcher struct{ st *PresenceStore }

var _ caching.ItemFetcherI[uint64, *PresenceEntity] = (*factsFetcher)(nil)
...
// compare against.
func (inst *factsFetcher) FetchItemSinglePartition(ctx context.Context, partition uint64, keys []uint64, target caching.ItemTargetI[uint64, *PresenceEntity]) (err error) {
...
const (
	factsScanWorkerPresenceFilter = "hasAll(\"tv:symbol:lr:lr:u64:1247:::0::data\", [9223372049739677791, 9223372049739677792, 9223372049739677793, 9223372049739677794]) AND has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677797) AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677791) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677792) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677793) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677794) = 1 AND countEqual(\"tv:symbolArray:lr:lr:u64:1247:::0::data\", 9223372049739677795) <= 1 AND countEqual(\"tv:symbolArray:lr:lr:u64:1247:::0::data\", 9223372049739677796) <= 1 AND countEqual(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677797) = 1 AND if(has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677797), \"tv:u32Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677797), arrayCumSum(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1"
)

// ScanWorkerPresence iterates the entities whose rows carry a conforming WorkerPresence
...
// rows.
func (inst *PresenceStore) ScanWorkerPresence(ctx context.Context, opts recordstore.ScanOpts) iter.Seq2[*PresenceEntity, error] {
	if opts.KeyPrefix != "" {
		return recordstore.RefuseKeyPrefix[*PresenceEntity]()
	}
	where := factsScanWorkerPresenceFilter
	if opts.ExtraPredicate != "" {
		where = "(" + where + ") AND (" + opts.ExtraPredicate + ")"
	}
	sql := "SELECT * FROM " + inst.tableName() +
		" WHERE " + where +
		" ORDER BY " + PresenceColOrder + " ASC, " + PresenceColKey + " ASC"
	if opts.Limit > 0 {
...
// flushed rows.
func (inst *PresenceStore) Latest(ctx context.Context, key uint64) (ent *PresenceEntity, found bool, err error) {
	sql := "SELECT * FROM " + inst.tableName() +
		" WHERE " + PresenceColKey + " = " + factsKeyLiteral(key) +
		" ORDER BY " + PresenceColOrder + " DESC LIMIT 1" + factsArrowOutputSettings
...
func (inst *PresenceStore) Replay(ctx context.Context, key uint64, fromOrder time.Time, opts recordstore.ReplayOpts) iter.Seq2[*PresenceEntity, error] {
	sql := "SELECT * FROM " + inst.tableName() +
		" WHERE " + PresenceColKey + " = " + factsKeyLiteral(key)
	if !fromOrder.IsZero() {
		sql += " AND " + PresenceColOrder + " >= fromUnixTimestamp64Nano(" + strconv.FormatInt(fromOrder.UnixNano(), 10) + ")"
	}
	if !opts.To.IsZero() {
		sql += " AND " + PresenceColOrder + " < fromUnixTimestamp64Nano(" + strconv.FormatInt(opts.To.UnixNano(), 10) + ")"
	}
	sql += " ORDER BY " + PresenceColOrder + " ASC"
	if opts.Limit > 0 {
		sql += " LIMIT " + strconv.Itoa(opts.Limit)
	}
	sql += factsArrowOutputSettings
...
// PresenceComponentSQL publishes this store's ADR-0066 read-back artefacts —
...
// join must bind them to PresenceTableName itself (ADR-0189 SD6).
var PresenceComponentSQL = componentsql.Set{
	Store: "Presence",
	Table: PresenceTableName,
	Kinds: map[string]componentsql.Artefacts{
		"WorkerPresence": {
			Presence:   "hasAll(\"tv:symbol:lr:lr:u64:1247:::0::data\", [9223372049739677791, 9223372049739677792, 9223372049739677793, 9223372049739677794]) AND has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677797)",
			Validator:  "countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677791) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677792) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677793) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677794) = 1 AND countEqual(\"tv:symbolArray:lr:lr:u64:1247:::0::data\", 9223372049739677795) <= 1 AND countEqual(\"tv:symbolArray:lr:lr:u64:1247:::0::data\", 9223372049739677796) <= 1 AND countEqual(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677797) = 1 AND if(has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677797), \"tv:u32Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677797), arrayCumSum(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1",
			Filter:     factsScanWorkerPresenceFilter,
			Projection: "CAST(tuple(\"id:id:u64:47::0:\", \"id:naturalKey:y:4::0:\", \"ts:ts:z64:47::0:\", LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677791, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677792, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677793, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677794, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_LIST_BY_TAG_EQUAL(\"tv:symbolArray:value:val:sh:124::I:0::data\", \"tv:symbolArray:len:len:u64:4D:::0::data\", \"tv:symbolArray:lr:lr:u64:1247:::0::data\", 9223372049739677795, LW_RAGGED_PARENT_IDS(\"tv:symbolArray:lrcard:lrcard:u64:4E:::0::data\")), LW_LIST_BY_TAG_EQUAL(\"tv:symbolArray:value:val:sh:124::I:0::data\", \"tv:symbolArray:len:len:u64:4D:::0::data\", \"tv:symbolArray:lr:lr:u64:1247:::0::data\", 9223372049739677796, LW_RAGGED_PARENT_IDS(\"tv:symbolArray:lrcard:lrcard:u64:4E:::0::data\")), LW_LIST_BY_TAG_EQUAL(\"tv:u32Array:value:val:u32h:4:::0::data\", \"tv:u32Array:len:len:u64:4D:::0::data\", \"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677797, LW_RAGGED_PARENT_IDS(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))[1]), 'Tuple(Id UInt64, NaturalKey String, Ts DateTime64(9,\\'UTC\\'), Kind String, RunId String, Host String, Phase String, Kinds Array(String), Queues Array(String), MaxWorkers UInt32)')",
		},
	},
}
...
func (inst *PresenceStore) queryEntities(ctx context.Context, sql string) (ents []*PresenceEntity, err error) {
	for rec, rerr := range inst.exec.QueryArrow(ctx, sql) {
		if rerr != nil {
			err = eh.Errorf("query entities: %w", rerr)
			return
		}
		batch, derr := decodePresenceRecord(rec)
		rec.Release()
		if derr != nil {
			err = derr
			return
...
// the sequence.
func (inst *PresenceStore) iterateEntities(ctx context.Context, sql string) iter.Seq2[*PresenceEntity, error] {
	return func(yield func(*PresenceEntity, error) bool) {
		ents, err := inst.queryEntities(ctx, sql)
		if err != nil {
			yield(nil, err)
			return
...
// decodePresenceRecord reads one fetched Arrow record into entity bags:
...
func decodePresenceRecord(rec arrow.RecordBatch) (ents []*PresenceEntity, err error) {
	...
	readers := []factsSectionReaderI{idR, tsR, lcR, symbolR, symbolArrayR, u32ArrayR}
	...
	for i := range n {
		...
		{
			row, ok, e := workerPresenceReadRow(i, symbolR.GetAttributes(), symbolR.GetMemberships(), symbolArrayR.GetAttributes(), symbolArrayR.GetMemberships(), u32ArrayR.GetAttributes(), u32ArrayR.GetMemberships())
			if e != nil {
				err = eh.Errorf("read workerPresence component: %w", e)
				return
			}
			if ok {
				row.Id = ent.ID
				row.Ts = ent.Ts
				ent.WorkerPresence = option.Some(row)
			}
		}
