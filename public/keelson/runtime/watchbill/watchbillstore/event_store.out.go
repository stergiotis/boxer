...
//
// EventStore composes the generated watchbillevent building blocks per ADR-0100:
// append-only primitives and iterator query verbs; batched
// cached retrieval is the attachable EventCache view. Reads see only
...
//
//go:embed watchbillevent_ddl_clickhouse.out.sql
var watchbilleventDDLCreate string

// EventTableName is the ClickHouse table this store binds — database-
// qualified ("<db>.<table>") when a Database was set at generation.
const EventTableName = "boxer.watchbillevent"
...
const (
	EventColKey   = `"id:id:s:4::0:"`
	EventColOrder = `"ts:ts:z64:47::0:"`
)

// Arrow output shape the read-access classes expect.
const watchbilleventArrowOutputSettings = " SETTINGS output_format_arrow_string_as_string=1, output_format_arrow_low_cardinality_as_dictionary=0"
...
var EventMembershipIds = map[string]map[string]uint64{
	"Event": {
		"watchbillEventState":     9223372049739677786,
		"watchbillEventAttempt":   9223372049739677787,
		"watchbillEventWorkerRun": 9223372049739677788,
		"watchbillEventError":     9223372049739677789,
		"watchbillEventNote":      9223372049739677790,
	},
}

// EventEntity is the entity bag (ADR-0100 SD5): the envelope plus one option
...
func (inst *EventEntity) Archetype() (a []string) {
	if inst.Event.Has {
		a = append(a, "event")
	}
	return
...
// EventStore is single-goroutine, like every part it composes. Batched
// cached retrieval is not built in — attach a EventCache view.
type EventStore struct {
	exec     recordstore.ExecutorI
	alloc    memory.Allocator
	cfg      EventStoreConfig
	dml      *lowlevel.InEntityWatchbilleventTable
	buffered int
	...
	// views (NewEventCache registers one pair per view): onWrite populates
	...
	onWrite []func(string, *EventEntity)
	onFlush []func(string)
	// stampers is cfg.Stampers, consulted per Begin (ADR-0112 M1).
	stampers []recordstore.ReferenceStamper
}

// NewEventStore wires the store. A nil alloc selects the Go allocator.
func NewEventStore(exec recordstore.ExecutorI, alloc memory.Allocator, cfg EventStoreConfig) (inst *EventStore) {
	if cfg.Table != "" {
	...
	inst = &EventStore{exec: exec, alloc: alloc, cfg: cfg, dml: lowlevel.NewInEntityWatchbilleventTable(alloc, 64), dirty: make(map[string]struct{}), stampers: cfg.Stampers}
	return
}
...
func (inst *EventStore) applyStampers() (pushed int) {
...
// the key instead.
func (inst *EventStore) notifyWrite(key string, ent *EventEntity) {
...
func (inst *EventStore) notifyFlush(key string) {
...
// EventMembershipIds against the writer's assignment when pointing this
...
// OpenEventStore is NewEventStore plus the startup sequence every caller otherwise
...
// caller's layer (ADR-0105 D1). NewEventStore panics on a malformed config
// and so does this. A caller that cannot afford the DESCRIBE at startup
// keeps using NewEventStore.
func OpenEventStore(ctx context.Context, exec recordstore.ExecutorI, alloc memory.Allocator, cfg EventStoreConfig) (inst *EventStore, err error) {
	st := NewEventStore(exec, alloc, cfg)
	err = st.EnsureTable(ctx)
	if err != nil {
		st.Close()
		err = eh.Errorf("open eventStore: %w", err)
		return
	}
...
// EventEntityBuilder assembles one entity: envelope from Begin, components
// via Add*, direct attribute manipulation via Raw, then Commit.
type EventEntityBuilder struct {
	store *EventStore
	key   string
...
func (inst *EventEntityBuilder) endSection(section string) error {
	switch section {
	case "eventState":
		inst.store.dml.GetSectionEventState().EndSection()
	case "eventAttempt":
		inst.store.dml.GetSectionEventAttempt().EndSection()
	case "eventWorkerRun":
		inst.store.dml.GetSectionEventWorkerRun().EndSection()
	case "eventError":
		inst.store.dml.GetSectionEventError().EndSection()
	case "eventNote":
		inst.store.dml.GetSectionEventNote().EndSection()
	}
	return nil
}
...
// (Key, Order).
func (inst *EventStore) Begin(id string, ts time.Time) *EventEntityBuilder {
	lowlevel.InEntityWatchbilleventTableBeginEntity(inst.dml)
	lowlevel.InEntityWatchbilleventTableSetId(inst.dml, id)
	lowlevel.InEntityWatchbilleventTableSetTimestamp(inst.dml, ts)
	b := &EventEntityBuilder{store: inst, key: id, ent: EventEntity{ID: id, Ts: ts}}
	b.pushed = inst.applyStampers()
	return b
}

// AddEvent contributes the Event component to the open entity.
//
...
func (inst *EventEntityBuilder) AddEvent(row Event) *EventEntityBuilder {
	if err := inst.buf.StartKind("Event"); err != nil {
		inst.store.dml.AppendError(err)
		return inst
	}
	inst.buf.Enqueue("eventState", "Event", func() error {
		return eventEmitSectionEventState(inst.store.dml.GetSectionEventState(), row)
	})
	inst.buf.Enqueue("eventAttempt", "Event", func() error {
		return eventEmitSectionEventAttempt(inst.store.dml.GetSectionEventAttempt(), row)
	})
	inst.buf.Enqueue("eventWorkerRun", "Event", func() error {
		return eventEmitSectionEventWorkerRun(inst.store.dml.GetSectionEventWorkerRun(), row)
	})
	inst.buf.Enqueue("eventError", "Event", func() error {
		return eventEmitSectionEventError(inst.store.dml.GetSectionEventError(), row)
	})
	inst.buf.Enqueue("eventNote", "Event", func() error {
		return eventEmitSectionEventNote(inst.store.dml.GetSectionEventNote(), row)
	})
	inst.ent.Event = option.Some(row)
	return inst
}
...
func (inst *EventEntityBuilder) Raw() *lowlevel.InEntityWatchbilleventTable {
...
func (inst *EventEntityBuilder) Rollback() (err error) {
	...
	return lowlevel.InEntityWatchbilleventTableRollbackEntity(inst.store.dml)
}

// IngestEvent buffers one whole entity per row carrying only the
// Event component, all stamped with ts — rows ship on the next Flush,
...
// them.
func (inst *EventStore) IngestEvent(ts time.Time, rows []Event) (err error) {
	seen := make(map[string]struct{}, len(rows))
	for i := range rows {
		if _, dup := seen[rows[i].ID]; dup {
			err = eh.Errorf("ingest event row %d: %w: key %v", i, recordstore.ErrDuplicateIngestKey, rows[i].ID)
			return
		}
		seen[rows[i].ID] = struct{}{}
		err = inst.Begin(rows[i].ID, ts).AddEvent(rows[i]).Commit()
		if err != nil {
			err = eh.Errorf("ingest event row %d: %w", i, err)
			return
		}
	}
	return
...
func (inst *EventStore) Buffered() int { return inst.buffered }
...
func (inst *EventStore) Flush(ctx context.Context) (n int, err error) {
	...
	records, err := lowlevel.InEntityWatchbilleventTableTransferRecords(inst.dml, nil)
	if err != nil {
...
func (inst *EventStore) FlushIfDue(ctx context.Context) (n int, err error) {
...
func (inst *EventStore) DiscardPending() {
	_ = lowlevel.InEntityWatchbilleventTableRollbackEntity(inst.dml) // no-op error when no frame is open
	inst.dml.ClearMembershipsHighCardRef()                           // an abandoned Begin's stamps must not outlive its frame
	if records, err := lowlevel.InEntityWatchbilleventTableTransferRecords(inst.dml, nil); err == nil {
		for _, rec := range records {
			rec.Release()
		}
	}
...
func (inst *EventStore) Close() {
	inst.DiscardPending()
	lowlevel.InEntityWatchbilleventTableReleaseBuilder(inst.dml)
}

// EventCacheConfig parameterizes an attached read-through cache view.
...
// EventCache is the batched read-through, write-through KV view over a
// EventStore (ADR-0100 SD5): misses queue under work items and flush
...
type EventCache[W comparable] struct {
	st    *EventStore
	cfg   EventCacheConfig
	cache *caching.ReadThroughCache[string, *EventEntity, W]
}

// NewEventCache attaches a read-through, write-through cache view to st,
// registering its write-through and flush hooks with the store. Views
// attach for the store's lifetime — there is no detach.
func NewEventCache[W comparable](st *EventStore, cfg EventCacheConfig) (inst *EventCache[W]) {
	if cfg.Capacity <= 0 {
		cfg.Capacity = 1024
	}
	inst = &EventCache[W]{st: st, cfg: cfg}
	inst.rebuild()
	// The hooks close over the view, not the cache instance, keeping
	// them correct if the cache is ever swapped out.
	st.onWrite = append(st.onWrite, func(k string, ent *EventEntity) {
		if ent == nil {
...
func (inst *EventCache[W]) rebuild() {
	opts := []caching.CacheOption[string, *EventEntity, W]{
		// Admission mirrors the table's newest-row-per-key semantics:
		// the Order timestamp is the entity's monotonic version.
		caching.WithVersioning[string, *EventEntity, W](func(e *EventEntity) int64 { return e.Ts.UnixNano() }),
	}
	if inst.cfg.FreshnessTTL > 0 {
		opts = append(opts, caching.WithFreshnessTTL[string, *EventEntity, W](inst.cfg.FreshnessTTL))
	}
	if inst.cfg.NegativeTTL > 0 {
		opts = append(opts, caching.WithNegativeCaching[string, *EventEntity, W](inst.cfg.NegativeTTL))
	}
	inst.cache = caching.NewReadThroughCache[string, *EventEntity, W](inst.cfg.Capacity, &watchbilleventFetcher{st: inst.st}, inst.cfg.FetchCriteria, opts...)
}
...
func (inst *EventCache[W]) Get(key string) (ent *EventEntity, found bool) {
...
func (inst *EventCache[W]) GetFetch(ctx context.Context, key string) (ent *EventEntity, found bool, err error) {
...
func (inst *EventCache[W]) WorkItem(w W) iter.Seq[functional.NilIteratorValueType] {
...
func (inst *EventCache[W]) IterateReadyWorkItems(ctx context.Context) iter.Seq[W] {
...
func (inst *EventCache[W]) IterateRestWorkItems(ctx context.Context) iter.Seq[W] {
...
func (inst *EventCache[W]) AdvanceEpoch() {
...
func (inst *EventCache[W]) MarkStale(key string) {
...
func (inst *EventCache[W]) MarkStaleIfOlder(key string, order time.Time) {
	inst.cache.MarkAsStaleIfOlder(key, order.UnixNano())
}

// Invalidate drops the key's cached entry (L1 and stash).
func (inst *EventCache[W]) Invalidate(key string) {
...
func (inst *EventCache[W]) InvalidateAll() {
	inst.cache.Clear()
}
...
func (inst *EventStore) fetchLatestSQL(keys []string) string {
	...
	for i, k := range keys {
		...
		sb.WriteString(watchbilleventKeyLiteral(k))
	}
	sb.WriteString(") ORDER BY " + EventColOrder + " DESC LIMIT 1 BY " + EventColKey)
	sb.WriteString(watchbilleventArrowOutputSettings)
	return sb.String()
}

// watchbilleventFetcher implements caching.ItemFetcherI for attached cache views —
// an unexported shim so the fetch plumbing stays off the store's
// public method set.
type watchbilleventFetcher struct{ st *EventStore }

var _ caching.ItemFetcherI[string, *EventEntity] = (*watchbilleventFetcher)(nil)

// DeterminePartition implements caching.ItemFetcherI. Single partition
// in v1 (one table, one server).
func (inst *watchbilleventFetcher) DeterminePartition(key string) uint64 { return 0 }
...
// compare against.
func (inst *watchbilleventFetcher) FetchItemSinglePartition(ctx context.Context, partition uint64, keys []string, target caching.ItemTargetI[string, *EventEntity]) (err error) {
...
const (
	watchbilleventScanEventFilter = "has(\"tv:eventState:lr:lr:u64:1247:::0::data\", 9223372049739677786) AND has(\"tv:eventAttempt:lr:lr:u64:1247:::0::data\", 9223372049739677787) AND has(\"tv:eventWorkerRun:lr:lr:u64:1247:::0::data\", 9223372049739677788) AND has(\"tv:eventError:lr:lr:u64:1247:::0::data\", 9223372049739677789) AND has(\"tv:eventNote:lr:lr:u64:1247:::0::data\", 9223372049739677790) AND countEqual(\"tv:eventState:lr:lr:u64:1247:::0::data\", 9223372049739677786) = 1 AND countEqual(\"tv:eventAttempt:lr:lr:u64:1247:::0::data\", 9223372049739677787) = 1 AND countEqual(\"tv:eventWorkerRun:lr:lr:u64:1247:::0::data\", 9223372049739677788) = 1 AND countEqual(\"tv:eventError:lr:lr:u64:1247:::0::data\", 9223372049739677789) = 1 AND countEqual(\"tv:eventNote:lr:lr:u64:1247:::0::data\", 9223372049739677790) = 1"
)

// ScanEvent iterates the entities whose rows carry a conforming Event
...
func (inst *EventStore) ScanEvent(ctx context.Context, opts recordstore.ScanOpts) iter.Seq2[*EventEntity, error] {
	...
	sql += watchbilleventArrowOutputSettings
...
func (inst *EventStore) Replay(ctx context.Context, key string, fromOrder time.Time, opts recordstore.ReplayOpts) iter.Seq2[*EventEntity, error] {
	...
	if !opts.To.IsZero() {
		sql += " AND " + EventColOrder + " < fromUnixTimestamp64Nano(" + strconv.FormatInt(opts.To.UnixNano(), 10) + ")"
	}
	sql += " ORDER BY " + EventColOrder + " ASC"
	if opts.Limit > 0 {
		sql += " LIMIT " + strconv.Itoa(opts.Limit)
	}
	sql += watchbilleventArrowOutputSettings
...
// EventComponentSQL publishes this store's ADR-0066 read-back artefacts —
...
// join must bind them to EventTableName itself (ADR-0189 SD6).
var EventComponentSQL = componentsql.Set{
	Store: "Event",
	Table: EventTableName,
	Kinds: map[string]componentsql.Artefacts{
		"Event": {
			Presence:   "has(\"tv:eventState:lr:lr:u64:1247:::0::data\", 9223372049739677786) AND has(\"tv:eventAttempt:lr:lr:u64:1247:::0::data\", 9223372049739677787) AND has(\"tv:eventWorkerRun:lr:lr:u64:1247:::0::data\", 9223372049739677788) AND has(\"tv:eventError:lr:lr:u64:1247:::0::data\", 9223372049739677789) AND has(\"tv:eventNote:lr:lr:u64:1247:::0::data\", 9223372049739677790)",
			Validator:  "countEqual(\"tv:eventState:lr:lr:u64:1247:::0::data\", 9223372049739677786) = 1 AND countEqual(\"tv:eventAttempt:lr:lr:u64:1247:::0::data\", 9223372049739677787) = 1 AND countEqual(\"tv:eventWorkerRun:lr:lr:u64:1247:::0::data\", 9223372049739677788) = 1 AND countEqual(\"tv:eventError:lr:lr:u64:1247:::0::data\", 9223372049739677789) = 1 AND countEqual(\"tv:eventNote:lr:lr:u64:1247:::0::data\", 9223372049739677790) = 1",
			Filter:     watchbilleventScanEventFilter,
			Projection: "CAST(tuple(\"id:id:s:4::0:\", LW_VALUE_BY_TAG_EQUAL(\"tv:eventState:value:val:s:24:::0::data\", \"tv:eventState:lr:lr:u64:1247:::0::data\", 9223372049739677786, LW_RAGGED_PARENT_IDS(\"tv:eventState:lrcard:lrcard:u64:4E:::0::data\")), LW_VALUE_BY_TAG_EQUAL(\"tv:eventAttempt:value:val:u32:4:::0::data\", \"tv:eventAttempt:lr:lr:u64:1247:::0::data\", 9223372049739677787, LW_RAGGED_PARENT_IDS(\"tv:eventAttempt:lrcard:lrcard:u64:4E:::0::data\")), LW_VALUE_BY_TAG_EQUAL(\"tv:eventWorkerRun:value:val:s:24:::0::data\", \"tv:eventWorkerRun:lr:lr:u64:1247:::0::data\", 9223372049739677788, LW_RAGGED_PARENT_IDS(\"tv:eventWorkerRun:lrcard:lrcard:u64:4E:::0::data\")), LW_VALUE_BY_TAG_EQUAL(\"tv:eventError:value:val:y:4:::0::data\", \"tv:eventError:lr:lr:u64:1247:::0::data\", 9223372049739677789, LW_RAGGED_PARENT_IDS(\"tv:eventError:lrcard:lrcard:u64:4E:::0::data\")), LW_VALUE_BY_TAG_EQUAL(\"tv:eventNote:value:val:s:4:::0::data\", \"tv:eventNote:lr:lr:u64:1247:::0::data\", 9223372049739677790, LW_RAGGED_PARENT_IDS(\"tv:eventNote:lrcard:lrcard:u64:4E:::0::data\"))), 'Tuple(ID String, State String, Attempt UInt32, WorkerRun String, Error String, Note String)')",
		},
	},
}
...
// watchbilleventSectionReaderI is the uniform slice of the generated read-access
// readers. Column indices stay at their constructor defaults — the
// schema order a SELECT * returns.
type watchbilleventSectionReaderI interface {
	LoadFromRecord(raruntime.RecordI) error
	Release()
}

func (inst *EventStore) queryEntities(ctx context.Context, sql string) (ents []*EventEntity, err error) {
	for rec, rerr := range inst.exec.QueryArrow(ctx, sql) {
		if rerr != nil {
			err = eh.Errorf("query entities: %w", rerr)
			return
		}
		batch, derr := decodeEventRecord(rec)
		rec.Release()
		if derr != nil {
			err = derr
			return
...
// the sequence.
func (inst *EventStore) iterateEntities(ctx context.Context, sql string) iter.Seq2[*EventEntity, error] {
	return func(yield func(*EventEntity, error) bool) {
		ents, err := inst.queryEntities(ctx, sql)
		if err != nil {
			yield(nil, err)
			return
...
// decodeEventRecord reads one fetched Arrow record into entity bags:
...
func decodeEventRecord(rec arrow.RecordBatch) (ents []*EventEntity, err error) {
	idR := lowlevel.NewReadAccessWatchbilleventTablePlainEntityIdAttributes()
	tsR := lowlevel.NewReadAccessWatchbilleventTablePlainEntityTimestampAttributes()
	eventStateR := lowlevel.NewReadAccessWatchbilleventTableTaggedEventState()
	eventAttemptR := lowlevel.NewReadAccessWatchbilleventTableTaggedEventAttempt()
	eventWorkerRunR := lowlevel.NewReadAccessWatchbilleventTableTaggedEventWorkerRun()
	eventErrorR := lowlevel.NewReadAccessWatchbilleventTableTaggedEventError()
	eventNoteR := lowlevel.NewReadAccessWatchbilleventTableTaggedEventNote()
	readers := []watchbilleventSectionReaderI{idR, tsR, eventStateR, eventAttemptR, eventWorkerRunR, eventErrorR, eventNoteR}
	for _, r := range readers {
		err = r.LoadFromRecord(rec)
		if err != nil {
			err = eh.Errorf("load watchbillevent reader from record: %w", err)
			return
		}
	}
	defer func() {
	...
	for i := range n {
		...
		{
			row, ok, e := eventReadRow(i, eventStateR.GetAttributes(), eventStateR.GetMemberships(), eventAttemptR.GetAttributes(), eventAttemptR.GetMemberships(), eventWorkerRunR.GetAttributes(), eventWorkerRunR.GetMemberships(), eventErrorR.GetAttributes(), eventErrorR.GetMemberships(), eventNoteR.GetAttributes(), eventNoteR.GetMemberships())
			if e != nil {
				err = eh.Errorf("read event component: %w", e)
				return
			}
			if ok {
				row.ID = ent.ID
				ent.Event = option.Some(row)
			}
		}
