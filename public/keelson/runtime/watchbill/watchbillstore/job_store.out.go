...
// watchbillKeyPrefixPredicate renders ScanOpts.KeyPrefix: keys starting with
// prefix. startsWith on the leading sort-key column is a primary-key
// range read, not a scan of every row.
func watchbillKeyPrefixPredicate(prefix string) string {
	return "startsWith(" + JobColKey + ", " + watchbillKeyLiteral(prefix) + ")"
}
...
// OpenJobStore is NewJobStore plus the startup sequence every caller otherwise
...
// caller's layer (ADR-0105 D1). NewJobStore panics on a malformed config
// and so does this. A caller that cannot afford the DESCRIBE at startup
// keeps using NewJobStore.
func OpenJobStore(ctx context.Context, exec recordstore.ExecutorI, alloc memory.Allocator, cfg JobStoreConfig) (inst *JobStore, err error) {
	st := NewJobStore(exec, alloc, cfg)
	err = st.EnsureTable(ctx)
	if err != nil {
		st.Close()
		err = eh.Errorf("open jobStore: %w", err)
		return
	}
...
func (inst *JobStore) FlushIfDue(ctx context.Context) (n int, err error) {
...
func (inst *JobStore) ScanJob(ctx context.Context, opts recordstore.ScanOpts) iter.Seq2[*JobEntity, error] {
	...
	if opts.KeyPrefix != "" {
		where = "(" + where + ") AND " + watchbillKeyPrefixPredicate(opts.KeyPrefix)
	}
