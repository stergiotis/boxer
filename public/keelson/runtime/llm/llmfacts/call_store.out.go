...
//
// CallStore composes the generated facts building blocks per ADR-0100:
// append-only primitives and iterator query verbs; batched
// cached retrieval is the attachable CallCache view. Reads see only
...
import (
	...
	"github.com/stergiotis/boxer/public/keelson/runtime/llm/llmfacts/internal/lowlevel"
...
// CallTableName is the ClickHouse table this store binds — database-
// qualified ("<db>.<table>") when a Database was set at generation.
const CallTableName = "boxer.facts"
...
const (
	CallColKey   = `"id:id:u64:47::0:"`
	CallColOrder = `"ts:ts:z64:47::0:"`
)
...
// CallMembershipIds is the membership-id assignment this store was
...
var CallMembershipIds = map[string]map[string]uint64{
	"LlmCall": {
		"runtimeKindLlmCall":     9223372049739677798,
		"llmCallId":              9223372049739677799,
		"llmCallApp":             9223372049739677800,
		"llmCallInstance":        9223372049739677801,
		"llmCallPurpose":         9223372049739677802,
		"llmCallSensitivity":     9223372049739677803,
		"llmCallModel":           9223372049739677804,
		"llmCallEndpointHost":    9223372049739677805,
		"llmCallMessages":        9223372049739677806,
		"llmCallTools":           9223372049739677807,
		"llmCallPromptBytes":     9223372049739677808,
		"llmCallCompletionBytes": 9223372049739677809,
		"llmCallInputTokens":     9223372049739677810,
		"llmCallOutputTokens":    9223372049739677811,
		"llmCallToolCalls":       9223372049739677812,
		"llmCallFinishReason":    9223372049739677813,
		"llmCallElapsedMs":       9223372049739677814,
		"llmCallIncomplete":      9223372049739677815,
		"llmCallRefused":         9223372049739677816,
		"llmCallError":           9223372049739677817,
	},
}

// CallEnvelope carries the pass-through backbone columns — every plain
...
// CallEntity is the entity bag (ADR-0100 SD5): the envelope plus one option
...
type CallEntity struct {
	ID uint64
	Ts time.Time
	// CallEnvelope is embedded: its pass-through columns read as
...
func (inst *CallEntity) Archetype() (a []string) {
	if inst.LlmCall.Has {
		a = append(a, "llmCall")
	}
	return
...
type CallStoreConfig struct {
	...
	// CallTableName — for every statement it issues (DDL, DESCRIBE, INSERT,
...
// CallStore is single-goroutine, like every part it composes. Batched
// cached retrieval is not built in — attach a CallCache view.
type CallStore struct {
	...
	// views (NewCallCache registers one pair per view): onWrite populates
	...
	onWrite []func(uint64, *CallEntity)
	onFlush []func(uint64)
	// stampers is cfg.Stampers, consulted per Begin (ADR-0112 M1).
	stampers []recordstore.ReferenceStamper
}

// NewCallStore wires the store. A nil alloc selects the Go allocator.
func NewCallStore(exec recordstore.ExecutorI, alloc memory.Allocator, cfg CallStoreConfig) (inst *CallStore) {
	if cfg.Table != "" {
		if terr := recordstore.CheckTableRef(cfg.Table); terr != nil {
			panic("CallStore: " + terr.Error())
		}
	}
	if alloc == nil {
		alloc = memory.NewGoAllocator()
	}
	inst = &CallStore{exec: exec, alloc: alloc, cfg: cfg, dml: lowlevel.NewInEntityFactsTable(alloc, 64), dirty: make(map[uint64]struct{}), stampers: cfg.Stampers}
	return
}

// tableName is the table reference every statement uses: the configured
// override when set, else the baked CallTableName.
func (inst *CallStore) tableName() string {
	if inst.cfg.Table != "" {
...
func (inst *CallStore) applyStampers() (pushed int) {
...
// the key instead.
func (inst *CallStore) notifyWrite(key uint64, ent *CallEntity) {
...
func (inst *CallStore) notifyFlush(key uint64) {
...
// CallMembershipIds against the writer's assignment when pointing this
...
func (inst *CallStore) VerifySchema(ctx context.Context) (err error) {
...
// OpenCallStore is NewCallStore plus the startup check: it runs VerifySchema and
...
// caller's layer (ADR-0105 D1). NewCallStore panics on a malformed config
// and so does this. A caller that cannot afford the DESCRIBE at startup
// keeps using NewCallStore.
func OpenCallStore(ctx context.Context, exec recordstore.ExecutorI, alloc memory.Allocator, cfg CallStoreConfig) (inst *CallStore, err error) {
	st := NewCallStore(exec, alloc, cfg)
	err = st.VerifySchema(ctx)
	if err != nil {
		st.Close()
		err = eh.Errorf("open callStore: %w", err)
		return
	}
	inst = st
	return
...
// CallEntityBuilder assembles one entity: envelope from Begin, components
// via Add*, direct attribute manipulation via Raw, then Commit.
type CallEntityBuilder struct {
	store *CallStore
	key   uint64
...
func (inst *CallEntityBuilder) endSection(section string) error {
	switch section {
	case "symbol":
...
func (inst *CallStore) Begin(id uint64, ts time.Time, env CallEnvelope) *CallEntityBuilder {
	...
	b := &CallEntityBuilder{store: inst, key: id, ent: CallEntity{ID: id, Ts: ts, CallEnvelope: env}}
	b.pushed = inst.applyStampers()
	return b
}

// AddLlmCall contributes the LlmCall component to the open entity.
//
...
func (inst *CallEntityBuilder) AddLlmCall(row LlmCall) *CallEntityBuilder {
	if err := inst.buf.StartKind("LlmCall"); err != nil {
		inst.store.dml.AppendError(err)
		return inst
	}
	inst.buf.Enqueue("symbol", "LlmCall", func() error {
		return llmCallEmitSectionSymbol(inst.store.dml.GetSectionSymbol(), row)
	})
	inst.buf.Enqueue("u64Array", "LlmCall", func() error {
		return llmCallEmitSectionU64Array(inst.store.dml.GetSectionU64Array(), row)
	})
	inst.buf.Enqueue("u32Array", "LlmCall", func() error {
		return llmCallEmitSectionU32Array(inst.store.dml.GetSectionU32Array(), row)
	})
	inst.buf.Enqueue("bool", "LlmCall", func() error {
		return llmCallEmitSectionBool(inst.store.dml.GetSectionBool(), row)
	})
	inst.buf.Enqueue("stringArray", "LlmCall", func() error {
		return llmCallEmitSectionStringArray(inst.store.dml.GetSectionStringArray(), row)
	})
	inst.ent.LlmCall = option.Some(row)
	return inst
}
...
func (inst *CallEntityBuilder) Raw() *lowlevel.InEntityFactsTable {
...
func (inst *CallEntityBuilder) Commit() (err error) {
...
func (inst *CallEntityBuilder) Rollback() (err error) {
...
// them.
func (inst *CallStore) IngestLlmCall(ts time.Time, rows []LlmCall) (err error) {
	seen := make(map[uint64]struct{}, len(rows))
	for i := range rows {
		if _, dup := seen[rows[i].Id]; dup {
			err = eh.Errorf("ingest llmCall row %d: %w: key %v", i, recordstore.ErrDuplicateIngestKey, rows[i].Id)
			return
		}
...
func (inst *CallStore) Buffered() int { return inst.buffered }
...
func (inst *CallStore) Flush(ctx context.Context) (n int, err error) {
...
func (inst *CallStore) FlushIfDue(ctx context.Context) (n int, err error) {
...
func (inst *CallStore) DiscardPending() {
...
func (inst *CallStore) Close() {
	inst.DiscardPending()
	lowlevel.InEntityFactsTableReleaseBuilder(inst.dml)
}

// CallCacheConfig parameterizes an attached read-through cache view.
...
// CallCache is the batched read-through, write-through KV view over a
// CallStore (ADR-0100 SD5): misses queue under work items and flush
...
type CallCache[W comparable] struct {
	st    *CallStore
	cfg   CallCacheConfig
	cache *caching.ReadThroughCache[uint64, *CallEntity, W]
}

// NewCallCache attaches a read-through, write-through cache view to st,
// registering its write-through and flush hooks with the store. Views
// attach for the store's lifetime — there is no detach.
func NewCallCache[W comparable](st *CallStore, cfg CallCacheConfig) (inst *CallCache[W]) {
	if cfg.Capacity <= 0 {
		cfg.Capacity = 1024
	}
	inst = &CallCache[W]{st: st, cfg: cfg}
	inst.rebuild()
	// The hooks close over the view, not the cache instance, keeping
	// them correct if the cache is ever swapped out.
	st.onWrite = append(st.onWrite, func(k uint64, ent *CallEntity) {
		if ent == nil {
...
func (inst *CallCache[W]) rebuild() {
	opts := []caching.CacheOption[uint64, *CallEntity, W]{
		// Admission mirrors the table's newest-row-per-key semantics:
		// the Order timestamp is the entity's monotonic version.
		caching.WithVersioning[uint64, *CallEntity, W](func(e *CallEntity) int64 { return e.Ts.UnixNano() }),
	}
	if inst.cfg.FreshnessTTL > 0 {
		opts = append(opts, caching.WithFreshnessTTL[uint64, *CallEntity, W](inst.cfg.FreshnessTTL))
	}
	if inst.cfg.NegativeTTL > 0 {
		opts = append(opts, caching.WithNegativeCaching[uint64, *CallEntity, W](inst.cfg.NegativeTTL))
	}
	inst.cache = caching.NewReadThroughCache[uint64, *CallEntity, W](inst.cfg.Capacity, &factsFetcher{st: inst.st}, inst.cfg.FetchCriteria, opts...)
}
...
func (inst *CallCache[W]) Get(key uint64) (ent *CallEntity, found bool) {
...
func (inst *CallCache[W]) GetFetch(ctx context.Context, key uint64) (ent *CallEntity, found bool, err error) {
...
func (inst *CallCache[W]) WorkItem(w W) iter.Seq[functional.NilIteratorValueType] {
...
func (inst *CallCache[W]) IterateReadyWorkItems(ctx context.Context) iter.Seq[W] {
...
func (inst *CallCache[W]) IterateRestWorkItems(ctx context.Context) iter.Seq[W] {
...
func (inst *CallCache[W]) AdvanceEpoch() {
...
func (inst *CallCache[W]) MarkStale(key uint64) {
...
func (inst *CallCache[W]) MarkStaleIfOlder(key uint64, order time.Time) {
	inst.cache.MarkAsStaleIfOlder(key, order.UnixNano())
}

// Invalidate drops the key's cached entry (L1 and stash).
func (inst *CallCache[W]) Invalidate(key uint64) {
...
func (inst *CallStore) fetchLatestSQL(keys []uint64) string {
	...
	sb.WriteString(") ORDER BY " + CallColOrder + " DESC LIMIT 1 BY " + CallColKey)
...
// public method set.
type factsFetcher struct{ st *CallStore }

var _ caching.ItemFetcherI[uint64, *CallEntity] = (*factsFetcher)(nil)
...
// compare against.
func (inst *factsFetcher) FetchItemSinglePartition(ctx context.Context, partition uint64, keys []uint64, target caching.ItemTargetI[uint64, *CallEntity]) (err error) {
...
const (
	factsScanLlmCallFilter = "hasAll(\"tv:symbol:lr:lr:u64:1247:::0::data\", [9223372049739677798, 9223372049739677799, 9223372049739677800, 9223372049739677802, 9223372049739677803, 9223372049739677804, 9223372049739677805, 9223372049739677813]) AND hasAll(\"tv:u64Array:lr:lr:u64:1247:::0::data\", [9223372049739677801, 9223372049739677808, 9223372049739677809, 9223372049739677814]) AND hasAll(\"tv:u32Array:lr:lr:u64:1247:::0::data\", [9223372049739677806, 9223372049739677807, 9223372049739677810, 9223372049739677811, 9223372049739677812]) AND hasAll(\"tv:bool:lr:lr:u64:1247:::0::data\", [9223372049739677815, 9223372049739677816]) AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677798) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677799) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677800) = 1 AND countEqual(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677801) = 1 AND if(has(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677801), \"tv:u64Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677801), arrayCumSum(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677802) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677803) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677804) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677805) = 1 AND countEqual(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677806) = 1 AND if(has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677806), \"tv:u32Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677806), arrayCumSum(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677807) = 1 AND if(has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677807), \"tv:u32Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677807), arrayCumSum(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677808) = 1 AND if(has(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677808), \"tv:u64Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677808), arrayCumSum(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677809) = 1 AND if(has(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677809), \"tv:u64Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677809), arrayCumSum(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677810) = 1 AND if(has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677810), \"tv:u32Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677810), arrayCumSum(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677811) = 1 AND if(has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677811), \"tv:u32Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677811), arrayCumSum(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677812) = 1 AND if(has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677812), \"tv:u32Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677812), arrayCumSum(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677813) = 1 AND countEqual(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677814) = 1 AND if(has(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677814), \"tv:u64Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677814), arrayCumSum(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:bool:lr:lr:u64:1247:::0::data\", 9223372049739677815) = 1 AND countEqual(\"tv:bool:lr:lr:u64:1247:::0::data\", 9223372049739677816) = 1 AND countEqual(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 9223372049739677817) <= 1"
)

// ScanLlmCall iterates the entities whose rows carry a conforming LlmCall
...
// rows.
func (inst *CallStore) ScanLlmCall(ctx context.Context, opts recordstore.ScanOpts) iter.Seq2[*CallEntity, error] {
	if opts.KeyPrefix != "" {
		return recordstore.RefuseKeyPrefix[*CallEntity]()
	}
	where := factsScanLlmCallFilter
	if opts.ExtraPredicate != "" {
		where = "(" + where + ") AND (" + opts.ExtraPredicate + ")"
	}
	sql := "SELECT * FROM " + inst.tableName() +
		" WHERE " + where +
		" ORDER BY " + CallColOrder + " ASC, " + CallColKey + " ASC"
	if opts.Limit > 0 {
...
// flushed rows.
func (inst *CallStore) Latest(ctx context.Context, key uint64) (ent *CallEntity, found bool, err error) {
	sql := "SELECT * FROM " + inst.tableName() +
		" WHERE " + CallColKey + " = " + factsKeyLiteral(key) +
		" ORDER BY " + CallColOrder + " DESC LIMIT 1" + factsArrowOutputSettings
...
func (inst *CallStore) Replay(ctx context.Context, key uint64, fromOrder time.Time, opts recordstore.ReplayOpts) iter.Seq2[*CallEntity, error] {
	sql := "SELECT * FROM " + inst.tableName() +
		" WHERE " + CallColKey + " = " + factsKeyLiteral(key)
	if !fromOrder.IsZero() {
		sql += " AND " + CallColOrder + " >= fromUnixTimestamp64Nano(" + strconv.FormatInt(fromOrder.UnixNano(), 10) + ")"
	}
	if !opts.To.IsZero() {
		sql += " AND " + CallColOrder + " < fromUnixTimestamp64Nano(" + strconv.FormatInt(opts.To.UnixNano(), 10) + ")"
	}
	sql += " ORDER BY " + CallColOrder + " ASC"
	if opts.Limit > 0 {
		sql += " LIMIT " + strconv.Itoa(opts.Limit)
	}
	sql += factsArrowOutputSettings
...
// CallComponentSQL publishes this store's ADR-0066 read-back artefacts —
...
// join must bind them to CallTableName itself (ADR-0189 SD6).
var CallComponentSQL = componentsql.Set{
	Store: "Call",
	Table: CallTableName,
	Kinds: map[string]componentsql.Artefacts{
		"LlmCall": {
			Presence:   "hasAll(\"tv:symbol:lr:lr:u64:1247:::0::data\", [9223372049739677798, 9223372049739677799, 9223372049739677800, 9223372049739677802, 9223372049739677803, 9223372049739677804, 9223372049739677805, 9223372049739677813]) AND hasAll(\"tv:u64Array:lr:lr:u64:1247:::0::data\", [9223372049739677801, 9223372049739677808, 9223372049739677809, 9223372049739677814]) AND hasAll(\"tv:u32Array:lr:lr:u64:1247:::0::data\", [9223372049739677806, 9223372049739677807, 9223372049739677810, 9223372049739677811, 9223372049739677812]) AND hasAll(\"tv:bool:lr:lr:u64:1247:::0::data\", [9223372049739677815, 9223372049739677816])",
			Validator:  "countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677798) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677799) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677800) = 1 AND countEqual(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677801) = 1 AND if(has(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677801), \"tv:u64Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677801), arrayCumSum(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677802) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677803) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677804) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677805) = 1 AND countEqual(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677806) = 1 AND if(has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677806), \"tv:u32Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677806), arrayCumSum(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677807) = 1 AND if(has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677807), \"tv:u32Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677807), arrayCumSum(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677808) = 1 AND if(has(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677808), \"tv:u64Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677808), arrayCumSum(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677809) = 1 AND if(has(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677809), \"tv:u64Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677809), arrayCumSum(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677810) = 1 AND if(has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677810), \"tv:u32Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677810), arrayCumSum(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677811) = 1 AND if(has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677811), \"tv:u32Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677811), arrayCumSum(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677812) = 1 AND if(has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677812), \"tv:u32Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677812), arrayCumSum(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677813) = 1 AND countEqual(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677814) = 1 AND if(has(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677814), \"tv:u64Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677814), arrayCumSum(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:bool:lr:lr:u64:1247:::0::data\", 9223372049739677815) = 1 AND countEqual(\"tv:bool:lr:lr:u64:1247:::0::data\", 9223372049739677816) = 1 AND countEqual(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 9223372049739677817) <= 1",
			Filter:     factsScanLlmCallFilter,
			Projection: "CAST(tuple(\"id:id:u64:47::0:\", \"id:naturalKey:y:4::0:\", \"ts:ts:z64:47::0:\", LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677798, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677799, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677800, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_LIST_BY_TAG_EQUAL(\"tv:u64Array:value:val:u64h:4:::0::data\", \"tv:u64Array:len:len:u64:4D:::0::data\", \"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677801, LW_RAGGED_PARENT_IDS(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677802, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677803, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677804, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677805, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_LIST_BY_TAG_EQUAL(\"tv:u32Array:value:val:u32h:4:::0::data\", \"tv:u32Array:len:len:u64:4D:::0::data\", \"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677806, LW_RAGGED_PARENT_IDS(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_LIST_BY_TAG_EQUAL(\"tv:u32Array:value:val:u32h:4:::0::data\", \"tv:u32Array:len:len:u64:4D:::0::data\", \"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677807, LW_RAGGED_PARENT_IDS(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_LIST_BY_TAG_EQUAL(\"tv:u64Array:value:val:u64h:4:::0::data\", \"tv:u64Array:len:len:u64:4D:::0::data\", \"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677808, LW_RAGGED_PARENT_IDS(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_LIST_BY_TAG_EQUAL(\"tv:u64Array:value:val:u64h:4:::0::data\", \"tv:u64Array:len:len:u64:4D:::0::data\", \"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677809, LW_RAGGED_PARENT_IDS(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_LIST_BY_TAG_EQUAL(\"tv:u32Array:value:val:u32h:4:::0::data\", \"tv:u32Array:len:len:u64:4D:::0::data\", \"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677810, LW_RAGGED_PARENT_IDS(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_LIST_BY_TAG_EQUAL(\"tv:u32Array:value:val:u32h:4:::0::data\", \"tv:u32Array:len:len:u64:4D:::0::data\", \"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677811, LW_RAGGED_PARENT_IDS(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_LIST_BY_TAG_EQUAL(\"tv:u32Array:value:val:u32h:4:::0::data\", \"tv:u32Array:len:len:u64:4D:::0::data\", \"tv:u32Array:lr:lr:u64:1247:::0::data\", 9223372049739677812, LW_RAGGED_PARENT_IDS(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 9223372049739677813, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_LIST_BY_TAG_EQUAL(\"tv:u64Array:value:val:u64h:4:::0::data\", \"tv:u64Array:len:len:u64:4D:::0::data\", \"tv:u64Array:lr:lr:u64:1247:::0::data\", 9223372049739677814, LW_RAGGED_PARENT_IDS(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_VALUE_BY_TAG_EQUAL(\"tv:bool:value:val:b:4:::0::data\", \"tv:bool:lr:lr:u64:1247:::0::data\", 9223372049739677815, LW_RAGGED_PARENT_IDS(\"tv:bool:lrcard:lrcard:u64:4E:::0::data\")), LW_VALUE_BY_TAG_EQUAL(\"tv:bool:value:val:b:4:::0::data\", \"tv:bool:lr:lr:u64:1247:::0::data\", 9223372049739677816, LW_RAGGED_PARENT_IDS(\"tv:bool:lrcard:lrcard:u64:4E:::0::data\")), LW_LIST_BY_TAG_EQUAL(\"tv:stringArray:value:val:sh:4::8:0::data\", \"tv:stringArray:len:len:u64:4D:::0::data\", \"tv:stringArray:lr:lr:u64:1247:::0::data\", 9223372049739677817, LW_RAGGED_PARENT_IDS(\"tv:stringArray:lrcard:lrcard:u64:4E:::0::data\"))), 'Tuple(Id UInt64, NaturalKey String, Ts DateTime64(9,\\'UTC\\'), Kind String, CallId String, App String, Instance UInt64, Purpose String, Sensitivity String, Model String, EndpointHost String, Messages UInt32, Tools UInt32, PromptBytes UInt64, CompletionBytes UInt64, InputTokens UInt32, OutputTokens UInt32, ToolCalls UInt32, FinishReason String, ElapsedMs UInt64, Incomplete Bool, Refused Bool, Error Array(String))')",
		},
	},
}
...
func (inst *CallStore) queryEntities(ctx context.Context, sql string) (ents []*CallEntity, err error) {
	for rec, rerr := range inst.exec.QueryArrow(ctx, sql) {
		if rerr != nil {
			err = eh.Errorf("query entities: %w", rerr)
			return
		}
		batch, derr := decodeCallRecord(rec)
		rec.Release()
		if derr != nil {
			err = derr
			return
...
// the sequence.
func (inst *CallStore) iterateEntities(ctx context.Context, sql string) iter.Seq2[*CallEntity, error] {
	return func(yield func(*CallEntity, error) bool) {
		ents, err := inst.queryEntities(ctx, sql)
		if err != nil {
			yield(nil, err)
			return
...
// decodeCallRecord reads one fetched Arrow record into entity bags:
...
func decodeCallRecord(rec arrow.RecordBatch) (ents []*CallEntity, err error) {
	...
	readers := []factsSectionReaderI{idR, tsR, lcR, symbolR, u64ArrayR, u32ArrayR, boolR, stringArrayR}
	...
	for i := range n {
		...
		{
			row, ok, e := llmCallReadRow(i, symbolR.GetAttributes(), symbolR.GetMemberships(), u64ArrayR.GetAttributes(), u64ArrayR.GetMemberships(), u32ArrayR.GetAttributes(), u32ArrayR.GetMemberships(), boolR.GetAttributes(), boolR.GetMemberships(), stringArrayR.GetAttributes(), stringArrayR.GetMemberships())
			if e != nil {
				err = eh.Errorf("read llmCall component: %w", e)
				return
			}
			if ok {
				row.Id = ent.ID
				row.Ts = ent.Ts
				ent.LlmCall = option.Some(row)
			}
		}
