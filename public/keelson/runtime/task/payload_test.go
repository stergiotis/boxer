...
func TestTaskCreated_MarshalRoundTrip(t *testing.T) {
	// AtNs lands as u32 unix seconds on the wire — use a
	...
	in := taskcreated.TaskCreated{
		...
		CancellableB: true,
		EstimatedMs:  30_000,
		AtNs:         1_700_000_000 * 1_000_000_000,
	}
...
func TestTaskProgress_MarshalRoundTrip(t *testing.T) {
	// AtNs lands as u32 unix seconds on the wire, so use a
	...
	in := taskprogress.TaskProgress{
		...
		EtaMs:            11_000,
		Note:             "encoding",
		AtNs:             1_700_000_001 * 1_000_000_000,
	}
...
func TestTaskDone_MarshalRoundTrip(t *testing.T) {
	// AtNs lands as u32 unix seconds — use a second-aligned value.
	...
	in := taskdone.TaskDone{
		FactId: 5,
		TaskId: "abc123",
		AtNs:   1_700_000_010 * 1_000_000_000,
...
func TestTaskCancel_MarshalRoundTrip(t *testing.T) {
	...
	in := taskcancel.TaskCancel{
		FactId: 9,
		TaskId: "abc123",
		AtNs:   1_700_000_005 * 1_000_000_000,
