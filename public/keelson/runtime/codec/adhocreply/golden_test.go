...
var updateGolden = flag.Bool("update", false, "rewrite adhocreply.out.go with the current generator output")

// TestGeneratorMatchesCheckedInOutput runs the keelson codec generator
// on adhocreply.go and asserts the bytes match the committed adhocreply.out.go. Rerun
// with `-update` after intentional generator changes.
func TestGeneratorMatchesCheckedInOutput(t *testing.T) {
	_, thisFile, _, _ := runtime.Caller(0)
	dir := filepath.Dir(thisFile)
	inputPath := filepath.Join(dir, "adhocreply.go")
	outputPath := filepath.Join(dir, "adhocreply.out.go")
