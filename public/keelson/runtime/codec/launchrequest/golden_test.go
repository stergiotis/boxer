...
var updateGolden = flag.Bool("update", false, "rewrite launchrequest.out.go with the current generator output")

// TestGeneratorMatchesCheckedInOutput runs the keelson codec generator
// on launchrequest.go and asserts the bytes match the committed
// launchrequest.out.go. Rerun with `-update` after intentional
// generator changes.
func TestGeneratorMatchesCheckedInOutput(t *testing.T) {
	_, thisFile, _, _ := runtime.Caller(0)
	dir := filepath.Dir(thisFile)
	inputPath := filepath.Join(dir, "launchrequest.go")
	outputPath := filepath.Join(dir, "launchrequest.out.go")
