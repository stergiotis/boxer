...
var updateGolden = flag.Bool("update", false, "rewrite capabilitygrant.out.go with the current generator output")

// TestGeneratorMatchesCheckedInOutput runs the keelson codec generator
// on capabilitygrant.go and asserts the bytes match the committed
// capabilitygrant.out.go. Rerun with `-update` after intentional
// generator changes.
func TestGeneratorMatchesCheckedInOutput(t *testing.T) {
	_, thisFile, _, _ := runtime.Caller(0)
	dir := filepath.Dir(thisFile)
	inputPath := filepath.Join(dir, "capabilitygrant.go")
	outputPath := filepath.Join(dir, "capabilitygrant.out.go")
