...
var updateGolden = flag.Bool("update", false, "rewrite instanceclosed.out.go with the current generator output")

// TestGeneratorMatchesCheckedInOutput runs the keelson codec generator
// on instanceclosed.go and asserts the bytes match the committed instanceclosed.out.go. Rerun
// with `-update` after intentional generator changes.
func TestGeneratorMatchesCheckedInOutput(t *testing.T) {
	_, thisFile, _, _ := runtime.Caller(0)
	dir := filepath.Dir(thisFile)
	inputPath := filepath.Join(dir, "instanceclosed.go")
	outputPath := filepath.Join(dir, "instanceclosed.out.go")
