...
var updateGolden = flag.Bool("update", false, "rewrite keelsonqueryreply.out.go with the current generator output")

// TestGeneratorMatchesCheckedInOutput runs the keelson codec generator
// on keelsonqueryreply.go and asserts the bytes match the committed
// keelsonqueryreply.out.go. Rerun with `-update` after intentional
// generator changes.
func TestGeneratorMatchesCheckedInOutput(t *testing.T) {
	_, thisFile, _, _ := runtime.Caller(0)
	dir := filepath.Dir(thisFile)
	inputPath := filepath.Join(dir, "keelsonqueryreply.go")
	outputPath := filepath.Join(dir, "keelsonqueryreply.out.go")
