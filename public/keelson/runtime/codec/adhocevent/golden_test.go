...
var updateGolden = flag.Bool("update", false, "rewrite adhocevent.out.go with the current generator output")

// TestGeneratorMatchesCheckedInOutput runs the keelson codec generator
// on adhocevent.go and asserts the bytes match the committed adhocevent.out.go. Rerun
// with `-update` after intentional generator changes.
func TestGeneratorMatchesCheckedInOutput(t *testing.T) {
	_, thisFile, _, _ := runtime.Caller(0)
	dir := filepath.Dir(thisFile)
	inputPath := filepath.Join(dir, "adhocevent.go")
	outputPath := filepath.Join(dir, "adhocevent.out.go")
