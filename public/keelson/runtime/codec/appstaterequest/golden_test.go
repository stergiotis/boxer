...
var updateGolden = flag.Bool("update", false, "rewrite appstaterequest.out.go with the current generator output")

// TestGeneratorMatchesCheckedInOutput runs the keelson codec generator
// on appstaterequest.go and asserts the bytes match the committed
// appstaterequest.out.go. Rerun with `-update` after intentional
// generator changes.
func TestGeneratorMatchesCheckedInOutput(t *testing.T) {
	_, thisFile, _, _ := runtime.Caller(0)
	dir := filepath.Dir(thisFile)
	inputPath := filepath.Join(dir, "appstaterequest.go")
	outputPath := filepath.Join(dir, "appstaterequest.out.go")
