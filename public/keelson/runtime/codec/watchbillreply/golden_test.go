...
var updateGolden = flag.Bool("update", false, "rewrite watchbillreply.out.go with the current generator output")

// TestGeneratorMatchesCheckedInOutput runs the keelson codec generator
// on watchbillreply.go and asserts the bytes match the committed
// watchbillreply.out.go. Rerun with `-update` after intentional
// generator changes.
func TestGeneratorMatchesCheckedInOutput(t *testing.T) {
	_, thisFile, _, _ := runtime.Caller(0)
	dir := filepath.Dir(thisFile)
	inputPath := filepath.Join(dir, "watchbillreply.go")
	outputPath := filepath.Join(dir, "watchbillreply.out.go")
