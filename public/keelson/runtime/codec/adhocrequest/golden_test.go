...
var updateGolden = flag.Bool("update", false, "rewrite adhocrequest.out.go with the current generator output")

// TestGeneratorMatchesCheckedInOutput runs the keelson codec generator
// on adhocrequest.go and asserts the bytes match the committed adhocrequest.out.go. Rerun
// with `-update` after intentional generator changes.
func TestGeneratorMatchesCheckedInOutput(t *testing.T) {
	_, thisFile, _, _ := runtime.Caller(0)
	dir := filepath.Dir(thisFile)
	inputPath := filepath.Join(dir, "adhocrequest.go")
	outputPath := filepath.Join(dir, "adhocrequest.out.go")
