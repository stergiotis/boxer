...
var updateGolden = flag.Bool("update", false, "rewrite launchreply.out.go with the current generator output")

// TestGeneratorMatchesCheckedInOutput runs the keelson codec generator
// on launchreply.go and asserts the bytes match the committed
// launchreply.out.go. Rerun with `-update` after intentional
// generator changes.
func TestGeneratorMatchesCheckedInOutput(t *testing.T) {
	_, thisFile, _, _ := runtime.Caller(0)
	dir := filepath.Dir(thisFile)
	inputPath := filepath.Join(dir, "launchreply.go")
	outputPath := filepath.Join(dir, "launchreply.out.go")
