...
var updateGolden = flag.Bool("update", false, "rewrite keelsonqueryrequest.out.go with the current generator output")

// TestGeneratorMatchesCheckedInOutput runs the keelson codec generator
// on keelsonqueryrequest.go and asserts the bytes match the committed
// keelsonqueryrequest.out.go. Rerun with `-update` after intentional
// generator changes.
func TestGeneratorMatchesCheckedInOutput(t *testing.T) {
	_, thisFile, _, _ := runtime.Caller(0)
	dir := filepath.Dir(thisFile)
	inputPath := filepath.Join(dir, "keelsonqueryrequest.go")
	outputPath := filepath.Join(dir, "keelsonqueryrequest.out.go")
