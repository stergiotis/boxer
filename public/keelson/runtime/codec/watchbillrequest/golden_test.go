...
var updateGolden = flag.Bool("update", false, "rewrite watchbillrequest.out.go with the current generator output")

// TestGeneratorMatchesCheckedInOutput runs the keelson codec generator
// on watchbillrequest.go and asserts the bytes match the committed
// watchbillrequest.out.go. Rerun with `-update` after intentional
// generator changes.
func TestGeneratorMatchesCheckedInOutput(t *testing.T) {
	_, thisFile, _, _ := runtime.Caller(0)
	dir := filepath.Dir(thisFile)
	inputPath := filepath.Join(dir, "watchbillrequest.go")
	outputPath := filepath.Join(dir, "watchbillrequest.out.go")
