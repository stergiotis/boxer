...
var updateGolden = flag.Bool("update", false, "rewrite errkind.out.go with the current generator output")

func TestGeneratorMatchesCheckedInOutput(t *testing.T) {
	_, thisFile, _, _ := runtime.Caller(0)
	dir := filepath.Dir(thisFile)
	inputPath := filepath.Join(dir, "errkind.go")
	outputPath := filepath.Join(dir, "errkind.out.go")
