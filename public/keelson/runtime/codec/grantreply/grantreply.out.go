...
func GrantReplyFillFromArrow[
	...
	for i := 0; i < n; i++ {
		...
		if boolApprovedCount != 1 {
			err = eb.Build().Int("row", i).Str("section", "bool").Str("membership", "grantApproved").Int("got", boolApprovedCount).Errorf("slot bool@grantApproved (field Approved) carries %d attributes but the DTO admits exactly 1 — several producers claim this slot, so the reader cannot tell which attribute is this kind's", boolApprovedCount)
			return
		}
		...
		var stringArrayGrantIdLastAttr int64
		nstringArray := stringArrayAttrs.GetNumberOfAttributes(raruntime.EntityIdx(i))
		for attrJ := int64(0); attrJ < nstringArray; attrJ++ {
			for membID := range stringArrayMembs.GetMembValueLowCardRef(raruntime.EntityIdx(i), raruntime.AttributeIdx(attrJ)) {
				switch membID {
				case kindGrantId:
					if stringArrayGrantIdLastAttr != attrJ+1 {
						stringArrayGrantIdLastAttr = attrJ + 1
						stringArrayGrantIdCount++
					}
					val, valErr := stringArrayAttrs.GetAttrValueSingle(raruntime.EntityIdx(i), raruntime.AttributeIdx(attrJ))
					if valErr != nil {
						err = eb.Build().Int("row", i).Str("section", "stringArray").Str("membership", "grantId").Str("field", "GrantId").Errorf("slot stringArray@grantId (field GrantId) has an attribute carrying other than one value, but the field's `,unit` shape admits exactly one: %w", valErr)
						return
					}
		...
		if stringArrayGrantIdCount != 1 {
			err = eb.Build().Int("row", i).Str("section", "stringArray").Str("membership", "grantId").Int("got", stringArrayGrantIdCount).Errorf("slot stringArray@grantId (field GrantId) carries %d attributes but the DTO admits exactly 1 — several producers claim this slot, so the reader cannot tell which attribute is this kind's", stringArrayGrantIdCount)
			return
		}
...
func GrantReplyReadRow[
	...
	if boolApprovedCount > 1 {
		err = eb.Build().Int("row", i).Str("section", "bool").Str("membership", "grantApproved").Int("got", boolApprovedCount).Errorf("slot bool@grantApproved (field Approved) carries %d attributes but the DTO admits at most 1 — several producers claim this slot, so the reader cannot tell which attribute is this kind's", boolApprovedCount)
		return
	}
	if boolApprovedCount == 1 {
	...
	if stringArrayGrantIdCount > 1 {
		err = eb.Build().Int("row", i).Str("section", "stringArray").Str("membership", "grantId").Int("got", stringArrayGrantIdCount).Errorf("slot stringArray@grantId (field GrantId) carries %d attributes but the DTO admits at most 1 — several producers claim this slot, so the reader cannot tell which attribute is this kind's", stringArrayGrantIdCount)
		return
	}
	if stringArrayGrantIdCount == 1 {
