...
var updateGolden = flag.Bool("update", false, "rewrite appstatereply.out.go with the current generator output")

// TestGeneratorMatchesCheckedInOutput runs the keelson codec generator
// on appstatereply.go and asserts the bytes match the committed
// appstatereply.out.go. Rerun with `-update` after intentional
// generator changes.
func TestGeneratorMatchesCheckedInOutput(t *testing.T) {
	_, thisFile, _, _ := runtime.Caller(0)
	dir := filepath.Dir(thisFile)
	inputPath := filepath.Join(dir, "appstatereply.go")
	outputPath := filepath.Join(dir, "appstatereply.out.go")
