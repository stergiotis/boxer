...
// co-located wiring the carousel host, the screenshot tour, and tests will
// share (Stage B). Linux-only: StartScraper builds the real collector set,
// which reads /proc.
func TestStartScraper_PublishesOverBus(t *testing.T) {
	...
	t.Cleanup(cancel)
	stop, err := sysmetricsbus.StartScraper(ctx, pub, "testhost", 30*time.Millisecond, zerolog.Nop())
	if err != nil {
