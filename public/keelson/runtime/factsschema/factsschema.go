...
func LoadRuntimeFactsMapping(manip common.TableManipulatorFluidI) {
	...
	}
	{
		...
		// text vs string: both are utf8 arrays, and before ADR-0168 §SD4 they
		// generated byte-identical columns — the section names claimed a
		// distinction nothing downstream could read. The split is now carried by
		// value semantics, and by a codec chosen for what each holds: prose has
		// redundancy worth ZSTD(12), machine-facing strings do not.
		{
			...
			sec.TaggedValueColumn("value", strh).
				...
				AddColumnEncodingHints(easp.AspectHeavyGeneralCompression)
		}
		{
