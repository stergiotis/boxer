...
func TestPebbleStash_SoftCapEviction(t *testing.T) {
	...
	assert.True(t, s.Add("c", caching.StashEntry[int]{Value: 3}), "Add at softCap must evict")
	assert.Equal(t, 2, s.Len(), "Len stays at softCap after eviction")

	// Update of an existing key never evicts and never grows count.
	assert.False(t, s.Add("c", caching.StashEntry[int]{Value: 33}), "update of present key does not evict")
	assert.Equal(t, 2, s.Len())
