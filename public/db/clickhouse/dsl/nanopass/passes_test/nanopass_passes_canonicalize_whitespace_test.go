...
func TestNormalizeWhitespace(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(fmt.Sprintf("case_%d", i), func(t *testing.T) {
			got, err := passes.CanonicalizeWhitespace.Run(tt.input)
...
func TestNormalizeWhitespaceSingleLine(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(fmt.Sprintf("case_%d", i), func(t *testing.T) {
			got, err := passes.CanonicalizeWhitespaceSingleLine.Run(tt.input)
			require.NoError(t, err)
			assert.Equal(t, tt.expected, got)
		})
	}
...
func TestNormalizeWhitespaceIdempotent(t *testing.T) {
	sql := "SELECT  a  \n\n  FROM  t  WHERE   b > 1"
	pass1, err := passes.CanonicalizeWhitespace.Run(sql)
	require.NoError(t, err)
	pass2, err := passes.CanonicalizeWhitespace.Run(pass1)
	require.NoError(t, err)
...
func TestNormalizeWhitespaceOutputValidity(t *testing.T) {
	...
	for _, entry := range entries {
		t.Run(entry.Name, func(t *testing.T) {
			out, err := passes.CanonicalizeWhitespaceSingleLine.Run(entry.SQL)
			if err != nil {
