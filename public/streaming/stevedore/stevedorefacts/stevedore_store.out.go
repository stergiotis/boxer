...
//
// StevedoreStore composes the generated facts building blocks per ADR-0100:
// append-only primitives and iterator query verbs; batched
// cached retrieval is the attachable StevedoreCache view. Reads see only
...
import (
	...
	"github.com/stergiotis/boxer/public/streaming/stevedore/stevedorefacts/internal/lowlevel"
)

// StevedoreTableName is the ClickHouse table this store binds — database-
// qualified ("<db>.<table>") when a Database was set at generation.
const StevedoreTableName = "boxer.facts"
...
const (
	StevedoreColKey   = `"id:id:u64:47::0:"`
	StevedoreColOrder = `"ts:ts:z64:47::0:"`
)
...
// StevedoreMembershipIds is the membership-id assignment this store was
...
var StevedoreMembershipIds = map[string]map[string]uint64{
	"DeadLetter": {
		"stevedoreKindDeadLetter": 576460765188325408,
		"stevedoreDeadRef":        576460765188325409,
		"stevedoreDeadOrigin":     576460765188325410,
		"stevedoreDeadOrdinal":    576460765188325411,
		"stevedoreDeadClass":      576460765188325412,
		"stevedoreDeadError":      576460765188325413,
		"stevedoreDeadTopic":      576460765188325414,
		"stevedoreDeadPartition":  576460765188325415,
		"stevedoreDeadOffset":     576460765188325416,
		"stevedoreDeadMessage":    576460765188325417,
	},
}

// StevedoreEnvelope carries the pass-through backbone columns — every plain
...
// the entity.
type StevedoreEnvelope struct {
	NaturalKey []byte
	ExpiresAt  time.Time
}

// StevedoreEntity is the entity bag (ADR-0100 SD5): the envelope plus one option
...
type StevedoreEntity struct {
	ID uint64
	Ts time.Time
	// StevedoreEnvelope is embedded: its pass-through columns read as
	// promoted entity fields.
	StevedoreEnvelope
	DeadLetter option.Option[DeadLetter]
}

// Archetype reports which components the entity carries, in schema order.
func (inst *StevedoreEntity) Archetype() (a []string) {
	if inst.DeadLetter.Has {
		a = append(a, "deadLetter")
	}
	return
}
...
// StevedoreStore is single-goroutine, like every part it composes. Batched
// cached retrieval is not built in — attach a StevedoreCache view.
type StevedoreStore struct {
	exec     recordstore.ExecutorI
	alloc    memory.Allocator
	cfg      StevedoreStoreConfig
	...
	// views (NewStevedoreCache registers one pair per view): onWrite populates
	...
	onWrite []func(uint64, *StevedoreEntity)
	onFlush []func(uint64)
	// stampers is cfg.Stampers, consulted per Begin (ADR-0112 M1).
	stampers []recordstore.ReferenceStamper
}

// NewStevedoreStore wires the store. A nil alloc selects the Go allocator.
func NewStevedoreStore(exec recordstore.ExecutorI, alloc memory.Allocator, cfg StevedoreStoreConfig) (inst *StevedoreStore) {
	if cfg.Table != "" {
	...
	inst = &StevedoreStore{exec: exec, alloc: alloc, cfg: cfg, dml: lowlevel.NewInEntityFactsTable(alloc, 64), dirty: make(map[uint64]struct{}), stampers: cfg.Stampers}
	return
}
...
func (inst *StevedoreStore) applyStampers() (pushed int) {
...
// the key instead.
func (inst *StevedoreStore) notifyWrite(key uint64, ent *StevedoreEntity) {
...
func (inst *StevedoreStore) notifyFlush(key uint64) {
...
// StevedoreMembershipIds against the writer's assignment when pointing this
...
// OpenStevedoreStore is NewStevedoreStore plus the startup check: it runs VerifySchema and
...
// caller's layer (ADR-0105 D1). NewStevedoreStore panics on a malformed config
// and so does this. A caller that cannot afford the DESCRIBE at startup
// keeps using NewStevedoreStore.
func OpenStevedoreStore(ctx context.Context, exec recordstore.ExecutorI, alloc memory.Allocator, cfg StevedoreStoreConfig) (inst *StevedoreStore, err error) {
	st := NewStevedoreStore(exec, alloc, cfg)
	err = st.VerifySchema(ctx)
	if err != nil {
		st.Close()
		err = eh.Errorf("open stevedoreStore: %w", err)
		return
	}
	inst = st
	return
...
// StevedoreEntityBuilder assembles one entity: envelope from Begin, components
// via Add*, direct attribute manipulation via Raw, then Commit.
type StevedoreEntityBuilder struct {
	store *StevedoreStore
	key   uint64
...
func (inst *StevedoreEntityBuilder) endSection(section string) error {
	switch section {
	case "symbol":
...
func (inst *StevedoreStore) Begin(id uint64, ts time.Time, env StevedoreEnvelope) *StevedoreEntityBuilder {
	...
	b := &StevedoreEntityBuilder{store: inst, key: id, ent: StevedoreEntity{ID: id, Ts: ts, StevedoreEnvelope: env}}
	b.pushed = inst.applyStampers()
	return b
}

// AddDeadLetter contributes the DeadLetter component to the open entity.
//
...
func (inst *StevedoreEntityBuilder) AddDeadLetter(row DeadLetter) *StevedoreEntityBuilder {
	if err := inst.buf.StartKind("DeadLetter"); err != nil {
		inst.store.dml.AppendError(err)
		return inst
	}
	inst.buf.Enqueue("symbol", "DeadLetter", func() error {
		return deadLetterEmitSectionSymbol(inst.store.dml.GetSectionSymbol(), row)
	})
	inst.buf.Enqueue("u64Array", "DeadLetter", func() error {
		return deadLetterEmitSectionU64Array(inst.store.dml.GetSectionU64Array(), row)
	})
	inst.buf.Enqueue("stringArray", "DeadLetter", func() error {
		return deadLetterEmitSectionStringArray(inst.store.dml.GetSectionStringArray(), row)
	})
	inst.buf.Enqueue("i32Array", "DeadLetter", func() error {
		return deadLetterEmitSectionI32Array(inst.store.dml.GetSectionI32Array(), row)
	})
	inst.buf.Enqueue("i64Array", "DeadLetter", func() error {
		return deadLetterEmitSectionI64Array(inst.store.dml.GetSectionI64Array(), row)
	})
	inst.buf.Enqueue("blobArray", "DeadLetter", func() error {
		return deadLetterEmitSectionBlobArray(inst.store.dml.GetSectionBlobArray(), row)
	})
	inst.ent.DeadLetter = option.Some(row)
	return inst
}
...
func (inst *StevedoreEntityBuilder) Raw() *lowlevel.InEntityFactsTable {
...
func (inst *StevedoreEntityBuilder) Commit() (err error) {
...
func (inst *StevedoreEntityBuilder) Rollback() (err error) {
...
// them.
func (inst *StevedoreStore) IngestDeadLetter(ts time.Time, rows []DeadLetter) (err error) {
	seen := make(map[uint64]struct{}, len(rows))
	for i := range rows {
		if _, dup := seen[rows[i].Id]; dup {
			err = eh.Errorf("ingest deadLetter row %d: %w: key %v", i, recordstore.ErrDuplicateIngestKey, rows[i].Id)
			return
		}
...
func (inst *StevedoreStore) Buffered() int { return inst.buffered }
...
func (inst *StevedoreStore) Flush(ctx context.Context) (n int, err error) {
...
func (inst *StevedoreStore) FlushIfDue(ctx context.Context) (n int, err error) {
...
func (inst *StevedoreStore) DiscardPending() {
...
func (inst *StevedoreStore) Close() {
	inst.DiscardPending()
	lowlevel.InEntityFactsTableReleaseBuilder(inst.dml)
}

// StevedoreCacheConfig parameterizes an attached read-through cache view.
type StevedoreCacheConfig struct {
...
// StevedoreCache is the batched read-through, write-through KV view over a
// StevedoreStore (ADR-0100 SD5): misses queue under work items and flush
...
type StevedoreCache[W comparable] struct {
	st    *StevedoreStore
	cfg   StevedoreCacheConfig
	cache *caching.ReadThroughCache[uint64, *StevedoreEntity, W]
}

// NewStevedoreCache attaches a read-through, write-through cache view to st,
// registering its write-through and flush hooks with the store. Views
// attach for the store's lifetime — there is no detach.
func NewStevedoreCache[W comparable](st *StevedoreStore, cfg StevedoreCacheConfig) (inst *StevedoreCache[W]) {
	if cfg.Capacity <= 0 {
		cfg.Capacity = 1024
	}
	inst = &StevedoreCache[W]{st: st, cfg: cfg}
	inst.rebuild()
	// The hooks close over the view, not the cache instance, keeping
	// them correct if the cache is ever swapped out.
	st.onWrite = append(st.onWrite, func(k uint64, ent *StevedoreEntity) {
		if ent == nil {
...
func (inst *StevedoreCache[W]) rebuild() {
	opts := []caching.CacheOption[uint64, *StevedoreEntity, W]{
		// Admission mirrors the table's newest-row-per-key semantics:
		// the Order timestamp is the entity's monotonic version.
		caching.WithVersioning[uint64, *StevedoreEntity, W](func(e *StevedoreEntity) int64 { return e.Ts.UnixNano() }),
	}
	if inst.cfg.FreshnessTTL > 0 {
		opts = append(opts, caching.WithFreshnessTTL[uint64, *StevedoreEntity, W](inst.cfg.FreshnessTTL))
	}
	if inst.cfg.NegativeTTL > 0 {
		opts = append(opts, caching.WithNegativeCaching[uint64, *StevedoreEntity, W](inst.cfg.NegativeTTL))
	}
	inst.cache = caching.NewReadThroughCache[uint64, *StevedoreEntity, W](inst.cfg.Capacity, &factsFetcher{st: inst.st}, inst.cfg.FetchCriteria, opts...)
}
...
func (inst *StevedoreCache[W]) Get(key uint64) (ent *StevedoreEntity, found bool) {
...
func (inst *StevedoreCache[W]) GetFetch(ctx context.Context, key uint64) (ent *StevedoreEntity, found bool, err error) {
...
func (inst *StevedoreCache[W]) WorkItem(w W) iter.Seq[functional.NilIteratorValueType] {
...
func (inst *StevedoreCache[W]) IterateReadyWorkItems(ctx context.Context) iter.Seq[W] {
...
func (inst *StevedoreCache[W]) IterateRestWorkItems(ctx context.Context) iter.Seq[W] {
...
func (inst *StevedoreCache[W]) AdvanceEpoch() {
...
func (inst *StevedoreCache[W]) MarkStale(key uint64) {
...
func (inst *StevedoreCache[W]) MarkStaleIfOlder(key uint64, order time.Time) {
	inst.cache.MarkAsStaleIfOlder(key, order.UnixNano())
}

// Invalidate drops the key's cached entry (L1 and stash).
func (inst *StevedoreCache[W]) Invalidate(key uint64) {
...
func (inst *StevedoreCache[W]) InvalidateAll() {
	inst.cache.Clear()
}
...
func (inst *StevedoreStore) fetchLatestSQL(keys []uint64) string {
	...
	sb.WriteString(") ORDER BY " + StevedoreColOrder + " DESC LIMIT 1 BY " + StevedoreColKey)
...
// public method set.
type factsFetcher struct{ st *StevedoreStore }

var _ caching.ItemFetcherI[uint64, *StevedoreEntity] = (*factsFetcher)(nil)
...
// compare against.
func (inst *factsFetcher) FetchItemSinglePartition(ctx context.Context, partition uint64, keys []uint64, target caching.ItemTargetI[uint64, *StevedoreEntity]) (err error) {
...
const (
	factsScanDeadLetterFilter = "hasAll(\"tv:symbol:lr:lr:u64:1247:::0::data\", [576460765188325408, 576460765188325412, 576460765188325414]) AND hasAll(\"tv:u64Array:lr:lr:u64:1247:::0::data\", [576460765188325409, 576460765188325411]) AND hasAll(\"tv:stringArray:lr:lr:u64:1247:::0::data\", [576460765188325410, 576460765188325413]) AND has(\"tv:i32Array:lr:lr:u64:1247:::0::data\", 576460765188325415) AND has(\"tv:i64Array:lr:lr:u64:1247:::0::data\", 576460765188325416) AND has(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 576460765188325417) AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 576460765188325408) = 1 AND countEqual(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 576460765188325409) = 1 AND if(has(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 576460765188325409), \"tv:u64Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 576460765188325409), arrayCumSum(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 576460765188325410) = 1 AND if(has(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 576460765188325410), \"tv:stringArray:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 576460765188325410), arrayCumSum(\"tv:stringArray:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 576460765188325411) = 1 AND if(has(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 576460765188325411), \"tv:u64Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 576460765188325411), arrayCumSum(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 576460765188325412) = 1 AND countEqual(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 576460765188325413) = 1 AND if(has(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 576460765188325413), \"tv:stringArray:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 576460765188325413), arrayCumSum(\"tv:stringArray:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 576460765188325414) = 1 AND countEqual(\"tv:i32Array:lr:lr:u64:1247:::0::data\", 576460765188325415) = 1 AND if(has(\"tv:i32Array:lr:lr:u64:1247:::0::data\", 576460765188325415), \"tv:i32Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:i32Array:lr:lr:u64:1247:::0::data\", 576460765188325415), arrayCumSum(\"tv:i32Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:i64Array:lr:lr:u64:1247:::0::data\", 576460765188325416) = 1 AND if(has(\"tv:i64Array:lr:lr:u64:1247:::0::data\", 576460765188325416), \"tv:i64Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:i64Array:lr:lr:u64:1247:::0::data\", 576460765188325416), arrayCumSum(\"tv:i64Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 576460765188325417) = 1 AND if(has(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 576460765188325417), \"tv:blobArray:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 576460765188325417), arrayCumSum(\"tv:blobArray:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1"
)

// ScanDeadLetter iterates the entities whose rows carry a conforming DeadLetter
...
// rows.
func (inst *StevedoreStore) ScanDeadLetter(ctx context.Context, opts recordstore.ScanOpts) iter.Seq2[*StevedoreEntity, error] {
	if opts.KeyPrefix != "" {
		return recordstore.RefuseKeyPrefix[*StevedoreEntity]()
	}
	where := factsScanDeadLetterFilter
...
func (inst *StevedoreStore) Replay(ctx context.Context, key uint64, fromOrder time.Time, opts recordstore.ReplayOpts) iter.Seq2[*StevedoreEntity, error] {
	...
	if !opts.To.IsZero() {
		sql += " AND " + StevedoreColOrder + " < fromUnixTimestamp64Nano(" + strconv.FormatInt(opts.To.UnixNano(), 10) + ")"
	}
	sql += " ORDER BY " + StevedoreColOrder + " ASC"
	if opts.Limit > 0 {
		sql += " LIMIT " + strconv.Itoa(opts.Limit)
	}
	sql += factsArrowOutputSettings
...
// StevedoreComponentSQL publishes this store's ADR-0066 read-back artefacts —
...
// join must bind them to StevedoreTableName itself (ADR-0189 SD6).
var StevedoreComponentSQL = componentsql.Set{
	Store: "Stevedore",
	Table: StevedoreTableName,
	Kinds: map[string]componentsql.Artefacts{
		"DeadLetter": {
			Presence:   "hasAll(\"tv:symbol:lr:lr:u64:1247:::0::data\", [576460765188325408, 576460765188325412, 576460765188325414]) AND hasAll(\"tv:u64Array:lr:lr:u64:1247:::0::data\", [576460765188325409, 576460765188325411]) AND hasAll(\"tv:stringArray:lr:lr:u64:1247:::0::data\", [576460765188325410, 576460765188325413]) AND has(\"tv:i32Array:lr:lr:u64:1247:::0::data\", 576460765188325415) AND has(\"tv:i64Array:lr:lr:u64:1247:::0::data\", 576460765188325416) AND has(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 576460765188325417)",
			Validator:  "countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 576460765188325408) = 1 AND countEqual(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 576460765188325409) = 1 AND if(has(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 576460765188325409), \"tv:u64Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 576460765188325409), arrayCumSum(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 576460765188325410) = 1 AND if(has(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 576460765188325410), \"tv:stringArray:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 576460765188325410), arrayCumSum(\"tv:stringArray:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 576460765188325411) = 1 AND if(has(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 576460765188325411), \"tv:u64Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 576460765188325411), arrayCumSum(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 576460765188325412) = 1 AND countEqual(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 576460765188325413) = 1 AND if(has(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 576460765188325413), \"tv:stringArray:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 576460765188325413), arrayCumSum(\"tv:stringArray:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 576460765188325414) = 1 AND countEqual(\"tv:i32Array:lr:lr:u64:1247:::0::data\", 576460765188325415) = 1 AND if(has(\"tv:i32Array:lr:lr:u64:1247:::0::data\", 576460765188325415), \"tv:i32Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:i32Array:lr:lr:u64:1247:::0::data\", 576460765188325415), arrayCumSum(\"tv:i32Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:i64Array:lr:lr:u64:1247:::0::data\", 576460765188325416) = 1 AND if(has(\"tv:i64Array:lr:lr:u64:1247:::0::data\", 576460765188325416), \"tv:i64Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:i64Array:lr:lr:u64:1247:::0::data\", 576460765188325416), arrayCumSum(\"tv:i64Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 576460765188325417) = 1 AND if(has(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 576460765188325417), \"tv:blobArray:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 576460765188325417), arrayCumSum(\"tv:blobArray:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1",
			Filter:     factsScanDeadLetterFilter,
			Projection: "CAST(tuple(\"id:id:u64:47::0:\", \"id:naturalKey:y:4::0:\", \"ts:ts:z64:47::0:\", LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 576460765188325408, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_LIST_BY_TAG_EQUAL(\"tv:u64Array:value:val:u64h:4:::0::data\", \"tv:u64Array:len:len:u64:4D:::0::data\", \"tv:u64Array:lr:lr:u64:1247:::0::data\", 576460765188325409, LW_RAGGED_PARENT_IDS(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_LIST_BY_TAG_EQUAL(\"tv:stringArray:value:val:sh:4::8:0::data\", \"tv:stringArray:len:len:u64:4D:::0::data\", \"tv:stringArray:lr:lr:u64:1247:::0::data\", 576460765188325410, LW_RAGGED_PARENT_IDS(\"tv:stringArray:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_LIST_BY_TAG_EQUAL(\"tv:u64Array:value:val:u64h:4:::0::data\", \"tv:u64Array:len:len:u64:4D:::0::data\", \"tv:u64Array:lr:lr:u64:1247:::0::data\", 576460765188325411, LW_RAGGED_PARENT_IDS(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 576460765188325412, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_LIST_BY_TAG_EQUAL(\"tv:stringArray:value:val:sh:4::8:0::data\", \"tv:stringArray:len:len:u64:4D:::0::data\", \"tv:stringArray:lr:lr:u64:1247:::0::data\", 576460765188325413, LW_RAGGED_PARENT_IDS(\"tv:stringArray:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 576460765188325414, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_LIST_BY_TAG_EQUAL(\"tv:i32Array:value:val:i32h:4:::0::data\", \"tv:i32Array:len:len:u64:4D:::0::data\", \"tv:i32Array:lr:lr:u64:1247:::0::data\", 576460765188325415, LW_RAGGED_PARENT_IDS(\"tv:i32Array:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_LIST_BY_TAG_EQUAL(\"tv:i64Array:value:val:i64h:4:::0::data\", \"tv:i64Array:len:len:u64:4D:::0::data\", \"tv:i64Array:lr:lr:u64:1247:::0::data\", 576460765188325416, LW_RAGGED_PARENT_IDS(\"tv:i64Array:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_LIST_BY_TAG_EQUAL(\"tv:blobArray:value:val:yh:4:::0::data\", \"tv:blobArray:len:len:u64:4D:::0::data\", \"tv:blobArray:lr:lr:u64:1247:::0::data\", 576460765188325417, LW_RAGGED_PARENT_IDS(\"tv:blobArray:lrcard:lrcard:u64:4E:::0::data\"))[1]), 'Tuple(Id UInt64, NaturalKey String, Ts DateTime64(9,\\'UTC\\'), Kind String, Ref UInt64, Origin String, Ordinal UInt64, Class String, Error String, Topic String, Partition Int32, Offset Int64, Message String)')",
		},
	},
}
...
func (inst *StevedoreStore) queryEntities(ctx context.Context, sql string) (ents []*StevedoreEntity, err error) {
	for rec, rerr := range inst.exec.QueryArrow(ctx, sql) {
		if rerr != nil {
			err = eh.Errorf("query entities: %w", rerr)
			return
		}
		batch, derr := decodeStevedoreRecord(rec)
		rec.Release()
		if derr != nil {
			err = derr
			return
...
// the sequence.
func (inst *StevedoreStore) iterateEntities(ctx context.Context, sql string) iter.Seq2[*StevedoreEntity, error] {
	return func(yield func(*StevedoreEntity, error) bool) {
		ents, err := inst.queryEntities(ctx, sql)
		if err != nil {
			yield(nil, err)
			return
...
// decodeStevedoreRecord reads one fetched Arrow record into entity bags:
...
func decodeStevedoreRecord(rec arrow.RecordBatch) (ents []*StevedoreEntity, err error) {
	...
	readers := []factsSectionReaderI{idR, tsR, lcR, symbolR, u64ArrayR, stringArrayR, i32ArrayR, i64ArrayR, blobArrayR}
	...
	for i := range n {
		...
		{
			row, ok, e := deadLetterReadRow(i, symbolR.GetAttributes(), symbolR.GetMemberships(), u64ArrayR.GetAttributes(), u64ArrayR.GetMemberships(), stringArrayR.GetAttributes(), stringArrayR.GetMemberships(), i32ArrayR.GetAttributes(), i32ArrayR.GetMemberships(), i64ArrayR.GetAttributes(), i64ArrayR.GetMemberships(), blobArrayR.GetAttributes(), blobArrayR.GetMemberships())
			if e != nil {
				err = eh.Errorf("read deadLetter component: %w", e)
				return
			}
			if ok {
				row.Id = ent.ID
				row.Ts = ent.Ts
				ent.DeadLetter = option.Some(row)
			}
		}
