...
func TestSample_Laptop_EnergyUnits(t *testing.T) {
	...
	assert.Equal(t, sysmsnap.StateDischarging, bat.State)
...
func TestSample_Laptop_ChargeUnits_Charging(t *testing.T) {
	...
	assert.Equal(t, sysmsnap.StateCharging, bat.State)
...
func TestSample_Full_StatusIsFull(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, snap.Batteries, 1)
	assert.Equal(t, sysmsnap.StateFull, snap.Batteries[0].State)
...
func TestSample_StatusUnknown_FallsBackToACOnline(t *testing.T) {
	...
	snap, err := c.Sample(context.Background())
	require.NoError(t, err)
	assert.Equal(t, sysmsnap.StateCharging, snap.Batteries[0].State)
	...
	require.NoError(t, err)
	assert.Equal(t, sysmsnap.StateDischarging, snap.Batteries[0].State)
}
...
func TestStateE_String(t *testing.T) {
	assert.Equal(t, "charging", sysmsnap.StateCharging.String())
	assert.Equal(t, "discharging", sysmsnap.StateDischarging.String())
	assert.Equal(t, "full", sysmsnap.StateFull.String())
	assert.Equal(t, "not_charging", sysmsnap.StateNotCharging.String())
	assert.Equal(t, "unknown", sysmsnap.StateUnknown.String())
}
