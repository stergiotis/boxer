...
//
// Two complementary views land in each [sysmsnap.DiskSnapshot]:
//
//   - [sysmsnap.DiskMount] describes one /proc/self/mounts entry: device, mountpoint,
//     filesystem type, and the [sysmsnap.DiskCapacity] computed via `statvfs(3)`.
//   - [sysmsnap.BlockDevice] describes one /sys/class/block/{name}/stat counter
...
//     same BlockDevice via [sysmsnap.DiskMount.BlockName].
//
