...
// [GPUDevice.Vendor] for forward-compatibility (third-party adapters can
// stamp arbitrary vendor strings without recompiling).
type VendorE uint8
...
// GPUDevice is the unified per-GPU sample. Only fields meaningful across
// every vendor live here; per-vendor richness (per-engine busy %, encoder
// utilization, etc.) belongs in the vendor-specific snapshot.
...
// GPUSnapshot is a single sample of every visible GPU across vendors.
