...
type ProvenanceStoreConfig struct {
	...
	// ProvenanceTableName — for every statement it issues (DDL, DESCRIBE, INSERT,
...
func NewProvenanceStore(exec recordstore.ExecutorI, alloc memory.Allocator, cfg ProvenanceStoreConfig) (inst *ProvenanceStore) {
	...
	if cfg.Table != "" {
		if terr := recordstore.CheckTableRef(cfg.Table); terr != nil {
			panic("ProvenanceStore: " + terr.Error())
		}
	}
	if alloc == nil {
...
// override when set, else the baked ProvenanceTableName.
func (inst *ProvenanceStore) tableName() string {
	if inst.cfg.Table != "" {
		return inst.cfg.Table
	}
	return ProvenanceTableName
}
...
func (inst *ProvenanceStore) EnsureTable(ctx context.Context) (err error) {
	stmts, err := recordstore.ProvisioningStatements(provenanceDDLCreate, ProvenanceTableName, inst.cfg.Table)
	if err != nil {
...
// OpenProvenanceStore is NewProvenanceStore plus the startup sequence every caller otherwise
...
// caller's layer (ADR-0105 D1). NewProvenanceStore panics on a malformed config
// and so does this. A caller that cannot afford the DESCRIBE at startup
// keeps using NewProvenanceStore.
func OpenProvenanceStore(ctx context.Context, exec recordstore.ExecutorI, alloc memory.Allocator, cfg ProvenanceStoreConfig) (inst *ProvenanceStore, err error) {
	st := NewProvenanceStore(exec, alloc, cfg)
	err = st.EnsureTable(ctx)
	if err != nil {
		st.Close()
		err = eh.Errorf("open provenanceStore: %w", err)
		return
	}
...
func (inst *ProvenanceEntityBuilder) endSection(section string) error {
	switch section {
	case "symbol":
...
// AddProvenance contributes the Provenance component to the open entity.
//
...
func (inst *ProvenanceEntityBuilder) AddProvenance(row Provenance) *ProvenanceEntityBuilder {
	if err := inst.buf.StartKind("Provenance"); err != nil {
		inst.store.dml.AppendError(err)
		return inst
	}
	inst.buf.Enqueue("symbol", "Provenance", func() error {
		return provenanceEmitSectionSymbol(inst.store.dml.GetSectionSymbol(), row)
	})
	inst.buf.Enqueue("symbolArray", "Provenance", func() error {
		return provenanceEmitSectionSymbolArray(inst.store.dml.GetSectionSymbolArray(), row)
	})
...
func (inst *ProvenanceStore) FlushIfDue(ctx context.Context) (n int, err error) {
...
func (inst *ProvenanceStore) ScanProvenance(ctx context.Context, opts recordstore.ScanOpts) iter.Seq2[*ProvenanceEntity, error] {
	if opts.KeyPrefix != "" {
		return recordstore.RefuseKeyPrefix[*ProvenanceEntity]()
	}
