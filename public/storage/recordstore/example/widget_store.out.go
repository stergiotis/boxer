...
type WidgetStoreConfig struct {
	...
	// (Entity.IsTombstone).
	TombstoneDetect func(*WidgetEntity) bool
	...
	TombstoneWrite func(*WidgetEntityBuilder)
}
...
func NewWidgetStore(exec recordstore.ExecutorI, alloc memory.Allocator, cfg WidgetStoreConfig) (inst *WidgetStore) {
	if (cfg.TombstoneDetect == nil) != (cfg.TombstoneWrite == nil) {
		panic("WidgetStore: the tombstone pair comes whole — supply both TombstoneDetect and TombstoneWrite, or neither")
	}
	if cfg.Table != "" {
		if terr := recordstore.CheckTableRef(cfg.Table); terr != nil {
			panic("WidgetStore: " + terr.Error())
		}
	}
	if alloc == nil {
...
// override when set, else the baked WidgetTableName.
func (inst *WidgetStore) tableName() string {
	if inst.cfg.Table != "" {
...
func (inst *WidgetStore) isTombstone(e *WidgetEntity) bool {
...
func (inst *WidgetStore) EnsureTable(ctx context.Context) (err error) {
	stmts, err := recordstore.ProvisioningStatements(widgetDDLCreate, WidgetTableName, inst.cfg.Table)
	if err != nil {
...
func (inst *WidgetStore) VerifySchema(ctx context.Context) (err error) {
	...
	for rec, rerr := range inst.exec.QueryArrow(ctx, "DESCRIBE (SELECT * FROM "+inst.tableName()+")"+widgetArrowOutputSettings) {
		if rerr != nil {
...
// OpenWidgetStore is NewWidgetStore plus the startup sequence every caller otherwise
...
// caller's layer (ADR-0105 D1). NewWidgetStore panics on a malformed config
// and so does this. A caller that cannot afford the DESCRIBE at startup
// keeps using NewWidgetStore.
func OpenWidgetStore(ctx context.Context, exec recordstore.ExecutorI, alloc memory.Allocator, cfg WidgetStoreConfig) (inst *WidgetStore, err error) {
	st := NewWidgetStore(exec, alloc, cfg)
	err = st.EnsureTable(ctx)
	if err != nil {
		st.Close()
		err = eh.Errorf("open widgetStore: %w", err)
		return
	}
...
func (inst *WidgetStore) FlushIfDue(ctx context.Context) (n int, err error) {
...
func (inst *WidgetStore) Delete(id uint64, ts time.Time) (err error) {
	if inst.cfg.TombstoneWrite != nil {
		b := inst.Begin(id, ts, WidgetEnvelope{})
