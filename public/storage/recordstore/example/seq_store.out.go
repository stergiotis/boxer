...
// OpenSeqStore is NewSeqStore plus the startup sequence every caller otherwise
...
// caller's layer (ADR-0105 D1). NewSeqStore panics on a malformed config
// and so does this. A caller that cannot afford the DESCRIBE at startup
// keeps using NewSeqStore.
func OpenSeqStore(ctx context.Context, exec recordstore.ExecutorI, alloc memory.Allocator, cfg SeqStoreConfig) (inst *SeqStore, err error) {
	st := NewSeqStore(exec, alloc, cfg)
	err = st.EnsureTable(ctx)
	if err != nil {
		st.Close()
		err = eh.Errorf("open seqStore: %w", err)
		return
	}
...
func (inst *SeqStore) FlushIfDue(ctx context.Context) (n int, err error) {
