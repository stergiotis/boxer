...
// DeviceMembershipIds is the membership-id assignment this store was
...
// DeviceMembershipIds against the writer's assignment when pointing this
...
// OpenDeviceStore is NewDeviceStore plus the startup sequence every caller otherwise
...
// caller's layer (ADR-0105 D1). NewDeviceStore panics on a malformed config
// and so does this. A caller that cannot afford the DESCRIBE at startup
// keeps using NewDeviceStore.
func OpenDeviceStore(ctx context.Context, exec recordstore.ExecutorI, alloc memory.Allocator, cfg DeviceStoreConfig) (inst *DeviceStore, err error) {
	st := NewDeviceStore(exec, alloc, cfg)
	err = st.EnsureTable(ctx)
	if err != nil {
		st.Close()
		err = eh.Errorf("open deviceStore: %w", err)
		return
	}
...
func (inst *DeviceStore) FlushIfDue(ctx context.Context) (n int, err error) {
