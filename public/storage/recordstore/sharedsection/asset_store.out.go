...
//
// AssetStore composes the generated asset building blocks per ADR-0100:
// append-only primitives and iterator query verbs; batched
// cached retrieval is the attachable AssetCache view. Reads see only
...
//
//go:embed asset_ddl_clickhouse.out.sql
var assetDDLCreate string

// AssetTableName is the ClickHouse table this store binds — database-
...
const assetArrowOutputSettings = " SETTINGS output_format_arrow_string_as_string=1, output_format_arrow_low_cardinality_as_dictionary=0"

// assetKeyLiteral renders a Key value as a ClickHouse SQL literal.
func assetKeyLiteral(k uint64) string { return strconv.FormatUint(k, 10) }

// AssetMembershipIds is the membership-id assignment this store was
...
// AssetEntity is the entity bag (ADR-0100 SD5): the envelope plus one option
...
// cached retrieval is not built in — attach a AssetCache view.
type AssetStore struct {
	exec     recordstore.ExecutorI
	alloc    memory.Allocator
	cfg      AssetStoreConfig
	dml      *lowlevel.InEntityAssetTable
	buffered int
	...
	// views (NewAssetCache registers one pair per view): onWrite populates
	...
	onWrite []func(uint64, *AssetEntity)
	onFlush []func(uint64)
...
// the key instead.
func (inst *AssetStore) notifyWrite(key uint64, ent *AssetEntity) {
...
func (inst *AssetStore) notifyFlush(key uint64) {
...
// AssetMembershipIds against the writer's assignment when pointing this
...
// OpenAssetStore is NewAssetStore plus the startup sequence every caller otherwise
...
// caller's layer (ADR-0105 D1). NewAssetStore panics on a malformed config
// and so does this. A caller that cannot afford the DESCRIBE at startup
// keeps using NewAssetStore.
func OpenAssetStore(ctx context.Context, exec recordstore.ExecutorI, alloc memory.Allocator, cfg AssetStoreConfig) (inst *AssetStore, err error) {
	st := NewAssetStore(exec, alloc, cfg)
	err = st.EnsureTable(ctx)
	if err != nil {
		st.Close()
		err = eh.Errorf("open assetStore: %w", err)
		return
	}
...
// AssetEntityBuilder assembles one entity: envelope from Begin, components
...
func (inst *AssetEntityBuilder) endSection(section string) error {
	switch section {
	case "symbol":
...
func (inst *AssetEntityBuilder) AddLabel(row Label) *AssetEntityBuilder {
	if err := inst.buf.StartKind("Label"); err != nil {
		inst.store.dml.AppendError(err)
		return inst
	}
	inst.buf.Enqueue("symbol", "Label", func() error {
		return labelEmitSectionSymbol(inst.store.dml.GetSectionSymbol(), row)
	})
	inst.ent.Label = option.Some(row)
	return inst
}
...
func (inst *AssetEntityBuilder) AddState(row State) *AssetEntityBuilder {
	...
	inst.buf.Enqueue("symbol", "State", func() error {
		return stateEmitSectionSymbol(inst.store.dml.GetSectionSymbol(), row)
	})
...
func (inst *AssetEntityBuilder) Commit() (err error) {
	...
	inst.buf.Reset()
	err = lowlevel.InEntityAssetTableCommitEntity(inst.store.dml)
	inst.store.dml.PopMembershipsHighCardRef(inst.pushed) // release Begin's ambient stamps (consumed by the closed attributes)
	if err != nil {
		_ = lowlevel.InEntityAssetTableRollbackEntity(inst.store.dml) // no-op error when no frame is open
		return
	}
	inst.store.buffered++
...
func (inst *AssetEntityBuilder) Rollback() (err error) {
	...
	return lowlevel.InEntityAssetTableRollbackEntity(inst.store.dml)
}

// IngestLabel buffers one whole entity per row carrying only the
// Label component, all stamped with ts — rows ship on the next Flush,
...
func (inst *AssetStore) IngestLabel(ts time.Time, rows []Label) (err error) {
	...
	for i := range rows {
		if _, dup := seen[rows[i].ID]; dup {
			err = eh.Errorf("ingest label row %d: %w: key %v", i, recordstore.ErrDuplicateIngestKey, rows[i].ID)
			return
		}
...
func (inst *AssetStore) Buffered() int { return inst.buffered }
...
func (inst *AssetStore) Flush(ctx context.Context) (n int, err error) {
	...
	records, err := lowlevel.InEntityAssetTableTransferRecords(inst.dml, nil)
	if err != nil {
...
func (inst *AssetStore) FlushIfDue(ctx context.Context) (n int, err error) {
...
func (inst *AssetStore) DiscardPending() {
	_ = lowlevel.InEntityAssetTableRollbackEntity(inst.dml) // no-op error when no frame is open
	inst.dml.ClearMembershipsHighCardRef()                  // an abandoned Begin's stamps must not outlive its frame
	if records, err := lowlevel.InEntityAssetTableTransferRecords(inst.dml, nil); err == nil {
		for _, rec := range records {
			rec.Release()
		}
	}
...
func (inst *AssetStore) Close() {
	inst.DiscardPending()
	lowlevel.InEntityAssetTableReleaseBuilder(inst.dml)
}
...
// AssetCache is the batched read-through, write-through KV view over a
// AssetStore (ADR-0100 SD5): misses queue under work items and flush
...
type AssetCache[W comparable] struct {
	st    *AssetStore
	cfg   AssetCacheConfig
	cache *caching.ReadThroughCache[uint64, *AssetEntity, W]
}

// NewAssetCache attaches a read-through, write-through cache view to st,
...
func (inst *AssetCache[W]) rebuild() {
	opts := []caching.CacheOption[uint64, *AssetEntity, W]{
		// Admission mirrors the table's newest-row-per-key semantics:
		// the Order timestamp is the entity's monotonic version.
		caching.WithVersioning[uint64, *AssetEntity, W](func(e *AssetEntity) int64 { return e.Ts.UnixNano() }),
	}
	if inst.cfg.FreshnessTTL > 0 {
		opts = append(opts, caching.WithFreshnessTTL[uint64, *AssetEntity, W](inst.cfg.FreshnessTTL))
	}
	if inst.cfg.NegativeTTL > 0 {
		opts = append(opts, caching.WithNegativeCaching[uint64, *AssetEntity, W](inst.cfg.NegativeTTL))
	}
	inst.cache = caching.NewReadThroughCache[uint64, *AssetEntity, W](inst.cfg.Capacity, &assetFetcher{st: inst.st}, inst.cfg.FetchCriteria, opts...)
}
...
func (inst *AssetStore) fetchLatestSQL(keys []uint64) string {
	...
	for i, k := range keys {
		...
		sb.WriteString(assetKeyLiteral(k))
	}
	sb.WriteString(") ORDER BY " + AssetColOrder + " DESC LIMIT 1 BY " + AssetColKey)
	sb.WriteString(assetArrowOutputSettings)
	return sb.String()
}

// assetFetcher implements caching.ItemFetcherI for attached cache views —
// an unexported shim so the fetch plumbing stays off the store's
// public method set.
type assetFetcher struct{ st *AssetStore }

var _ caching.ItemFetcherI[uint64, *AssetEntity] = (*assetFetcher)(nil)

// DeterminePartition implements caching.ItemFetcherI. Single partition
// in v1 (one table, one server).
func (inst *assetFetcher) DeterminePartition(key uint64) uint64 { return 0 }
...
// compare against.
func (inst *assetFetcher) FetchItemSinglePartition(ctx context.Context, partition uint64, keys []uint64, target caching.ItemTargetI[uint64, *AssetEntity]) (err error) {
...
func (inst *AssetStore) ScanLabel(ctx context.Context, opts recordstore.ScanOpts) iter.Seq2[*AssetEntity, error] {
	if opts.KeyPrefix != "" {
		return recordstore.RefuseKeyPrefix[*AssetEntity]()
	}
	where := assetScanLabelFilter
...
func (inst *AssetStore) Replay(ctx context.Context, key uint64, fromOrder time.Time, opts recordstore.ReplayOpts) iter.Seq2[*AssetEntity, error] {
	...
	if !opts.To.IsZero() {
		sql += " AND " + AssetColOrder + " < fromUnixTimestamp64Nano(" + strconv.FormatInt(opts.To.UnixNano(), 10) + ")"
	}
	sql += " ORDER BY " + AssetColOrder + " ASC"
	if opts.Limit > 0 {
		sql += " LIMIT " + strconv.Itoa(opts.Limit)
	}
	sql += assetArrowOutputSettings
...
// assetSectionReaderI is the uniform slice of the generated read-access
// readers. Column indices stay at their constructor defaults — the
// schema order a SELECT * returns.
type assetSectionReaderI interface {
	LoadFromRecord(raruntime.RecordI) error
	Release()
}

func (inst *AssetStore) queryEntities(ctx context.Context, sql string) (ents []*AssetEntity, err error) {
	for rec, rerr := range inst.exec.QueryArrow(ctx, sql) {
		if rerr != nil {
			err = eh.Errorf("query entities: %w", rerr)
			return
		}
		batch, derr := decodeAssetRecord(rec)
		rec.Release()
		if derr != nil {
			err = derr
			return
...
// the sequence.
func (inst *AssetStore) iterateEntities(ctx context.Context, sql string) iter.Seq2[*AssetEntity, error] {
	return func(yield func(*AssetEntity, error) bool) {
		ents, err := inst.queryEntities(ctx, sql)
		if err != nil {
			yield(nil, err)
			return
...
// decodeAssetRecord reads one fetched Arrow record into entity bags:
...
func decodeAssetRecord(rec arrow.RecordBatch) (ents []*AssetEntity, err error) {
	idR := lowlevel.NewReadAccessAssetTablePlainEntityIdAttributes()
	tsR := lowlevel.NewReadAccessAssetTablePlainEntityTimestampAttributes()
	symbolR := lowlevel.NewReadAccessAssetTableTaggedSymbol()
	readers := []assetSectionReaderI{idR, tsR, symbolR}
	for _, r := range readers {
		err = r.LoadFromRecord(rec)
		if err != nil {
			err = eh.Errorf("load asset reader from record: %w", err)
			return
		}
	}
	defer func() {
	...
	for i := range n {
		...
		{
			row, ok, e := labelReadRow(i, symbolR.GetAttributes(), symbolR.GetMemberships())
			if e != nil {
				err = eh.Errorf("read label component: %w", e)
				return
			}
			if ok {
				row.ID = ent.ID
				ent.Label = option.Some(row)
			}
		}
		{
			row, ok, e := stateReadRow(i, symbolR.GetAttributes(), symbolR.GetMemberships())
			if e != nil {
