...
const (
	PushoutColKey       = `"id:id:s:4::0:"`
	PushoutColOrder     = `"ts:ts:z64:47::0:"`
	PushoutColLifecycle = `"lc:lifecycle:u8:4::0:"`
)
...
var PushoutMembershipIds = map[string]map[string]uint64{
	"Envelope": {
		"pushoutFramed": 1,
	},
	"LogEntry": {
...
// OpenPushoutStore is NewPushoutStore plus the startup sequence every caller otherwise
...
// caller's layer (ADR-0105 D1). NewPushoutStore panics on a malformed config
// and so does this. A caller that cannot afford the DESCRIBE at startup
// keeps using NewPushoutStore.
func OpenPushoutStore(ctx context.Context, exec recordstore.ExecutorI, alloc memory.Allocator, cfg PushoutStoreConfig) (inst *PushoutStore, err error) {
	st := NewPushoutStore(exec, alloc, cfg)
	err = st.EnsureTable(ctx)
	if err != nil {
		st.Close()
		err = eh.Errorf("open pushoutStore: %w", err)
		return
	}
...
func (inst *PushoutStore) FlushIfDue(ctx context.Context) (n int, err error) {
