...
func (inst Input) emitStoreHeader(sb *strings.Builder, key, order, lifecycle envelopeCol, stateView bool) {
	...
	p("// cached retrieval is the attachable %s view.", inst.cacheType())
...
func (inst Input) emitCacheView(sb *strings.Builder) {
	p := func(format string, args ...any) { fmt.Fprintf(sb, format+"\n", args...) }
	p("// %s parameterizes an attached read-through cache view.", inst.cacheConfigType())
	p("type %s struct {", inst.cacheConfigType())
	p("\t// Capacity is the L1 capacity in entries (not bytes) — size it for")
	p("\t// the largest expected entity payloads. Zero or negative selects")
	p("\t// the default (1024).")
	p("\tCapacity int")
	...
	p("// IN (…) lookup. Intended for immutable-by-key access — see SD4.")
	p("// Local writes on the store evict the written key from every attached")
	p("// view; only external writers can leave Get stale. Like the store it")
	p("// wraps, a view is single-goroutine. W is the work-item type (use")
	...
	p("// local-write invalidation hook with the store.")
	p("func New%s[W comparable](st *%s, cfg %s) (inst *%s[W]) {", inst.cacheType(), inst.storeType(), inst.cacheConfigType(), inst.cacheType())
	p("\tif cfg.Capacity <= 0 {")
	p("\t\tcfg.Capacity = 1024")
	p("\t}")
	p("\tinst = &%s[W]{st: st}", inst.cacheType())
	p("\tinst.cache = caching.NewReadThroughCache[%s, *%s, W](cfg.Capacity, &%sFetcher{st: st}, cfg.FetchCriteria)", inst.keyGoType, inst.entityType(), inst.TableName)
	p("\tst.onWrite = append(st.onWrite, inst.cache.Delete)")
	p("\treturn")
	p("}")
	p("")
	...
	p("// fetch errors; the circuit breaker backs off) — the store's Latest")
	p("// is the authoritative existence check.")
...
func (inst Input) emitQueryVerbs(sb *strings.Builder, comps []storeComponent, stateView bool) {
	...
	p("// in ascending order — the event-replay primitive. The sequence is")
	p("// single-use; ctx must stay valid until iteration completes; the")
	p("// query may execute at call time or lazily during iteration (buffered")
	p("// in v1 — a streaming executor changes nothing visible); an error")
	p("// ends the sequence as a final (nil, err) pair.")
	p("func (inst *%s) Replay(ctx context.Context, key %s, fromOrder time.Time) iter.Seq2[*%s, error] {", inst.storeType(), inst.keyGoType, inst.entityType())
	p("\tsql := \"SELECT * FROM \" + %sTableName +", inst.TableName)
	p("\t\t\" WHERE \" + %sColKey + \" = \" + %sKeyLiteral(key) +", inst.TableName, inst.TableName)
	...
	p("// name.")
	p("func (inst *%s) Put(id %s, ts time.Time) *%s {", inst.storeType(), inst.keyGoType, inst.builderType())
	p("\treturn inst.Begin(id, ts)")
	p("}")
	p("")
	p("// Delete appends a tombstone row for id (no components; lifecycle marks")
	p("// the deletion); attached cache views evict the key. GetLatest reads")
	p("// it as absent.")
	...
	p("func (inst *%s) GetLatest(ctx context.Context, key %s) (ent *%s, found bool, err error) {", inst.storeType(), inst.keyGoType, inst.entityType())
