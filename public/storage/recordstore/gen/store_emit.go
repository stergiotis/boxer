...
func (inst emitter) emitStoreType(sb *strings.Builder) {
	...
	p("\trecords, err := inst.exec.QueryArrow(ctx, \"DESCRIBE TABLE \"+%sTableName+%sArrowOutputSettings)", inst.StoreName, inst.TableName)
	p("\tif err != nil {")
	p("\t\terr = eh.Errorf(\"describe table %%s: %%w\", %sTableName, err)", inst.StoreName)
	p("\t\treturn")
	p("\t}")
	p("\tdefer func() {")
...
func (inst emitter) emitQueryVerbs(sb *strings.Builder, comps []storeComponent, stateView bool) {
	...
	p("// primitive). Reads see only flushed rows.")
	...
	p("// recordstore.SeqTs(0) is the equivalent explicit bound). The")
	p("// sequence is single-use; ctx must stay valid until iteration")
	p("// completes; the query may execute at call time or lazily during")
	p("// iteration (buffered in v1 — a streaming executor changes nothing")
	p("// visible); an error ends the sequence as a final (nil, err) pair.")
	p("// Reads see only flushed rows.")
	...
	p("\t}")
	p("\tsql += \" ORDER BY \" + %sColOrder + \" ASC\" + %sArrowOutputSettings", inst.StoreName, inst.TableName)
	p("\treturn inst.iterateEntities(ctx, sql)")
	p("}")
	p("")
	if !stateView {
