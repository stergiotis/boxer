...
func (inst emitter) emitStoreType(sb *strings.Builder) {
	...
	p("//")
	p("// MATERIALIZED and ALIAS columns are skipped. They are not in SELECT *,")
	p("// so the positional decode never sees them, and a table may legitimately")
	p("// carry them beside the generated shape — derived tree or routing columns")
	p("// added by ALTER after EnsureTable, which is how a store gets skip indexes")
	p("// over values its leeway attributes only encode. Counting them would fail")
	p("// every such table while the contract this verb guards still held.")
	p("func (inst *%s) VerifySchema(ctx context.Context) (err error) {", inst.storeType())
	p("\tlive := make([]string, 0, 64)")
	p("\tfor rec, rerr := range inst.exec.QueryArrow(ctx, \"DESCRIBE TABLE \"+inst.tableName()+%sArrowOutputSettings) {", inst.TableName)
	p("\t\tif rerr != nil {")
	...
	p("\t\t\treturn")
	p("\t\t}")
	// DESCRIBE TABLE's third column is default_type: '' for a stored column,
	// 'DEFAULT' / 'MATERIALIZED' / 'ALIAS' otherwise. Only the last two are
	// absent from SELECT *; a DEFAULT column is an ordinary stored one.
	p("\t\tkinds, ok := rec.Column(2).(*array.String)")
	p("\t\tif !ok {")
	p("\t\t\terr = eh.Errorf(\"describe table %%s: default_type column is %%s, not a string\", inst.tableName(), rec.Column(2).DataType())")
	p("\t\t\trec.Release()")
	p("\t\t\treturn")
	p("\t\t}")
	p("\t\tfor i := range int(rec.NumRows()) {")
	p("\t\t\tswitch kinds.Value(i) {")
	p("\t\t\tcase \"MATERIALIZED\", \"ALIAS\":")
	p("\t\t\t\tcontinue")
	p("\t\t\t}")
