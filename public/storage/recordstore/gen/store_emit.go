...
func (inst emitter) emitCacheView(sb *strings.Builder, stateView bool) {
	...
	p("")
	if stateView {
		p("// GetLatest is the cached state-view read: the cache's newest row for")
		...
		p("// need MarkStale / Invalidate. The store's uncached GetLatest stays")
		p("// the authoritative read. A miss queues the batch fetch like Get.")
		p("func (inst *%s[W]) GetLatest(key %s) (ent *%s, found bool) {", inst.cacheType(), inst.keyGoType, inst.entityType())
		p("\tent, found = inst.cache.Get(key)")
		p("\tif found && ent.Lifecycle == %sLifecycleTombstone {", inst.TableName)
		p("\t\tent = nil")
		p("\t\tfound = false")
		p("\t}")
		p("\treturn")
		p("}")
		p("")
		p("// GetLatestAcceptStale is the stale-while-revalidate state-view read:")
		...
		p("func (inst *%s[W]) GetLatestAcceptStale(key %s) (ent *%s, found bool, stale bool) {", inst.cacheType(), inst.keyGoType, inst.entityType())
