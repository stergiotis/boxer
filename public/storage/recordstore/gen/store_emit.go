...
func (inst emitter) emitStoreType(sb *strings.Builder) {
	...
	p("\t// FlushEvery is the buffered-row count at which FlushIfDue flushes, so")
	p("\t// a caller streaming a source bounds the Arrow memory it holds without")
	p("\t// a counter of its own. Zero (the default) makes FlushIfDue a no-op.")
	...
	p("")
	if inst.ExternallyProvisioned {
		p("// Open%s is New%s plus the startup check: it runs VerifySchema and", inst.storeType(), inst.storeType())
		p("// returns a store ready to Begin. This store cannot create its table")
		p("// (generated ExternallyProvisioned), so a missing table fails here.")
	} else {
		p("// Open%s is New%s plus the startup sequence every caller otherwise", inst.storeType(), inst.storeType())
		p("// repeats: EnsureTable, then VerifySchema, returning a store ready to")
		p("// Begin.")
	}
	p("//")
	p("// On failure the store is closed and nil is returned. It takes an")
	p("// executor, not a client — which executor reaches the server is the")
	p("// caller's layer (ADR-0105 D1). New%s panics on a malformed config", inst.storeType())
	p("// and so does this. A caller that cannot afford the DESCRIBE at startup")
	p("// keeps using New%s.", inst.storeType())
	p("func Open%s(ctx context.Context, exec recordstore.ExecutorI, alloc memory.Allocator, cfg %sStoreConfig) (inst *%s, err error) {", inst.storeType(), inst.StoreName, inst.storeType())
	p("\tst := New%s(exec, alloc, cfg)", inst.storeType())
	if !inst.ExternallyProvisioned {
		p("\terr = st.EnsureTable(ctx)")
		p("\tif err != nil {")
		p("\t\tst.Close()")
		p("\t\terr = eh.Errorf(\"open %s: %%w\", err)", lowerFirst(inst.storeType()))
		p("\t\treturn")
		p("\t}")
	}
	p("\terr = st.VerifySchema(ctx)")
	p("\tif err != nil {")
	p("\t\tst.Close()")
	p("\t\terr = eh.Errorf(\"open %s: %%w\", err)", lowerFirst(inst.storeType()))
	p("\t\treturn")
...
func (inst emitter) emitFlush(sb *strings.Builder) {
	...
	p("}")
	p("")
	p("// FlushIfDue is Flush once cfg.FlushEvery rows are buffered, and a no-op")
	p("// before that — or always, when FlushEvery is zero. Call it between")
	p("// entities (after Commit): like Flush it errors on an open frame. A")
	p("// failure leaves the rows retained exactly as Flush does, so the next")
	p("// FlushIfDue or Flush retries them. Flush stays the end-of-run call —")
	p("// FlushIfDue leaves up to FlushEvery-1 rows buffered.")
	p("func (inst *%s) FlushIfDue(ctx context.Context) (n int, err error) {", inst.storeType())
	p("\tif inst.cfg.FlushEvery <= 0 || inst.buffered < inst.cfg.FlushEvery {")
	p("\t\treturn")
	p("\t}")
	p("\treturn inst.Flush(ctx)")
	p("}")
	p("")
