...
- **Layout** — a left-to-right reel of uniform fixed-width "slot-machine" cells,
  one per member, each split by a rule into a canonical register over a
  byte-size register ("4 B" / "var"), interleaved with the structural boundary
  markers (`-` between members of a group, an accent `_` between groups of a
  signature) and closed by a baseline rule.
