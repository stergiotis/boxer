...
func (p *Plot) emitToolsClipped(tr transform, areaX, areaY, areaW, areaH float32) {
	for ti := range p.tools {
		...
		case toolAnnotation:
			...
			c.PaintText(bx+4, by+bh/2, 0, 1, t.text, tickFontSize, color.Hex(0x10131aff)).Monospace().Send()
		}
	}
}
...
func (p *Plot) emitToolChrome(tr transform, areaX, areaY, areaW, areaH float32, regions bool) {
	for ti := range p.tools {
		...
		case toolTagX:
			...
			c.PaintText(px, areaY+areaH+tickLen+3, 1, 0, lbl, tickFontSize, color.Hex(0x10131aff)).Monospace().Send()
		case toolTagY:
			py := tr.pxY(t.y)
			lbl := formatTick(t.y, 2)
			bw := float32(len(lbl))*charW + 6
			c.PaintRectFilled(areaX-tickLen-bw-1, py-7, areaX-1, py+7, 2.0, color.Hex(t.colHex)).Send()
			c.PaintText(areaX-tickLen-4, py, 2, 1, lbl, tickFontSize, color.Hex(0x10131aff)).Monospace().Send()
		}
	}
}
