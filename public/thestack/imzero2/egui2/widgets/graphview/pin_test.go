...
func TestPinOnDragHoldsWhereTheNodeWasDropped(t *testing.T) {
	...
	v.applyInput(st, 100, 100, true, true, c.DragStartedResponseFlags|down, wheel)
	...
	v.applyInput(st, 140, 100, true, true, c.DraggedResponseFlags|down, wheel)
	v.events = v.events[:0]
	v.applyInput(st, 140, 100, true, true, c.DragStoppedResponseFlags, wheel)
	require.True(t, v.IsPinned(1), "held where it was dropped")
	require.Equal(t, float32(140), v.g.x[0])
	var end *Event
	...
	require.Equal(t, [2]float32{140, 100}, [2]float32{end.X, end.Y}, "the drag-end event carries the drop position")
	v.g.applyPins(-1)
	v.fs.step(&v.g, 500, 500, ForceParams{}.withDefaults(), 0.3)
	require.Equal(t, float32(140), v.g.x[0], "centre gravity does not pull a held node")
}
