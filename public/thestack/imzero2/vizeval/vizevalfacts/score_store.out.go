...
//
// ScoreStore composes the generated facts building blocks per ADR-0100:
// append-only primitives and iterator query verbs; batched
// cached retrieval is the attachable ScoreCache view. Reads see only
...
// ScoreTableName is the ClickHouse table this store binds — database-
// qualified ("<db>.<table>") when a Database was set at generation.
const ScoreTableName = "boxer.facts"
...
const (
	ScoreColKey   = `"id:id:u64:47::0:"`
	ScoreColOrder = `"ts:ts:z64:47::0:"`
)
...
// ScoreMembershipIds is the membership-id assignment this store was
...
var ScoreMembershipIds = map[string]map[string]uint64{
	"VizevalScore": {
...
// ScoreEnvelope carries the pass-through backbone columns — every plain
...
// ScoreEntity is the entity bag (ADR-0100 SD5): the envelope plus one option
...
type ScoreEntity struct {
	ID uint64
	Ts time.Time
	// ScoreEnvelope is embedded: its pass-through columns read as
...
func (inst *ScoreEntity) Archetype() (a []string) {
...
type ScoreStoreConfig struct {
	...
	// ScoreTableName — for every statement it issues (DDL, DESCRIBE, INSERT,
...
// ScoreStore is single-goroutine, like every part it composes. Batched
// cached retrieval is not built in — attach a ScoreCache view.
type ScoreStore struct {
	exec     recordstore.ExecutorI
	alloc    memory.Allocator
	cfg      ScoreStoreConfig
	...
	// views (NewScoreCache registers one pair per view): onWrite populates
	...
	onWrite []func(uint64, *ScoreEntity)
	onFlush []func(uint64)
	// stampers is cfg.Stampers, consulted per Begin (ADR-0112 M1).
	stampers []recordstore.ReferenceStamper
}

// NewScoreStore wires the store. A nil alloc selects the Go allocator.
func NewScoreStore(exec recordstore.ExecutorI, alloc memory.Allocator, cfg ScoreStoreConfig) (inst *ScoreStore) {
	if cfg.Table != "" {
		if terr := recordstore.CheckTableRef(cfg.Table); terr != nil {
			panic("ScoreStore: " + terr.Error())
		}
	}
	if alloc == nil {
		alloc = memory.NewGoAllocator()
	}
	inst = &ScoreStore{exec: exec, alloc: alloc, cfg: cfg, dml: lowlevel.NewInEntityFactsTable(alloc, 64), dirty: make(map[uint64]struct{}), stampers: cfg.Stampers}
	return
}

// tableName is the table reference every statement uses: the configured
// override when set, else the baked ScoreTableName.
func (inst *ScoreStore) tableName() string {
	if inst.cfg.Table != "" {
...
func (inst *ScoreStore) applyStampers() (pushed int) {
...
// the key instead.
func (inst *ScoreStore) notifyWrite(key uint64, ent *ScoreEntity) {
...
func (inst *ScoreStore) notifyFlush(key uint64) {
...
// ScoreMembershipIds against the writer's assignment when pointing this
...
func (inst *ScoreStore) VerifySchema(ctx context.Context) (err error) {
...
// OpenScoreStore is NewScoreStore plus the startup check: it runs VerifySchema and
...
// caller's layer (ADR-0105 D1). NewScoreStore panics on a malformed config
// and so does this. A caller that cannot afford the DESCRIBE at startup
// keeps using NewScoreStore.
func OpenScoreStore(ctx context.Context, exec recordstore.ExecutorI, alloc memory.Allocator, cfg ScoreStoreConfig) (inst *ScoreStore, err error) {
	st := NewScoreStore(exec, alloc, cfg)
	err = st.VerifySchema(ctx)
	if err != nil {
		st.Close()
		err = eh.Errorf("open scoreStore: %w", err)
		return
	}
	inst = st
	return
...
// ScoreEntityBuilder assembles one entity: envelope from Begin, components
// via Add*, direct attribute manipulation via Raw, then Commit.
type ScoreEntityBuilder struct {
	store *ScoreStore
	key   uint64
...
func (inst *ScoreEntityBuilder) endSection(section string) error {
	switch section {
	case "symbol":
...
func (inst *ScoreStore) Begin(id uint64, ts time.Time, env ScoreEnvelope) *ScoreEntityBuilder {
	...
	b := &ScoreEntityBuilder{store: inst, key: id, ent: ScoreEntity{ID: id, Ts: ts, ScoreEnvelope: env}}
...
func (inst *ScoreEntityBuilder) AddVizevalScore(row VizevalScore) *ScoreEntityBuilder {
...
func (inst *ScoreEntityBuilder) Raw() *lowlevel.InEntityFactsTable {
...
func (inst *ScoreEntityBuilder) Commit() (err error) {
...
func (inst *ScoreEntityBuilder) Rollback() (err error) {
...
// them.
func (inst *ScoreStore) IngestVizevalScore(ts time.Time, rows []VizevalScore) (err error) {
	...
	for i := range rows {
		...
		err = inst.Begin(rows[i].Id, ts, ScoreEnvelope{}).AddVizevalScore(rows[i]).Commit()
		if err != nil {
...
func (inst *ScoreStore) Buffered() int { return inst.buffered }
...
func (inst *ScoreStore) Flush(ctx context.Context) (n int, err error) {
...
func (inst *ScoreStore) FlushIfDue(ctx context.Context) (n int, err error) {
...
func (inst *ScoreStore) DiscardPending() {
...
func (inst *ScoreStore) Close() {
	inst.DiscardPending()
	lowlevel.InEntityFactsTableReleaseBuilder(inst.dml)
}

// ScoreCacheConfig parameterizes an attached read-through cache view.
...
// ScoreCache is the batched read-through, write-through KV view over a
// ScoreStore (ADR-0100 SD5): misses queue under work items and flush
...
type ScoreCache[W comparable] struct {
	st    *ScoreStore
	cfg   ScoreCacheConfig
	cache *caching.ReadThroughCache[uint64, *ScoreEntity, W]
}

// NewScoreCache attaches a read-through, write-through cache view to st,
// registering its write-through and flush hooks with the store. Views
// attach for the store's lifetime — there is no detach.
func NewScoreCache[W comparable](st *ScoreStore, cfg ScoreCacheConfig) (inst *ScoreCache[W]) {
	if cfg.Capacity <= 0 {
		cfg.Capacity = 1024
	}
	inst = &ScoreCache[W]{st: st, cfg: cfg}
	inst.rebuild()
	// The hooks close over the view, not the cache instance, keeping
	// them correct if the cache is ever swapped out.
	st.onWrite = append(st.onWrite, func(k uint64, ent *ScoreEntity) {
		if ent == nil {
...
func (inst *ScoreCache[W]) rebuild() {
	opts := []caching.CacheOption[uint64, *ScoreEntity, W]{
		// Admission mirrors the table's newest-row-per-key semantics:
		// the Order timestamp is the entity's monotonic version.
		caching.WithVersioning[uint64, *ScoreEntity, W](func(e *ScoreEntity) int64 { return e.Ts.UnixNano() }),
	}
	if inst.cfg.FreshnessTTL > 0 {
		opts = append(opts, caching.WithFreshnessTTL[uint64, *ScoreEntity, W](inst.cfg.FreshnessTTL))
	}
	if inst.cfg.NegativeTTL > 0 {
		opts = append(opts, caching.WithNegativeCaching[uint64, *ScoreEntity, W](inst.cfg.NegativeTTL))
	}
	inst.cache = caching.NewReadThroughCache[uint64, *ScoreEntity, W](inst.cfg.Capacity, &factsFetcher{st: inst.st}, inst.cfg.FetchCriteria, opts...)
}
...
func (inst *ScoreCache[W]) Get(key uint64) (ent *ScoreEntity, found bool) {
...
func (inst *ScoreCache[W]) GetFetch(ctx context.Context, key uint64) (ent *ScoreEntity, found bool, err error) {
...
func (inst *ScoreCache[W]) WorkItem(w W) iter.Seq[functional.NilIteratorValueType] {
...
func (inst *ScoreCache[W]) IterateReadyWorkItems(ctx context.Context) iter.Seq[W] {
...
func (inst *ScoreCache[W]) IterateRestWorkItems(ctx context.Context) iter.Seq[W] {
...
func (inst *ScoreCache[W]) AdvanceEpoch() {
...
func (inst *ScoreCache[W]) MarkStale(key uint64) {
...
func (inst *ScoreCache[W]) MarkStaleIfOlder(key uint64, order time.Time) {
	inst.cache.MarkAsStaleIfOlder(key, order.UnixNano())
}

// Invalidate drops the key's cached entry (L1 and stash).
func (inst *ScoreCache[W]) Invalidate(key uint64) {
...
func (inst *ScoreStore) fetchLatestSQL(keys []uint64) string {
	...
	sb.WriteString(") ORDER BY " + ScoreColOrder + " DESC LIMIT 1 BY " + ScoreColKey)
...
// public method set.
type factsFetcher struct{ st *ScoreStore }

var _ caching.ItemFetcherI[uint64, *ScoreEntity] = (*factsFetcher)(nil)
...
// compare against.
func (inst *factsFetcher) FetchItemSinglePartition(ctx context.Context, partition uint64, keys []uint64, target caching.ItemTargetI[uint64, *ScoreEntity]) (err error) {
...
// rows.
func (inst *ScoreStore) ScanVizevalScore(ctx context.Context, opts recordstore.ScanOpts) iter.Seq2[*ScoreEntity, error] {
	if opts.KeyPrefix != "" {
		return recordstore.RefuseKeyPrefix[*ScoreEntity]()
	}
	...
	sql := "SELECT * FROM " + inst.tableName() +
		" WHERE " + where +
		" ORDER BY " + ScoreColOrder + " ASC, " + ScoreColKey + " ASC"
	if opts.Limit > 0 {
...
// flushed rows.
func (inst *ScoreStore) Latest(ctx context.Context, key uint64) (ent *ScoreEntity, found bool, err error) {
	sql := "SELECT * FROM " + inst.tableName() +
		" WHERE " + ScoreColKey + " = " + factsKeyLiteral(key) +
		" ORDER BY " + ScoreColOrder + " DESC LIMIT 1" + factsArrowOutputSettings
...
func (inst *ScoreStore) Replay(ctx context.Context, key uint64, fromOrder time.Time, opts recordstore.ReplayOpts) iter.Seq2[*ScoreEntity, error] {
	sql := "SELECT * FROM " + inst.tableName() +
		" WHERE " + ScoreColKey + " = " + factsKeyLiteral(key)
	if !fromOrder.IsZero() {
		sql += " AND " + ScoreColOrder + " >= fromUnixTimestamp64Nano(" + strconv.FormatInt(fromOrder.UnixNano(), 10) + ")"
	}
	if !opts.To.IsZero() {
		sql += " AND " + ScoreColOrder + " < fromUnixTimestamp64Nano(" + strconv.FormatInt(opts.To.UnixNano(), 10) + ")"
	}
	sql += " ORDER BY " + ScoreColOrder + " ASC"
	if opts.Limit > 0 {
		sql += " LIMIT " + strconv.Itoa(opts.Limit)
	}
	sql += factsArrowOutputSettings
...
// ScoreComponentSQL publishes this store's ADR-0066 read-back artefacts —
...
// join must bind them to ScoreTableName itself (ADR-0189 SD6).
var ScoreComponentSQL = componentsql.Set{
	Store: "Score",
	Table: ScoreTableName,
...
func (inst *ScoreStore) queryEntities(ctx context.Context, sql string) (ents []*ScoreEntity, err error) {
	for rec, rerr := range inst.exec.QueryArrow(ctx, sql) {
		if rerr != nil {
			err = eh.Errorf("query entities: %w", rerr)
			return
		}
		batch, derr := decodeScoreRecord(rec)
		rec.Release()
		if derr != nil {
			err = derr
			return
...
// the sequence.
func (inst *ScoreStore) iterateEntities(ctx context.Context, sql string) iter.Seq2[*ScoreEntity, error] {
	return func(yield func(*ScoreEntity, error) bool) {
		ents, err := inst.queryEntities(ctx, sql)
		if err != nil {
			yield(nil, err)
			return
...
// decodeScoreRecord reads one fetched Arrow record into entity bags:
...
func decodeScoreRecord(rec arrow.RecordBatch) (ents []*ScoreEntity, err error) {
