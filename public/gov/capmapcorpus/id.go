...
// competence key is the same shape as every other key in that table.
const naturalKeyBytes = 16

// naturalKeyDomain prefixes every digest so a competence key can never collide
// with a key minted for some other kind of fact from the same string.
//
// It is part of the identity, so changing it re-keys every row. That was free
// when the word changed (ADR-0168 §SD6, 2026-08-05) because no corpus had been
// ingested anywhere yet; it will not be free again.
const naturalKeyDomain = "capmap.competence"
...
// and `adversarial-robustness` are one competence.
//
...
// string cannot be a competence slug, so a link naming one points outside the
...
// at all.
//
// wellFormed false means the text is not a valid competence slug, so no
// competence can ever carry it — the link names something outside the corpus,
...
// repository's own competence catalog roughly a quarter of body links are
...
// NaturalKey derives a competence's durable identity from its normalized slug:
// a domain-separated blake3 digest, matching the natural-key convention the
// runtime facts store already follows.
//
// The slug is the whole of the identity. Moving a competence's file, renaming
// its display name, or re-scoring it does not change its key; only renaming
// the slug does, and that is a new competence by design.
//
...
// mint two keys for one competence.
...
// slugForPath derives a competence's slug from where its file sits: the
...
// other competence is addressable and this one would not be, so the vault has
