...
// A directory-backed competence takes its slug from the directory, never from
// the "capability.md" marker filename.
...
- [[Jouppi-1990]] is a citation, not a competence.
`,
	})
	corpus, err := capmapcorpus.ParseDir(root)
	...
	// A parent naming a directory-backed competence resolves directly: only
...
// Both spellings of a link to a directory-backed competence name the same
// target, and only the bare one is a defect.
//
// This is a regression guard with a measured cost: before it was handled,
// roughly 800 of the 5,961 relations on the real competence tree — every link
// written the Obsidian-correct way — were reported as broken.
func TestParseDirResolvesQualifiedMarkerLink(t *testing.T) {
	root := writeVault(t, map[string]string{
		"analytics/capability.md": "---\nname: Analytics\nlevel: 1\n---\n\n# Vision and Scope\n\nroot\n",
		"leaf.md": "---\nname: Leaf\nlevel: 2\n---\n\n# Activities\n\n" +
	...
	for _, r := range corpus.Relations {
		...
		require.Equal(t, "analytics", r.Target, "both spellings reduce to the competence's slug")
		switch r.Resolution {
...
// A vault may hold notes beside its competences. Those are skipped and
// reported, not fatal — and not silent.
func TestParseDirSkipsFilesThatCannotBeCompetences(t *testing.T) {
	root := writeVault(t, map[string]string{
		"similarity.md": "---\nname: Similarity\nlevel: 2\n---\n\n# Vision and Scope\n\nx\n",
		"AMQP-1.0.md":   "---\nname: \"AMQP 1.0\"\ntype: de-jure\n---\n",
	})
	corpus, err := capmapcorpus.ParseDir(root)
	require.NoError(t, err, "a reference note beside a competence must not fail the read")
	require.Len(t, corpus.Competences, 1)
	assert.Equal(t, "similarity", corpus.Competences[0].Slug)
	require.Len(t, corpus.Skipped, 1)
	assert.Equal(t, "AMQP-1.0.md", corpus.Skipped[0].Path)
	assert.NotEmpty(t, corpus.Skipped[0].Reason)
}

// A README is the one non-competence whose name *is* a well-formed slug, so
// nothing else would catch it. ADR-0168 §SD7 puts one at the root of this
// repository's own vault, and its prose cites competences by wikilink — which
// is how it was found: the first parse reported one competence too many and an
// extra relation the corpus never declared.
func TestParseDirSkipsReadme(t *testing.T) {
	root := writeVault(t, map[string]string{
		"similarity.md":    "---\nname: Similarity\nlevel: 2\n---\n\n# Vision and Scope\n\nx\n",
		"README.md":        "---\ntype: reference\n---\n\n# The vault\n\nlink a competence like [[similarity]]\n",
		"nested/readme.md": "---\nname: Nested\nlevel: 3\n---\n\n# Vision and Scope\n\nlower case too\n",
	})
	corpus, err := capmapcorpus.ParseDir(root)
	require.NoError(t, err)
	require.Len(t, corpus.Competences, 1)
	assert.Equal(t, "similarity", corpus.Competences[0].Slug)
	assert.Empty(t, corpus.Relations, "a README's prose links are not corpus relations")
	require.Len(t, corpus.Skipped, 2)
	assert.Equal(t, "README.md", corpus.Skipped[0].Path)
	assert.Contains(t, corpus.Skipped[0].Reason, "front page")
	assert.Equal(t, filepath.Join("nested", "readme.md"), corpus.Skipped[1].Path)
}
...
func TestParseDirRefusesDuplicateSlug(t *testing.T) {
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "duplicate competence slug")
}
...
// Citation keys are not competence slugs, and the distinction is load-bearing:
...
func TestNormalizeTargetSeparatesCitations(t *testing.T) {
	for _, raw := range []string{"Jouppi-1990", "GDPR-Art-17", "boxer-adr-0009-environment-variable-registry"} {
		...
		assert.False(t, wellFormed, "%q is not a competence slug", raw)
