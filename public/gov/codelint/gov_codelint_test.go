...
func TestCS008_FlagsTypeAliases(t *testing.T) {
	...
	linter := codelint.NewLinter()
	linter.Register(codelint.NewRuleCS008())
...
func TestCS008_PassesGoodFile(t *testing.T) {
...
func TestCS010_FlagsIterMethodNaming(t *testing.T) {
	...
	linter := codelint.NewLinter()
	linter.Register(codelint.NewRuleCS010())
	...
	require.Len(t, findings, 2, "expected 2 unsuppressed CS010 findings (sole-iterator with bad name; sole iterator not fixed by any interface; suppressed counterpart filtered)")
	for _, f := range findings {
...
func TestCS010_PassesGoodFile(t *testing.T) {
