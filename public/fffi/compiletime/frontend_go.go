...
func (inst *CodeTransformerFrontendGo) Emit(out io.Writer, preamble []byte) (n int, err error) {
	var n2 int
...
`))
	...
	for scanner.Scan() {
		...
		if emitImports {
			...
			if inst.needsAdditionalImports {
			...
			if len(preamble) > 0 {
				n2, err = out.Write(preamble)
				n += n2
				if err != nil {
					err = eh.Errorf("unable to write preamble to output: %w", err)
					return
				}
			}
		}
