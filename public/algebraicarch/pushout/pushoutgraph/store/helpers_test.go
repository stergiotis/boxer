...
func assertNoInvariantViolations(tt *testing.T, g *PushoutGraph) {
	tt.Helper()
...
func makeBasePushoutGraph(n int, seed string) (*PushoutGraph, *patch.Patch) {
	g := New()
	...
	for i := range n {
		...
		changes[i] = patch.Change{
			...
			Content:   fmt.Appendf(nil, "%s_line_%d\n", seed, i),
