...
func TestUnapply_NewEdgeRestoresShadowedPseudoEdge(tt *testing.T) {
	...
	if order := algo.LinearOrder(g); order == nil {
		tt.Fatal("after Unapply(edge) the pushoutgraph lost its linear order — pseudo-edge A->C not restored")
	}
	assertNoInvariantViolations(tt, g)
}

// canonicalObservableState projects the pushoutgraph onto its observable
...
func canonicalObservableState(g *PushoutGraph) string {
...
// a fresh pushoutgraph. This is the property the Unrecord workflow depends on.
