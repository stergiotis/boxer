...
// store (white-box) so they can deliberately corrupt unexported PushoutGraph
...
// A clean pushoutgraph must produce zero invariant violations.
func TestCheckInvariants_Clean(tt *testing.T) {
	g := New()
	if errs := qc.CheckInvariants(g); len(errs) != 0 {
		tt.Fatalf("expected no errors on clean pushoutgraph, got: %v", errs)
	}
}
...
// CheckInvariants must not mutate the pushoutgraph it inspects. The idempotence
...
func TestCheckInvariants_DoesNotMutate(tt *testing.T) {
	...
	if g.DirtyRepCount() == 0 {
		tt.Fatal("CheckInvariants resolved dirty reps — checker mutated the pushoutgraph")
	}

	// And on a clean pushoutgraph the full state must be untouched bit for bit.
	g.ResolvePseudoEdges()
	want := g.Debug()
	...
	if errs := qc.CheckInvariants(g); len(errs) != 0 {
		tt.Fatalf("expected clean pushoutgraph, got: %v", errs)
	}
