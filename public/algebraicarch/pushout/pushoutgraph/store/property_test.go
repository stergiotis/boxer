...
func liveNodeSet(g *PushoutGraph) map[t.NodeID]struct{} {
...
func TestProperty_Commutativity(tt *testing.T) {
	for seed := range int64(50) {
		...
		g1, base := makeBasePushoutGraph(lineCount, fmt.Sprintf("comm%d", seed))
		g2, _ := makeBasePushoutGraph(lineCount, fmt.Sprintf("comm%d", seed))
...
func TestProperty_ApplyUnapplyIdentity(tt *testing.T) {
	for seed := range int64(50) {
		...
		g, base := makeBasePushoutGraph(lineCount, fmt.Sprintf("roundtrip%d", seed))
...
func TestProperty_PseudoEdgeIdempotent(tt *testing.T) {
	for seed := range int64(30) {
		...
		g, base := makeBasePushoutGraph(lineCount, fmt.Sprintf("pseudo_idem%d", seed))
...
func TestProperty_CloneEquivalence(tt *testing.T) {
	for seed := range int64(30) {
		...
		g, _ := makeBasePushoutGraph(lineCount, fmt.Sprintf("clone_eq%d", seed))
		clone := g.Clone()
...
func TestProperty_IncrementalVsBatch(tt *testing.T) {
	...
	// the same pushoutgraph as applying them all at once in a single patch.
	for seed := range int64(40) {
		...
		// Build base.
		gBatch, base := makeBasePushoutGraph(lineCount, baseSeed)
		gIncr, _ := makeBasePushoutGraph(lineCount, baseSeed)
...
// randomChange generates a random applicable change against a pushoutgraph.
// Returns the change and true, or zero-value and false if no valid change could be generated.
func randomChange(g *PushoutGraph, rng *rand.Rand, label string, nodeIdx *uint64) (patch.Change, bool) {
...
func TestProperty_InsertOnlyPairCommutativity(tt *testing.T) {
	for seed := range int64(30) {
		...
		g1, base := makeBasePushoutGraph(lineCount, baseSeed)
		g2, _ := makeBasePushoutGraph(lineCount, baseSeed)
...
func TestProperty_ApplyUnapplyReapply(tt *testing.T) {
	...
	for seed := range int64(40) {
		...
		g1, base := makeBasePushoutGraph(lineCount, fmt.Sprintf("reapply_%d", seed))
		g2, _ := makeBasePushoutGraph(lineCount, fmt.Sprintf("reapply_%d", seed))
...
func TestProperty_GhostMonotonicity(tt *testing.T) {
	...
	for seed := range int64(30) {
		...
		g, base := makeBasePushoutGraph(lineCount, fmt.Sprintf("ghost%d", seed))
