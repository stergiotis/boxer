...
func TestRender_EmptyPushoutGraph(tt *testing.T) {
	g := New()
	rendered := string(g.Render())
	if rendered != "" {
		tt.Fatalf("empty pushoutgraph should render empty, got %q", rendered)
	}
}
...
func TestRenderLines_Empty(tt *testing.T) {
	...
	lines := g.RenderLines()
	if lines != nil {
		tt.Fatalf("empty pushoutgraph should return nil lines, got %v", lines)
	}
}

// Render of a deep conflict pushoutgraph exercises the iterative renderDFS at
