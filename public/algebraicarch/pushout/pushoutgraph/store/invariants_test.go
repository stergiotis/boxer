...
func TestInvariants_EmptyPushoutGraph(tt *testing.T) {
	g := New()
...
func TestInvariants_RandomPatchSequence(tt *testing.T) {
	for seed := range int64(50) {
		...
		g, base := makeBasePushoutGraph(3+rng.Intn(5), fmt.Sprintf("rand_inv_%d", seed))
		assertNoInvariantViolations(tt, g)
...
func TestInvariants_RandomDeleteSequence(tt *testing.T) {
	for seed := range int64(30) {
		...
		g, base := makeBasePushoutGraph(lineCount, fmt.Sprintf("rand_del_%d", seed))
		assertNoInvariantViolations(tt, g)
