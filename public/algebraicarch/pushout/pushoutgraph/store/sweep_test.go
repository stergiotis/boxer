...
// DeleteNode via PushoutGraph.SetClock.
...
func TestSweepTombstones_NothingToPurge(tt *testing.T) {
	...
	if purgedCount != 0 || len(purgedIDs) != 0 {
		tt.Fatalf("empty pushoutgraph should produce zero purges, got count=%d ids=%v", purgedCount, purgedIDs)
	}
}
...
func TestClone_PreservesTombstoneBookkeeping(tt *testing.T) {
	...
	if count != 0 {
		tt.Fatalf("cloned pushoutgraph re-sweep should purge nothing, got %d", count)
	}
}
