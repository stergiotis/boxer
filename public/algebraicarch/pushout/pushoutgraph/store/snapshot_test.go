...
// keep loading), a rapid property drives random pushoutgraphs through
...
// goldenPushoutGraph builds a deterministic pushoutgraph exercising every encoded
...
func goldenPushoutGraph(tt testing.TB) *PushoutGraph {
	g := New()
...
func TestSnapshot_Golden(tt *testing.T) {
	g := goldenPushoutGraph(tt)
	data, err := g.EncodeSnapshot()
	if err != nil {
		tt.Fatal(err)
	}
	path := filepath.Join("testdata", "golden_pushoutgraph.grg1")
...
func snapshotState(g *PushoutGraph) string {
	var sb bytes.Buffer
...
func TestSnapshot_RoundTripProperty(tt *testing.T) {
	...
	rapid.Check(tt, func(rt *rapid.T) {
		...
		if errs := qc.CheckInvariants(dec); len(errs) != 0 {
			rt.Fatalf("decoded pushoutgraph violates invariants: %v", errs)
		}
		if got, want := snapshotState(dec), snapshotState(g); got != want {
			rt.Fatalf("state diverged across snapshot round-trip:\n got:\n%s\nwant:\n%s", got, want)
		}
		// Idempotence: re-encoding the decoded pushoutgraph is byte-identical.
