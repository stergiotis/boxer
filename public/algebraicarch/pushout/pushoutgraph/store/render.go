...
// renderLinear renders a linearly ordered pushoutgraph to plain text.
func (inst *PushoutGraph) renderLinear(order []t.NodeID) []byte {
...
func (inst *PushoutGraph) renderWithConflicts() []byte {
	var buf bytes.Buffer
...
func liveSelfEdge(g *PushoutGraph, v t.NodeID) bool {
...
func (inst *PushoutGraph) RenderLines() []string {
	content := inst.Render()
	if len(content) == 0 {
		return nil
	}
