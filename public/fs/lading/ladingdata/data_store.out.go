...
//
// DataStore composes the generated fsdata building blocks per ADR-0100:
// append-only primitives and iterator query verbs; batched
// cached retrieval is the attachable DataCache view. Reads see only
...
//
//go:embed fsdata_ddl_clickhouse.out.sql
var fsdataDDLCreate string

// DataTableName is the ClickHouse table this store binds — database-
// qualified ("<db>.<table>") when a Database was set at generation.
const DataTableName = "boxer.fsdata"
...
const (
	DataColKey   = `"id:id:u64:47::0:"`
	DataColOrder = `"ts:ts:z64:47::0:"`
)

// Arrow output shape the read-access classes expect.
const fsdataArrowOutputSettings = " SETTINGS output_format_arrow_string_as_string=1, output_format_arrow_low_cardinality_as_dictionary=0"

// fsdataKeyLiteral renders a Key value as a ClickHouse SQL literal.
func fsdataKeyLiteral(k uint64) string { return strconv.FormatUint(k, 10) }

// DataMembershipIds is the membership-id assignment this store was
...
var DataMembershipIds = map[string]map[string]uint64{
	"LadingBlock": {
		"ladingKindBlock": 10376293554346524674,
		"ladingData":      10376293554346524692,
		"ladingBlockHash": 10376293554346524693,
		"ladingLine0":     10376293554346524694,
	},
}

// DataEnvelope carries the pass-through backbone columns — every plain
...
// DataEntity is the entity bag (ADR-0100 SD5): the envelope plus one option
...
type DataEntity struct {
	ID uint64
	Ts time.Time
	// DataEnvelope is embedded: its pass-through columns read as
	// promoted entity fields.
	DataEnvelope
	LadingBlock option.Option[LadingBlock]
}

// Archetype reports which components the entity carries, in schema order.
func (inst *DataEntity) Archetype() (a []string) {
	if inst.LadingBlock.Has {
		a = append(a, "ladingBlock")
	}
	return
}
...
// cached retrieval is not built in — attach a DataCache view.
type DataStore struct {
	exec     recordstore.ExecutorI
	alloc    memory.Allocator
	cfg      DataStoreConfig
	dml      *lowlevel.InEntityFsdataTable
	buffered int
	...
	// views (NewDataCache registers one pair per view): onWrite populates
	...
	onWrite []func(uint64, *DataEntity)
	onFlush []func(uint64)
...
func NewDataStore(exec recordstore.ExecutorI, alloc memory.Allocator, cfg DataStoreConfig) (inst *DataStore) {
	if cfg.Table != "" {
	...
	inst = &DataStore{exec: exec, alloc: alloc, cfg: cfg, dml: lowlevel.NewInEntityFsdataTable(alloc, 64), dirty: make(map[uint64]struct{}), stampers: cfg.Stampers}
	return
}
...
// the key instead.
func (inst *DataStore) notifyWrite(key uint64, ent *DataEntity) {
...
// DataMembershipIds against the writer's assignment when pointing this
...
// OpenDataStore is NewDataStore plus the startup sequence every caller otherwise
...
// caller's layer (ADR-0105 D1). NewDataStore panics on a malformed config
// and so does this. A caller that cannot afford the DESCRIBE at startup
// keeps using NewDataStore.
func OpenDataStore(ctx context.Context, exec recordstore.ExecutorI, alloc memory.Allocator, cfg DataStoreConfig) (inst *DataStore, err error) {
	st := NewDataStore(exec, alloc, cfg)
	err = st.EnsureTable(ctx)
	if err != nil {
		st.Close()
		err = eh.Errorf("open dataStore: %w", err)
		return
	}
...
// DataEntityBuilder assembles one entity: envelope from Begin, components
...
func (inst *DataEntityBuilder) endSection(section string) error {
	switch section {
	case "symbol":
...
// AddLadingBlock contributes the LadingBlock component to the open entity.
//
...
func (inst *DataEntityBuilder) AddLadingBlock(row LadingBlock) *DataEntityBuilder {
	if err := inst.buf.StartKind("LadingBlock"); err != nil {
		inst.store.dml.AppendError(err)
		return inst
	}
	inst.buf.Enqueue("symbol", "LadingBlock", func() error {
		return ladingBlockEmitSectionSymbol(inst.store.dml.GetSectionSymbol(), row)
	})
	inst.buf.Enqueue("blobArray", "LadingBlock", func() error {
		return ladingBlockEmitSectionBlobArray(inst.store.dml.GetSectionBlobArray(), row)
	})
	inst.buf.Enqueue("u32Array", "LadingBlock", func() error {
		return ladingBlockEmitSectionU32Array(inst.store.dml.GetSectionU32Array(), row)
	})
	inst.ent.LadingBlock = option.Some(row)
	return inst
}
...
func (inst *DataEntityBuilder) Raw() *lowlevel.InEntityFsdataTable {
...
func (inst *DataEntityBuilder) Rollback() (err error) {
	...
	return lowlevel.InEntityFsdataTableRollbackEntity(inst.store.dml)
}
...
func (inst *DataStore) IngestLadingBlock(ts time.Time, rows []LadingBlock) (err error) {
	...
	for i := range rows {
		if _, dup := seen[rows[i].Id]; dup {
			err = eh.Errorf("ingest ladingBlock row %d: %w: key %v", i, recordstore.ErrDuplicateIngestKey, rows[i].Id)
			return
		}
...
func (inst *DataStore) Buffered() int { return inst.buffered }
...
func (inst *DataStore) Flush(ctx context.Context) (n int, err error) {
	...
	records, err := lowlevel.InEntityFsdataTableTransferRecords(inst.dml, nil)
	if err != nil {
...
func (inst *DataStore) FlushIfDue(ctx context.Context) (n int, err error) {
...
func (inst *DataStore) DiscardPending() {
	_ = lowlevel.InEntityFsdataTableRollbackEntity(inst.dml) // no-op error when no frame is open
	inst.dml.ClearMembershipsHighCardRef()                   // an abandoned Begin's stamps must not outlive its frame
	if records, err := lowlevel.InEntityFsdataTableTransferRecords(inst.dml, nil); err == nil {
		for _, rec := range records {
			rec.Release()
		}
	}
...
func (inst *DataStore) Close() {
	inst.DiscardPending()
	lowlevel.InEntityFsdataTableReleaseBuilder(inst.dml)
}

// DataCacheConfig parameterizes an attached read-through cache view.
...
// DataCache is the batched read-through, write-through KV view over a
// DataStore (ADR-0100 SD5): misses queue under work items and flush
...
type DataCache[W comparable] struct {
	st    *DataStore
	cfg   DataCacheConfig
	cache *caching.ReadThroughCache[uint64, *DataEntity, W]
}

// NewDataCache attaches a read-through, write-through cache view to st,
// registering its write-through and flush hooks with the store. Views
// attach for the store's lifetime — there is no detach.
func NewDataCache[W comparable](st *DataStore, cfg DataCacheConfig) (inst *DataCache[W]) {
	if cfg.Capacity <= 0 {
		cfg.Capacity = 1024
	}
	inst = &DataCache[W]{st: st, cfg: cfg}
	inst.rebuild()
	// The hooks close over the view, not the cache instance, keeping
	// them correct if the cache is ever swapped out.
	st.onWrite = append(st.onWrite, func(k uint64, ent *DataEntity) {
		if ent == nil {
...
func (inst *DataCache[W]) rebuild() {
	opts := []caching.CacheOption[uint64, *DataEntity, W]{
		// Admission mirrors the table's newest-row-per-key semantics:
		// the Order timestamp is the entity's monotonic version.
		caching.WithVersioning[uint64, *DataEntity, W](func(e *DataEntity) int64 { return e.Ts.UnixNano() }),
	}
	if inst.cfg.FreshnessTTL > 0 {
		opts = append(opts, caching.WithFreshnessTTL[uint64, *DataEntity, W](inst.cfg.FreshnessTTL))
	}
	if inst.cfg.NegativeTTL > 0 {
		opts = append(opts, caching.WithNegativeCaching[uint64, *DataEntity, W](inst.cfg.NegativeTTL))
	}
	inst.cache = caching.NewReadThroughCache[uint64, *DataEntity, W](inst.cfg.Capacity, &fsdataFetcher{st: inst.st}, inst.cfg.FetchCriteria, opts...)
}
...
func (inst *DataCache[W]) Get(key uint64) (ent *DataEntity, found bool) {
...
func (inst *DataCache[W]) GetFetch(ctx context.Context, key uint64) (ent *DataEntity, found bool, err error) {
...
func (inst *DataCache[W]) WorkItem(w W) iter.Seq[functional.NilIteratorValueType] {
...
func (inst *DataCache[W]) IterateReadyWorkItems(ctx context.Context) iter.Seq[W] {
...
func (inst *DataCache[W]) IterateRestWorkItems(ctx context.Context) iter.Seq[W] {
...
func (inst *DataCache[W]) AdvanceEpoch() {
...
func (inst *DataCache[W]) MarkStale(key uint64) {
...
func (inst *DataCache[W]) MarkStaleIfOlder(key uint64, order time.Time) {
	inst.cache.MarkAsStaleIfOlder(key, order.UnixNano())
}

// Invalidate drops the key's cached entry (L1 and stash).
func (inst *DataCache[W]) Invalidate(key uint64) {
...
func (inst *DataCache[W]) InvalidateAll() {
	inst.cache.Clear()
}
...
func (inst *DataStore) fetchLatestSQL(keys []uint64) string {
	...
	for i, k := range keys {
		...
		sb.WriteString(fsdataKeyLiteral(k))
	}
	sb.WriteString(") ORDER BY " + DataColOrder + " DESC LIMIT 1 BY " + DataColKey)
	sb.WriteString(fsdataArrowOutputSettings)
	return sb.String()
}

// fsdataFetcher implements caching.ItemFetcherI for attached cache views —
// an unexported shim so the fetch plumbing stays off the store's
// public method set.
type fsdataFetcher struct{ st *DataStore }

var _ caching.ItemFetcherI[uint64, *DataEntity] = (*fsdataFetcher)(nil)

// DeterminePartition implements caching.ItemFetcherI. Single partition
// in v1 (one table, one server).
func (inst *fsdataFetcher) DeterminePartition(key uint64) uint64 { return 0 }
...
// compare against.
func (inst *fsdataFetcher) FetchItemSinglePartition(ctx context.Context, partition uint64, keys []uint64, target caching.ItemTargetI[uint64, *DataEntity]) (err error) {
...
const (
	fsdataScanLadingBlockFilter = "has(\"tv:symbol:lr:lr:u64:1247:::0::data\", 10376293554346524674) AND hasAll(\"tv:blobArray:lr:lr:u64:1247:::0::data\", [10376293554346524692, 10376293554346524693]) AND has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 10376293554346524694) AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 10376293554346524674) = 1 AND countEqual(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 10376293554346524692) = 1 AND if(has(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 10376293554346524692), \"tv:blobArray:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 10376293554346524692), arrayCumSum(\"tv:blobArray:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 10376293554346524693) = 1 AND if(has(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 10376293554346524693), \"tv:blobArray:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 10376293554346524693), arrayCumSum(\"tv:blobArray:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 10376293554346524694) = 1 AND if(has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 10376293554346524694), \"tv:u32Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 10376293554346524694), arrayCumSum(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1"
)

// ScanLadingBlock iterates the entities whose rows carry a conforming LadingBlock
...
// rows.
func (inst *DataStore) ScanLadingBlock(ctx context.Context, opts recordstore.ScanOpts) iter.Seq2[*DataEntity, error] {
	if opts.KeyPrefix != "" {
		return recordstore.RefuseKeyPrefix[*DataEntity]()
	}
	where := fsdataScanLadingBlockFilter
...
func (inst *DataStore) Replay(ctx context.Context, key uint64, fromOrder time.Time, opts recordstore.ReplayOpts) iter.Seq2[*DataEntity, error] {
	...
	if !opts.To.IsZero() {
		sql += " AND " + DataColOrder + " < fromUnixTimestamp64Nano(" + strconv.FormatInt(opts.To.UnixNano(), 10) + ")"
	}
	sql += " ORDER BY " + DataColOrder + " ASC"
	if opts.Limit > 0 {
		sql += " LIMIT " + strconv.Itoa(opts.Limit)
	}
	sql += fsdataArrowOutputSettings
...
// DataComponentSQL publishes this store's ADR-0066 read-back artefacts —
...
// join must bind them to DataTableName itself (ADR-0189 SD6).
var DataComponentSQL = componentsql.Set{
	Store: "Data",
	Table: DataTableName,
	Kinds: map[string]componentsql.Artefacts{
		"LadingBlock": {
			Presence:   "has(\"tv:symbol:lr:lr:u64:1247:::0::data\", 10376293554346524674) AND hasAll(\"tv:blobArray:lr:lr:u64:1247:::0::data\", [10376293554346524692, 10376293554346524693]) AND has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 10376293554346524694)",
			Validator:  "countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 10376293554346524674) = 1 AND countEqual(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 10376293554346524692) = 1 AND if(has(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 10376293554346524692), \"tv:blobArray:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 10376293554346524692), arrayCumSum(\"tv:blobArray:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 10376293554346524693) = 1 AND if(has(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 10376293554346524693), \"tv:blobArray:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:blobArray:lr:lr:u64:1247:::0::data\", 10376293554346524693), arrayCumSum(\"tv:blobArray:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 10376293554346524694) = 1 AND if(has(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 10376293554346524694), \"tv:u32Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u32Array:lr:lr:u64:1247:::0::data\", 10376293554346524694), arrayCumSum(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1",
			Filter:     fsdataScanLadingBlockFilter,
			Projection: "CAST(tuple(\"id:id:u64:47::0:\", \"id:naturalKey:y:4::0:\", \"ts:ts:z64:47::0:\", LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 10376293554346524674, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_LIST_BY_TAG_EQUAL(\"tv:blobArray:value:val:yh:4:::0::data\", \"tv:blobArray:len:len:u64:4D:::0::data\", \"tv:blobArray:lr:lr:u64:1247:::0::data\", 10376293554346524692, LW_RAGGED_PARENT_IDS(\"tv:blobArray:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_LIST_BY_TAG_EQUAL(\"tv:blobArray:value:val:yh:4:::0::data\", \"tv:blobArray:len:len:u64:4D:::0::data\", \"tv:blobArray:lr:lr:u64:1247:::0::data\", 10376293554346524693, LW_RAGGED_PARENT_IDS(\"tv:blobArray:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_LIST_BY_TAG_EQUAL(\"tv:u32Array:value:val:u32h:4:::0::data\", \"tv:u32Array:len:len:u64:4D:::0::data\", \"tv:u32Array:lr:lr:u64:1247:::0::data\", 10376293554346524694, LW_RAGGED_PARENT_IDS(\"tv:u32Array:lrcard:lrcard:u64:4E:::0::data\"))[1]), 'Tuple(Id UInt64, NaturalKey String, Ts DateTime64(9,\\'UTC\\'), Kind String, Data String, Hash String, Line0 UInt32)')",
		},
	},
}
...
// fsdataSectionReaderI is the uniform slice of the generated read-access
// readers. Column indices stay at their constructor defaults — the
// schema order a SELECT * returns.
type fsdataSectionReaderI interface {
	LoadFromRecord(raruntime.RecordI) error
	Release()
}

func (inst *DataStore) queryEntities(ctx context.Context, sql string) (ents []*DataEntity, err error) {
	for rec, rerr := range inst.exec.QueryArrow(ctx, sql) {
		if rerr != nil {
			err = eh.Errorf("query entities: %w", rerr)
			return
		}
		batch, derr := decodeDataRecord(rec)
		rec.Release()
		if derr != nil {
			err = derr
			return
...
// the sequence.
func (inst *DataStore) iterateEntities(ctx context.Context, sql string) iter.Seq2[*DataEntity, error] {
	return func(yield func(*DataEntity, error) bool) {
		ents, err := inst.queryEntities(ctx, sql)
		if err != nil {
			yield(nil, err)
			return
...
// decodeDataRecord reads one fetched Arrow record into entity bags:
...
func decodeDataRecord(rec arrow.RecordBatch) (ents []*DataEntity, err error) {
	...
	readers := []fsdataSectionReaderI{idR, tsR, lcR, symbolR, blobArrayR, u32ArrayR}
	for _, r := range readers {
		err = r.LoadFromRecord(rec)
		if err != nil {
			err = eh.Errorf("load fsdata reader from record: %w", err)
			return
		}
	}
	defer func() {
	...
	for i := range n {
		...
		{
			row, ok, e := ladingBlockReadRow(i, symbolR.GetAttributes(), symbolR.GetMemberships(), blobArrayR.GetAttributes(), blobArrayR.GetMemberships(), u32ArrayR.GetAttributes(), u32ArrayR.GetMemberships())
			if e != nil {
				err = eh.Errorf("read ladingBlock component: %w", e)
				return
			}
			if ok {
				row.Id = ent.ID
				row.Ts = ent.Ts
				ent.LadingBlock = option.Some(row)
			}
		}
