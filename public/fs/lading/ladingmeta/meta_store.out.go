...
// OpenMetaStore is NewMetaStore plus the startup sequence every caller otherwise
...
// caller's layer (ADR-0105 D1). NewMetaStore panics on a malformed config
// and so does this. A caller that cannot afford the DESCRIBE at startup
// keeps using NewMetaStore.
func OpenMetaStore(ctx context.Context, exec recordstore.ExecutorI, alloc memory.Allocator, cfg MetaStoreConfig) (inst *MetaStore, err error) {
	st := NewMetaStore(exec, alloc, cfg)
	err = st.EnsureTable(ctx)
	if err != nil {
		st.Close()
		err = eh.Errorf("open metaStore: %w", err)
		return
	}
...
func (inst *MetaStore) FlushIfDue(ctx context.Context) (n int, err error) {
