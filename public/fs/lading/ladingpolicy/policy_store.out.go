...
//
// PolicyStore composes the generated facts building blocks per ADR-0100:
// append-only primitives and iterator query verbs; batched
// cached retrieval is the attachable PolicyCache view. Reads see only
...
import (
	...
	"github.com/stergiotis/boxer/public/fs/lading/ladingpolicy/internal/lowlevel"
...
// PolicyTableName is the ClickHouse table this store binds — database-
// qualified ("<db>.<table>") when a Database was set at generation.
const PolicyTableName = "boxer.facts"
...
const (
	PolicyColKey   = `"id:id:u64:47::0:"`
	PolicyColOrder = `"ts:ts:z64:47::0:"`
)
...
// PolicyMembershipIds is the membership-id assignment this store was
...
var PolicyMembershipIds = map[string]map[string]uint64{
	"LadingMount": {
		"ladingKindMount":      10376293554346524675,
		"ladingMountName":      10376293554346524695,
		"ladingMountStore":     10376293554346524696,
		"ladingMountTtlClass":  10376293554346524697,
		"ladingMountTextRule":  10376293554346524698,
		"ladingMountInlineMax": 10376293554346524699,
	},
}

// PolicyEnvelope carries the pass-through backbone columns — every plain
...
// PolicyEntity is the entity bag (ADR-0100 SD5): the envelope plus one option
...
type PolicyEntity struct {
	ID uint64
	Ts time.Time
	// PolicyEnvelope is embedded: its pass-through columns read as
	// promoted entity fields.
	PolicyEnvelope
	LadingMount option.Option[LadingMount]
}

// Archetype reports which components the entity carries, in schema order.
func (inst *PolicyEntity) Archetype() (a []string) {
	if inst.LadingMount.Has {
		a = append(a, "ladingMount")
	}
	return
}
...
// PolicyStore is single-goroutine, like every part it composes. Batched
// cached retrieval is not built in — attach a PolicyCache view.
type PolicyStore struct {
	...
	// views (NewPolicyCache registers one pair per view): onWrite populates
	...
	onWrite []func(uint64, *PolicyEntity)
	onFlush []func(uint64)
	// stampers is cfg.Stampers, consulted per Begin (ADR-0112 M1).
	stampers []recordstore.ReferenceStamper
}

// NewPolicyStore wires the store. A nil alloc selects the Go allocator.
func NewPolicyStore(exec recordstore.ExecutorI, alloc memory.Allocator, cfg PolicyStoreConfig) (inst *PolicyStore) {
	if cfg.Table != "" {
	...
	inst = &PolicyStore{exec: exec, alloc: alloc, cfg: cfg, dml: lowlevel.NewInEntityFactsTable(alloc, 64), dirty: make(map[uint64]struct{}), stampers: cfg.Stampers}
	return
}
...
func (inst *PolicyStore) applyStampers() (pushed int) {
...
// the key instead.
func (inst *PolicyStore) notifyWrite(key uint64, ent *PolicyEntity) {
...
func (inst *PolicyStore) notifyFlush(key uint64) {
...
// PolicyMembershipIds against the writer's assignment when pointing this
...
// OpenPolicyStore is NewPolicyStore plus the startup check: it runs VerifySchema and
...
// caller's layer (ADR-0105 D1). NewPolicyStore panics on a malformed config
// and so does this. A caller that cannot afford the DESCRIBE at startup
// keeps using NewPolicyStore.
func OpenPolicyStore(ctx context.Context, exec recordstore.ExecutorI, alloc memory.Allocator, cfg PolicyStoreConfig) (inst *PolicyStore, err error) {
	st := NewPolicyStore(exec, alloc, cfg)
	err = st.VerifySchema(ctx)
	if err != nil {
		st.Close()
		err = eh.Errorf("open policyStore: %w", err)
		return
	}
	inst = st
	return
...
// PolicyEntityBuilder assembles one entity: envelope from Begin, components
...
func (inst *PolicyEntityBuilder) endSection(section string) error {
	switch section {
	case "symbol":
...
func (inst *PolicyStore) Begin(id uint64, ts time.Time, env PolicyEnvelope) *PolicyEntityBuilder {
	...
	b := &PolicyEntityBuilder{store: inst, key: id, ent: PolicyEntity{ID: id, Ts: ts, PolicyEnvelope: env}}
	b.pushed = inst.applyStampers()
	return b
}

// AddLadingMount contributes the LadingMount component to the open entity.
//
...
func (inst *PolicyEntityBuilder) AddLadingMount(row LadingMount) *PolicyEntityBuilder {
	if err := inst.buf.StartKind("LadingMount"); err != nil {
		inst.store.dml.AppendError(err)
		return inst
	}
	inst.buf.Enqueue("symbol", "LadingMount", func() error {
		return ladingMountEmitSectionSymbol(inst.store.dml.GetSectionSymbol(), row)
	})
	inst.buf.Enqueue("stringArray", "LadingMount", func() error {
		return ladingMountEmitSectionStringArray(inst.store.dml.GetSectionStringArray(), row)
	})
	inst.buf.Enqueue("u64Array", "LadingMount", func() error {
		return ladingMountEmitSectionU64Array(inst.store.dml.GetSectionU64Array(), row)
	})
	inst.ent.LadingMount = option.Some(row)
	return inst
}
...
func (inst *PolicyEntityBuilder) Raw() *lowlevel.InEntityFactsTable {
...
func (inst *PolicyEntityBuilder) Commit() (err error) {
...
func (inst *PolicyEntityBuilder) Rollback() (err error) {
...
// them.
func (inst *PolicyStore) IngestLadingMount(ts time.Time, rows []LadingMount) (err error) {
	seen := make(map[uint64]struct{}, len(rows))
	for i := range rows {
		if _, dup := seen[rows[i].Id]; dup {
			err = eh.Errorf("ingest ladingMount row %d: %w: key %v", i, recordstore.ErrDuplicateIngestKey, rows[i].Id)
			return
		}
...
func (inst *PolicyStore) Buffered() int { return inst.buffered }
...
func (inst *PolicyStore) Flush(ctx context.Context) (n int, err error) {
...
func (inst *PolicyStore) FlushIfDue(ctx context.Context) (n int, err error) {
...
func (inst *PolicyStore) DiscardPending() {
...
func (inst *PolicyStore) Close() {
	inst.DiscardPending()
	lowlevel.InEntityFactsTableReleaseBuilder(inst.dml)
}

// PolicyCacheConfig parameterizes an attached read-through cache view.
...
// PolicyCache is the batched read-through, write-through KV view over a
// PolicyStore (ADR-0100 SD5): misses queue under work items and flush
...
type PolicyCache[W comparable] struct {
	st    *PolicyStore
	cfg   PolicyCacheConfig
	cache *caching.ReadThroughCache[uint64, *PolicyEntity, W]
}

// NewPolicyCache attaches a read-through, write-through cache view to st,
// registering its write-through and flush hooks with the store. Views
// attach for the store's lifetime — there is no detach.
func NewPolicyCache[W comparable](st *PolicyStore, cfg PolicyCacheConfig) (inst *PolicyCache[W]) {
	if cfg.Capacity <= 0 {
		cfg.Capacity = 1024
	}
	inst = &PolicyCache[W]{st: st, cfg: cfg}
	inst.rebuild()
	// The hooks close over the view, not the cache instance, keeping
	// them correct if the cache is ever swapped out.
	st.onWrite = append(st.onWrite, func(k uint64, ent *PolicyEntity) {
		if ent == nil {
...
func (inst *PolicyCache[W]) rebuild() {
	opts := []caching.CacheOption[uint64, *PolicyEntity, W]{
		// Admission mirrors the table's newest-row-per-key semantics:
		// the Order timestamp is the entity's monotonic version.
		caching.WithVersioning[uint64, *PolicyEntity, W](func(e *PolicyEntity) int64 { return e.Ts.UnixNano() }),
	}
	if inst.cfg.FreshnessTTL > 0 {
		opts = append(opts, caching.WithFreshnessTTL[uint64, *PolicyEntity, W](inst.cfg.FreshnessTTL))
	}
	if inst.cfg.NegativeTTL > 0 {
		opts = append(opts, caching.WithNegativeCaching[uint64, *PolicyEntity, W](inst.cfg.NegativeTTL))
	}
	inst.cache = caching.NewReadThroughCache[uint64, *PolicyEntity, W](inst.cfg.Capacity, &factsFetcher{st: inst.st}, inst.cfg.FetchCriteria, opts...)
}
...
func (inst *PolicyCache[W]) Get(key uint64) (ent *PolicyEntity, found bool) {
...
func (inst *PolicyCache[W]) GetFetch(ctx context.Context, key uint64) (ent *PolicyEntity, found bool, err error) {
...
func (inst *PolicyCache[W]) WorkItem(w W) iter.Seq[functional.NilIteratorValueType] {
...
func (inst *PolicyCache[W]) IterateReadyWorkItems(ctx context.Context) iter.Seq[W] {
...
func (inst *PolicyCache[W]) IterateRestWorkItems(ctx context.Context) iter.Seq[W] {
...
func (inst *PolicyCache[W]) AdvanceEpoch() {
...
func (inst *PolicyCache[W]) MarkStale(key uint64) {
...
func (inst *PolicyCache[W]) MarkStaleIfOlder(key uint64, order time.Time) {
	inst.cache.MarkAsStaleIfOlder(key, order.UnixNano())
}

// Invalidate drops the key's cached entry (L1 and stash).
func (inst *PolicyCache[W]) Invalidate(key uint64) {
...
func (inst *PolicyCache[W]) InvalidateAll() {
	inst.cache.Clear()
}
...
func (inst *PolicyStore) fetchLatestSQL(keys []uint64) string {
	...
	sb.WriteString(") ORDER BY " + PolicyColOrder + " DESC LIMIT 1 BY " + PolicyColKey)
...
// public method set.
type factsFetcher struct{ st *PolicyStore }

var _ caching.ItemFetcherI[uint64, *PolicyEntity] = (*factsFetcher)(nil)
...
// compare against.
func (inst *factsFetcher) FetchItemSinglePartition(ctx context.Context, partition uint64, keys []uint64, target caching.ItemTargetI[uint64, *PolicyEntity]) (err error) {
...
const (
	factsScanLadingMountFilter = "hasAll(\"tv:symbol:lr:lr:u64:1247:::0::data\", [10376293554346524675, 10376293554346524696, 10376293554346524697, 10376293554346524698]) AND has(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 10376293554346524695) AND has(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 10376293554346524699) AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 10376293554346524675) = 1 AND countEqual(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 10376293554346524695) = 1 AND if(has(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 10376293554346524695), \"tv:stringArray:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 10376293554346524695), arrayCumSum(\"tv:stringArray:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 10376293554346524696) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 10376293554346524697) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 10376293554346524698) = 1 AND countEqual(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 10376293554346524699) = 1 AND if(has(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 10376293554346524699), \"tv:u64Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 10376293554346524699), arrayCumSum(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1"
)

// ScanLadingMount iterates the entities whose rows carry a conforming LadingMount
...
func (inst *PolicyStore) Replay(ctx context.Context, key uint64, fromOrder time.Time, opts recordstore.ReplayOpts) iter.Seq2[*PolicyEntity, error] {
	...
	if !opts.To.IsZero() {
		sql += " AND " + PolicyColOrder + " < fromUnixTimestamp64Nano(" + strconv.FormatInt(opts.To.UnixNano(), 10) + ")"
	}
	sql += " ORDER BY " + PolicyColOrder + " ASC"
	if opts.Limit > 0 {
		sql += " LIMIT " + strconv.Itoa(opts.Limit)
	}
	sql += factsArrowOutputSettings
...
// PolicyComponentSQL publishes this store's ADR-0066 read-back artefacts —
...
// join must bind them to PolicyTableName itself (ADR-0189 SD6).
var PolicyComponentSQL = componentsql.Set{
	Store: "Policy",
	Table: PolicyTableName,
	Kinds: map[string]componentsql.Artefacts{
		"LadingMount": {
			Presence:   "hasAll(\"tv:symbol:lr:lr:u64:1247:::0::data\", [10376293554346524675, 10376293554346524696, 10376293554346524697, 10376293554346524698]) AND has(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 10376293554346524695) AND has(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 10376293554346524699)",
			Validator:  "countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 10376293554346524675) = 1 AND countEqual(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 10376293554346524695) = 1 AND if(has(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 10376293554346524695), \"tv:stringArray:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:stringArray:lr:lr:u64:1247:::0::data\", 10376293554346524695), arrayCumSum(\"tv:stringArray:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 10376293554346524696) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 10376293554346524697) = 1 AND countEqual(\"tv:symbol:lr:lr:u64:1247:::0::data\", 10376293554346524698) = 1 AND countEqual(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 10376293554346524699) = 1 AND if(has(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 10376293554346524699), \"tv:u64Array:len:len:u64:4D:::0::data\"[arrayFirstIndex(cum -> cum >= indexOf(\"tv:u64Array:lr:lr:u64:1247:::0::data\", 10376293554346524699), arrayCumSum(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))], 0) = 1",
			Filter:     factsScanLadingMountFilter,
			Projection: "CAST(tuple(\"id:id:u64:47::0:\", \"id:naturalKey:y:4::0:\", \"ts:ts:z64:47::0:\", LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 10376293554346524675, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_LIST_BY_TAG_EQUAL(\"tv:stringArray:value:val:sh:4::8:0::data\", \"tv:stringArray:len:len:u64:4D:::0::data\", \"tv:stringArray:lr:lr:u64:1247:::0::data\", 10376293554346524695, LW_RAGGED_PARENT_IDS(\"tv:stringArray:lrcard:lrcard:u64:4E:::0::data\"))[1], LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 10376293554346524696, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 10376293554346524697, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_VALUE_BY_TAG_EQUAL(\"tv:symbol:value:val:s:124::I:0::data\", \"tv:symbol:lr:lr:u64:1247:::0::data\", 10376293554346524698, LW_RAGGED_PARENT_IDS(\"tv:symbol:lrcard:lrcard:u64:4E:::0::data\")), LW_LIST_BY_TAG_EQUAL(\"tv:u64Array:value:val:u64h:4:::0::data\", \"tv:u64Array:len:len:u64:4D:::0::data\", \"tv:u64Array:lr:lr:u64:1247:::0::data\", 10376293554346524699, LW_RAGGED_PARENT_IDS(\"tv:u64Array:lrcard:lrcard:u64:4E:::0::data\"))[1]), 'Tuple(Id UInt64, NaturalKey String, Ts DateTime64(9,\\'UTC\\'), Kind String, Name String, Store String, TtlClass String, TextRule String, InlineMax UInt64)')",
		},
	},
}
...
func (inst *PolicyStore) queryEntities(ctx context.Context, sql string) (ents []*PolicyEntity, err error) {
	for rec, rerr := range inst.exec.QueryArrow(ctx, sql) {
		if rerr != nil {
			err = eh.Errorf("query entities: %w", rerr)
			return
		}
		batch, derr := decodePolicyRecord(rec)
		rec.Release()
		if derr != nil {
			err = derr
			return
...
// the sequence.
func (inst *PolicyStore) iterateEntities(ctx context.Context, sql string) iter.Seq2[*PolicyEntity, error] {
	return func(yield func(*PolicyEntity, error) bool) {
		ents, err := inst.queryEntities(ctx, sql)
		if err != nil {
			yield(nil, err)
			return
...
// decodePolicyRecord reads one fetched Arrow record into entity bags:
...
func decodePolicyRecord(rec arrow.RecordBatch) (ents []*PolicyEntity, err error) {
	...
	readers := []factsSectionReaderI{idR, tsR, lcR, symbolR, stringArrayR, u64ArrayR}
	...
	for i := range n {
		...
		{
			row, ok, e := ladingMountReadRow(i, symbolR.GetAttributes(), symbolR.GetMemberships(), stringArrayR.GetAttributes(), stringArrayR.GetMemberships(), u64ArrayR.GetAttributes(), u64ArrayR.GetMemberships())
			if e != nil {
				err = eh.Errorf("read ladingMount component: %w", e)
				return
			}
			if ok {
				row.Id = ent.ID
				row.Ts = ent.Ts
				ent.LadingMount = option.Some(row)
			}
		}
