...
func renderPropsFile(pr PackageReport, kind packageprops.Kind) (src []byte, err error) {
	...
	// The command path is spelled in full because it is the only thing in this
	// comment a reader can act on, and `wasmsurvey` is four levels down: the
	// short form these files carried until 2026-08-14 answered "No help topic
	// for 'wasmsurvey'" and read as evidence the tool had been deleted.
	b.WriteString("// Seeded by `boxer code analysis golang wasmsurvey props generate`; curate by\n")
	b.WriteString("// hand. The same group's `props verify` reconciles it.\n")
...
func renderHarvestGo(rows []HarvestRow, pkgName string) (src []byte, err error) {
	var b strings.Builder
	// One line, and it stays one line: Go's generated-file convention is
	// detected by a regex anchored to `^// Code generated .* DO NOT EDIT\.$`,
	// so wrapping this would hide the marker from every tool that honours it.
	// `--tracked` is part of the command, not decoration — `props drift` gates
	// this file against the tracked scope, so a regen without it fails.
	b.WriteString("// Code generated by `boxer code analysis golang wasmsurvey props harvest --tracked --emit go`; DO NOT EDIT.\n\n")
	fmt.Fprintf(&b, "package %s\n\n", pkgName)
	fmt.Fprintf(&b, "import %q\n\n", propsImportPath)
