...
// OpenMddocStore is NewMddocStore plus the startup check: it runs VerifySchema and
...
// caller's layer (ADR-0105 D1). NewMddocStore panics on a malformed config
// and so does this. A caller that cannot afford the DESCRIBE at startup
// keeps using NewMddocStore.
func OpenMddocStore(ctx context.Context, exec recordstore.ExecutorI, alloc memory.Allocator, cfg MddocStoreConfig) (inst *MddocStore, err error) {
	st := NewMddocStore(exec, alloc, cfg)
	err = st.VerifySchema(ctx)
	if err != nil {
		st.Close()
		err = eh.Errorf("open mddocStore: %w", err)
		return
	}
	inst = st
	return
...
func (inst *MddocStore) FlushIfDue(ctx context.Context) (n int, err error) {
