...
The helper UDFs ship in [`lw_dql_udfs.sql`](./lw_dql_udfs.sql) (accessor `HelperUDFsSQL()`),
...
verified by a truth-table run through `clickhouse-local` (`lw_dql_udfs_test.go`). Naming follows
...
**Generated artefacts** (`lw_dql_generator.go`, `Generator.Generate`):
