...
func writeSectionReadInterfaces(sb *strings.Builder, plan *Plan, g SectionGroup) (err error) {
	...
	if len(g.SubColumns) > 1 {
		for _, sc := range g.SubColumns {
			f := sc.Fields[0]
			linef(sb, 1, "GetAttrValue%s(entityIdx raruntime.EntityIdx, attrIdx raruntime.AttributeIdx) %s", UpperFirst(sc.Name), f.GoType)
		}
	} else {
...
func writeMultiSubColumnDecode(sb *strings.Builder, g SectionGroup, attrsVar, membsVar, prefix string) (err error) {
	...
	linef(sb, 2, "for attrJ := int64(0); attrJ < n%s; attrJ++ {", prefix)
	for _, s := range subs {
		linef(sb, 3, "%s%sLocal := %s.GetAttrValue%s(raruntime.EntityIdx(i), raruntime.AttributeIdx(attrJ))", prefix, s.Field.GoFieldName, attrsVar, UpperFirst(s.ColName))
	}
