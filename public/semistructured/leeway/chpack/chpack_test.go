...
// any diff here is either a new function (append below, and bump
// lwsqlsurface.Version — one marker covers all three families) or a mistake.
...
func TestRosterInvariants(t *testing.T) {
	...
	// No marker here: the surface owns the one marker for all three
	// families (ADR-0171 §SD2), and lwsqlsurface's tests pin it. A marker
	// reappearing in this roster is what that decision forbids.
	for _, f := range fns {
