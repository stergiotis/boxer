...
// Version is the pack revision reported by leewayPackVersion(). Bump on any
...
const VersionFunctionName = "leewayPackVersion"
...
// except raggedNest, whose codomain is nested — it is the explicit
...
func Functions() (fns []Function) {
	fns = []Function{
		...
		},
		{
			...
			Doc:    "coLookup with an absent key distinguishable from a stored default: NULL when k is not present",
		},
		{
			Name:   "coGather",
		...
		},
		{
			...
			Doc:    "permutation that sorts keys; coGather every sibling lane through it to sort co-lanes consistently",
		},
		{
			Name:   "coArgMax",
		...
		{
			Name:   "raggedParentIds",
			Params: []string{"card"},
			Body:   "arrayFlatten(arrayMap((i, c) -> arrayWithConstant(c, i), arrayEnumerate(card), card))",
			Doc:    "instance index of every stream element; coGather an instance lane through it to broadcast",
		},
		{
			Name:   "raggedIota",
			Params: []string{"card"},
			Body:   "arrayMap((e, s) -> e - s + 1, arrayEnumerate(raggedParentIds(card)), coGather(raggedStarts(card), raggedParentIds(card)))",
		...
		},
		{
			...
			Body:   "arrayReduceInRanges(agg, raggedRanges(card), vals)",
		...
		},
		{
			...
			Body:   "arrayReduceInRanges('max', raggedRanges(card), arrayMap(f, vals))",
			Doc:    "per-run existence of a per-element predicate, fused (range-max over the lifted boolean stream)",
		},
		{
			Name:   "raggedCount",
			Params: []string{"f", "vals", "card"},
			Body:   "arrayReduceInRanges('sum', raggedRanges(card), arrayMap(f, vals))",
