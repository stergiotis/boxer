...
func canonicaltypesignatureparserParserInit() {
	...
	staticData.serializedATN = []int32{
		4, 1, 20, 117, 2, 0, 7, 0, 2, 1, 7, 1, 2, 2, 7, 2, 2, 3, 7, 3, 2, 4, 7,
		...
		0, 43, 11, 1, 0, 0, 0, 44, 45, 5, 16, 0, 0, 45, 46, 5, 20, 0, 0, 46, 13,
		...
		57, 5, 20, 0, 0, 56, 58, 3, 8, 4, 0, 57, 56, 1, 0, 0, 0, 57, 58, 1, 0,
		0, 0, 58, 75, 1, 0, 0, 0, 59, 60, 3, 2, 1, 0, 60, 62, 5, 20, 0, 0, 61,
...
const (
	...
	CanonicalTypeSignatureParserCIDR_MODIFIER    = 19
	CanonicalTypeSignatureParserNUMBER           = 20
)
...
func (s *CanonicalTypeNetworkContext) CIDR_MODIFIER() antlr.TerminalNode {
	return s.GetToken(CanonicalTypeSignatureParserCIDR_MODIFIER, 0)
}
...
func (p *CanonicalTypeSignatureParser) CanonicalType() (localctx ICanonicalTypeContext) {
	...
	case CanonicalTypeSignatureParserIPV4, CanonicalTypeSignatureParserIPV6:
		...
		if _la == CanonicalTypeSignatureParserCIDR_MODIFIER {
			{
				p.SetState(68)
				p.Match(CanonicalTypeSignatureParserCIDR_MODIFIER)
				if p.HasError() {
					// Recognition error - abort rule
					goto errorExit
				}
