...
// LabeledTextDocColumns is the SoA storage for batches of LabeledTextDoc rows.
// All slices grow in lockstep — Len returns the row count.
type LabeledTextDocColumns struct {
	ID       []uint64
	Tracking [][]byte
...
func (c *LabeledTextDocColumns) Len() int { return len(c.ID) }
...
func (c *LabeledTextDocColumns) Append(row LabeledTextDoc) {
	c.ID = append(c.ID, row.ID)
	c.Tracking = append(c.Tracking, row.Tracking)
	c.Texts = append(c.Texts, row.Texts)
}

// Row reconstructs entity i as an AoS LabeledTextDoc record. Inverse of
// Append: slice / pointer fields are shared by reference (no
// defensive copy); scalar fields and Option[T] are copied.
func (c *LabeledTextDocColumns) Row(i int) (row LabeledTextDoc) {
	row.ID = c.ID[i]
...
//
// LabeledTextDocBuildEntities walks the SoA columns and emits one entity per
...
// LabeledTextDocTextAttrI is the InAttr-side view of the text section. P-variants only —
// every method returns void so no F-bounded `[Self]` parameter is
// needed.
type LabeledTextDocTextAttrI interface {
	dmlruntime.InAttributeMembershipLowCardVerbatimPI
	AddToCoContainersP(wordLength uint32, wordBag string)
	EndAttributeP()
}

// LabeledTextDocTextSecI is the Section-side view: opens an attribute and closes
// the section. Attr and Ent are bound at the call site by inference.
type LabeledTextDocTextSecI[Attr any, Ent any] interface {
	BeginAttribute(text string) Attr
	EndSection() Ent
}

// LabeledTextDocEntityI is the entity-builder surface LabeledTextDocAddSections drives.
...
// LabeledTextDocBuildEntities walks c row-by-row, drives dml's entity / section
...
func LabeledTextDocBuildEntities[
	TextAttr LabeledTextDocTextAttrI,
	TextSec LabeledTextDocTextSecI[TextAttr, Ent],
	Ent any,
	DML LabeledTextDocEntityI[
		TextAttr, TextSec,
		Ent,
	],
](dml DML, c *LabeledTextDocColumns) (err error) {
	n := c.Len()
	for i := 0; i < n; i++ {
		dml.BeginEntity()
...
func LabeledTextDocAddSections[
	TextAttr LabeledTextDocTextAttrI,
	TextSec LabeledTextDocTextSecI[TextAttr, Ent],
	Ent any,
	DML LabeledTextDocEntityI[
...
//
// LabeledTextDocFillFromArrow walks the Arrow record row-by-row and appends
...
// LabeledTextDocTextAttrsReadI is the Attributes-side view of the text section.
type LabeledTextDocTextAttrsReadI interface {
...
// LabeledTextDocTextMembsReadI is the Memberships-side view of the text section.
type LabeledTextDocTextMembsReadI interface {
	GetMembValueLowCardVerbatim(entityIdx raruntime.EntityIdx, attrIdx raruntime.AttributeIdx) iter.Seq[[]byte]
}

// LabeledTextDocFillFromArrow walks rec row-by-row and appends each entity's
...
func LabeledTextDocFillFromArrow[
	TextAttrs LabeledTextDocTextAttrsReadI,
	TextMembs LabeledTextDocTextMembsReadI,
](
	c *LabeledTextDocColumns,
	n int,
	idCol *array.Uint64,
	nkCol *array.Binary,
	textAttrs TextAttrs,
	...
	for i := 0; i < n; i++ {
		...
		// --- text. ---
		var textTextsElems []LabeledText
...
// LabeledTextDocReadRow is not emitted: field Texts is a dynamic-membership tuple.
