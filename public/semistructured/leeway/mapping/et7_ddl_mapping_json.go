...
func LoadJsonMapping(manip common.TableManipulatorFluidI) {
	manip.PlainValueColumn(common.PlainItemTypeEntityId, "blake3hash").
		ColumnCanonicalType(canonicalTypes2.StringAstNode{BaseType: canonicalTypes2.BaseTypeStringBytes}).
	...
	manip.TaggedValueSection("bool").
		...
		ColumnCanonicalType(canonicalTypes2.StringAstNode{BaseType: canonicalTypes2.BaseTypeStringBool})
	...
	manip.TaggedValueSection("string").
		AddSectionMembership(common.MembershipSpecMixedLowCardVerbatimHighCardParameters).
		TaggedValueColumn("value").
		ColumnCanonicalType(canonicalTypes2.StringAstNode{BaseType: canonicalTypes2.BaseTypeStringUtf8}).
		AddColumnEncodingHints(encodingaspects2.AspectLightGeneralCompression)
	...
	manip.TaggedValueSection("float64").
		AddSectionMembership(common.MembershipSpecMixedLowCardVerbatimHighCardParameters).
		TaggedValueColumn("value").
		ColumnCanonicalType(canonicalTypes2.MachineNumericTypeAstNode{BaseType: canonicalTypes2.BaseTypeMachineNumericFloat, Width: 64})
	manip.TaggedValueSection("int64").
		AddSectionMembership(common.MembershipSpecMixedLowCardVerbatimHighCardParameters).
		TaggedValueColumn("value").
		ColumnCanonicalType(canonicalTypes2.MachineNumericTypeAstNode{BaseType: canonicalTypes2.BaseTypeMachineNumericSigned, Width: 64})
}
