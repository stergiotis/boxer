...
func LoadAccelFieldFromRecord[F, B IndexConstraintI](idx uint32, rec arrow.Record, dest *RandomAccessTwoLevelLookupAccel[F, B, int, int64]) (err error) {
...
func LoadScalarValueFieldFromRecord[S any](idx uint32, expectedDatatype arrow.Type, rec arrow.Record, dest **S, ctor func(data arrow.ArrayData) *S) (err error) {
	c := rec.Column(int(idx))
...
func LoadNonScalarValueFieldFromRecord[S any](idx uint32, expectedDatatype arrow.Type, rec arrow.Record, dest **array.List, destElementAccess **S, ctorElementAccess func(data arrow.ArrayData) *S) (err error) {
	c := rec.Column(int(idx))
	if c.DataType().ID() != arrow.LIST {
		err = eb.Build().Uint32("columnIndex", idx).Stringer("effective", c.DataType()).Stringer("expected", arrow.LIST).Errorf("unexpected data type: %w", ErrUnexpectedArrowDataType)
		return
