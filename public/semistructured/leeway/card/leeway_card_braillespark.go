...
// BrailleSpark is an OnlineApiSinkI that renders entity topology as a
...
func (s *BrailleSpark) BeginSection(name naming.StylableName, valueNames []naming.StylableName, valueCanonicalTypes []canonicaltypes.PrimitiveAstNodeI, _ useaspects.AspectSet, nAttrs int) {
	if !s.firstSection {
		if s.inCoGroup {
			s.line.WriteString("·")
		} else {
...
func (s *BrailleSpark) BeginColumn(colAddr streamreadaccess.PhysicalColumnAddr, name naming.StylableName, canonicalType canonicaltypes.PrimitiveAstNodeI, _ valueaspects.AspectSet) {
	if s.inPlainSection {
		s.line.WriteString("▪")
	}
}
