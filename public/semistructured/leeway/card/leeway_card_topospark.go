...
// TopologySpark is an OnlineApiSinkI that writes a single-line unicode
...
func (s *TopologySpark) BeginSection(name naming.StylableName, valueNames []naming.StylableName, valueCanonicalTypes []canonicaltypes.PrimitiveAstNodeI, _ useaspects.AspectSet, nAttrs int) {
	if !s.inCoGroup {
		if s.line.Len() > 0 {
			s.line.WriteByte(' ')
		}
...
func (s *TopologySpark) BeginColumn(colAddr streamreadaccess.PhysicalColumnAddr, name naming.StylableName, canonicalType canonicaltypes.PrimitiveAstNodeI, _ valueaspects.AspectSet) {
	if s.inPlainSection {
		if s.plainColIdx > 0 {
			s.line.WriteString("·")
		}
