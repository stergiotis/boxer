...
// SvgTopologySink is an OnlineApiSinkI that renders a compact SVG
...
func (s *SvgTopologySink) BeginSection(name naming.StylableName, valueNames []naming.StylableName, valueCanonicalTypes []canonicaltypes.PrimitiveAstNodeI, _ useaspects.AspectSet, nAttrs int) {
	sec := svgSection{
		name:      name.String(),
		nCols:     len(valueNames),
		nAttrs:    nAttrs,
...
func (s *SvgTopologySink) BeginColumn(colAddr streamreadaccess.PhysicalColumnAddr, name naming.StylableName, canonicalType canonicaltypes.PrimitiveAstNodeI, _ valueaspects.AspectSet) {
	if s.inPlain {
		s.plainCols++
		if s.curSection != nil {
