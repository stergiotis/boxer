...
// TreemapSpark is an OnlineApiSinkI that renders each entity's topology
...
func (s *TreemapSpark) BeginSection(name naming.StylableName, valueNames []naming.StylableName, valueCanonicalTypes []canonicaltypes.PrimitiveAstNodeI, _ useaspects.AspectSet, nAttrs int) {
	s.cur = &treemapCell{
		name:      name.String(),
		nCols:     len(valueNames),
		nAttrs:    nAttrs,
...
func (s *TreemapSpark) BeginColumn(colAddr streamreadaccess.PhysicalColumnAddr, name naming.StylableName, canonicalType canonicaltypes.PrimitiveAstNodeI, _ valueaspects.AspectSet) {
	if s.inPlainSection {
		s.plainColCount++
		if s.cur != nil {
