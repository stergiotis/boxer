...
-- ahead of this file, and level-2 unflattening is the pack's LW_RAGGED_NEST.
--
...
-- `m2v` = LW_RAGGED_PARENT_IDS(cardcol), computed once per row.
CREATE OR REPLACE FUNCTION LW_LU_ATTR_BY_TAG AS (tagcol, tagval, m2v) ->
    m2v[indexOf(tagcol, tagval)];

-- Membership set an attribute plays on a channel (aliasing-aware).
CREATE OR REPLACE FUNCTION LW_LU_MEMBS_OF_VAL_IDX AS (membcol, cardcol, validx) ->
    arraySlice(membcol, LW_LU_VAL_IDX_TO_MEMB_IDX_BEGIN_INCL(cardcol)[validx], cardcol[validx]);

-- Scalar value of the attribute tagged with `tagval`; default ('' / 0) if absent.
CREATE OR REPLACE FUNCTION LW_VALUE_BY_TAG_EQUAL AS (valcol, tagcol, tagval, m2v) ->
    valcol[LW_LU_ATTR_BY_TAG(tagcol, tagval, m2v)];

-- Level-2 unflatten (flattened value array -> array-of-arrays by the length
-- column) is the pack's LW_RAGGED_NEST, emitted ahead of this file.
...
CREATE OR REPLACE FUNCTION LW_LIST_BY_TAG_EQUAL AS (valFlat, lencol, tagcol, tagval, m2v) ->
    arraySlice(valFlat,
               LW_LU_VAL_IDX_TO_MEMB_IDX_BEGIN_INCL(lencol)[LW_LU_ATTR_BY_TAG(tagcol, tagval, m2v)],
               lencol[LW_LU_ATTR_BY_TAG(tagcol, tagval, m2v)]);
