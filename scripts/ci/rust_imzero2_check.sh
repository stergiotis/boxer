...
# ADR-0215 retired the feature, so every shape below IS the allocator-free one.
# That also collapses the ADR-0205 M6 appliance check into the matrix —
# `libmimalloc-sys` and its cross C toolchain are gone from every graph rather
# than from one specially-checked graph.
for features in \
...
run_step "check desktop (default features)" cargo check --locked --quiet --target-dir "$target" --all-targets
...
run_step "clippy (default features)" \
    cargo clippy --locked --quiet --target-dir "$target" --all-targets -- -D warnings -W clippy::all
...
run_step "test --features headless_soft" \
    cargo test --locked --quiet --no-default-features --features headless_soft --target-dir "$target"
...
for features in headless headless_svg headless_soft; do
    if cargo tree --locked --quiet --no-default-features --features "$features" \
        ...
        cargo tree --locked --quiet --no-default-features --features "$features" \
            -e normal --prefix none --target x86_64-unknown-linux-gnu 2>/dev/null |
