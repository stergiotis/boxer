...
| Node size grows with incoming link count | caller maps in-degree to `Radius`, from the query or from `nav.Degree(id, nav.DirectionIn)` | caller | One `groupUniqArray` over the backlinks query, or no query at all since ADR-0225's 2026-09-12 update publishes the degree the helper already knew. A radius-aware repulsion (NetChart §3) keeps large hubs from overlapping. |
...
| Hover: highlight the note's connections, dim the rest | the caller reads `HoveredNode`, asks `nav.Neighbours`, and fades the rest with `NodeSpec.Opacity` / `EdgeSpec.Opacity` | ✓ (caller) | Both halves landed on 2026-09-12: ADR-0224 §SD14 gave the fade, ADR-0225's update gave the neighbour set. The widget does not do it itself on purpose — what counts as a neighbour, one hop or two, in or out, is the vault owner's question. One frame late by SD5, which for a hover is not visible. |
