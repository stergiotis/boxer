...
| --- | --- | --- |
| Capture | `--pprofCpuOutputFile` (file, `runtime/pprof`) and `--pprofHttpListenAddress` (`net/http/pprof`, in the `pprofhttp` subpackage a host imports explicitly — ADR-0212). Both are runtime flags; no build tag gates either | [`public/observability/profiling/`](../../public/observability/profiling/profiling.go) |
