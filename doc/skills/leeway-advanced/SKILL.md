...
description: "Use for advanced Leeway columnar modelling — multi-membership encoding, co-sections, membership roles, and mapping JSON to an equivalent Leeway representation. For the component/ECS layer (component DTOs over sections, overlap rules, archetypes) see the leeway-components skill."
type: reference
...
Under boxer ADR-0007 the canonical JSON layout is attribute-centric: primary memberships form attribute keys, secondary memberships ride in a per-attribute `labels` slot. Aliasing (multiple primary memberships on one value, e.g. `/price/current` ≡ `/stats/min`) collapses into one attribute object with an `aliases` field. JSON Schema / data-contract generation reads the classifier output to decide `properties` + `required` for primary versus a scoped extension slot for secondary.
