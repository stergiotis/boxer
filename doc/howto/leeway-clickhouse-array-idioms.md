...
```sql
CREATE OR REPLACE FUNCTION LW_RAGGED_NEST AS (vals, card) ->
...
- **Lambdas can be parameters.** A UDF may accept a lambda and forward it to
  ...
  `CREATE FUNCTION LW_RAGGED_EXISTS AS (f, vals, card) -> arrayReduceInRanges('max', arrayMap((h, c) -> (h - c + 1, c), arrayCumSum(card), card), arrayMap(f, vals))`
...
- **Prefer fused bodies.** Materializing per-instance lists copies the
  ...
  exists ran roughly 1.5–2.4× faster than their `LW_RAGGED_NEST`-based
  equivalents (26.7, single-threaded). Reserve `LW_RAGGED_NEST` for genuinely
...
```sql
SELECT LW_RAGGED_REDUCE('avg', vals, card)
FROM t
WHERE LW_CO_EXISTS_EQ2("string:semanticType", 'myst', "string:short", 'abc')
```
