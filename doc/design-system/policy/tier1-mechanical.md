...
Painter free-functions are a second trigger class. Their coordinate args are bare numeric literals too, so the either-arg approach above would false-positive on them; instead each registered function carries the 0-indexed position of its `strokeWidth` arg (the same per-name table idiom L11 uses for `durSecs`), and only that position is inspected. Registered today: `c.PaintEllipseStroke(cx,cy,rx,ry,color,width)` — width at index 5.
...
**Not yet registered.** `c.PaintRectStroke(x1,y1,x2,y2,radius,color,width)` and `c.PaintCircleStroke(cx,cy,r,color,width)`. The per-name width-index table covers their shape, but registering a function expands the trigger surface — do it deliberately, sweeping the new callers to zero in the same change (as `PaintEllipseStroke`'s registration did).
