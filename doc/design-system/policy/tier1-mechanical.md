...
Painter free-functions are a second trigger class. Their coordinate args are bare numeric literals too, so the either-arg approach above would false-positive on them; instead each registered function carries the 0-indexed position of its `strokeWidth` arg (the same per-name table idiom L11 uses for `durSecs`), and only that position is inspected. Registered: `c.PaintCircleStroke(cx,cy,r,color,width)` — index 4; `c.PaintEllipseStroke(cx,cy,rx,ry,color,width)` — index 5; `c.PaintRectStroke(minX,minY,maxX,maxY,rounding,color,width)` — index 6 (the rounding arg is L4's concern, not L10's).
...
**Registering a new painter surface.** All three painter strokes are in the table today. A future stroke-bearing free function joins by adding its name and width index — deliberately, sweeping the new callers to zero in the same change (as the `PaintEllipseStroke` and `PaintRectStroke` / `PaintCircleStroke` registrations did). Off-ladder widths found in a sweep normalize to the nearest rung rather than earning ignores, unless the site documents a reason.
