...
Boxer's imported platform stack hosts four distinct concerns that until now shared a single namespace prefix (`public/thestack/`):
...
- A **data-centrality layer**: ClickHouse client + low-latency local-CH pool ([ADR-0028](./0028-chlocal-low-latency-sql-cap.md)) — `chclient`, `chlocalbroker`, `chlocalpool` — plus the leeway columnar protocol (`public/semistructured/leeway/`).
- A **security model**: the capslock-cross-checker at `public/app/commands/capslock/` enforces ADR-0026 §SD10 (Manifest.Caps must match the static call graph) and is the visible surface of the cap-as-subject design.
- A **GUI framework + apps**: ImZero2 + FFFI2 plus a growing set of `AppI` implementations (`imztop`, `capinspector`, `capdemo`, demo carousel apps, `spinnaker/hmi/play`).

Three forces make the current naming untenable:

- **No visible boundary.** A new contributor cannot see in `ls public/thestack/` where the platform spine ends and the GUI tech / apps begin. The historical name `thestack` is a placeholder that conveys nothing.
- **ImZero2 upstreamed into boxer.** When `thestack/imzero2/` and `thestack/fffi2/` came across, the residue was a platform with no name; settling its identity *before* the move kept the path migration a one-step rename rather than two.
...
- **O1 — Repo + module rename.** Rename the repo and module to `keelson` everywhere: repo name, `go.mod` path (`github.com/stergiotis/keelson`), import paths across all 392 affected files.
- **O2 — Subdirectory namespace (chosen).** Repo and `go.mod` stay `boxer`. `public/keelson/` becomes a subdirectory that gathers platform packages under three pillars (data / runtime / security). Apps live at a sibling top-level `apps/<name>/`. ImZero2 / FFFI2 stay at `public/thestack/`.
- **O3 — Branding and docs only.** The name appears in README, ADRs, package docs, and CLI banners. No code paths change.
- **O4 — Module sub-path.** Keep `github.com/stergiotis/boxer` but introduce `github.com/stergiotis/boxer/keelson` as a separate Go module (its own `go.mod`).
...
- **C4 — Future-fit.** Does the layout survive the imzero2 / FFFI2 consolidation into boxer without needing another reshuffle?
...
We introduce **`public/keelson/`** as a subdirectory namespace gathering the platform spine under three pillars:
...
- **`keelson/security/capslock/`** — the cap-cross-checker package (formerly `public/app/commands/capslock/main.go`). The `public/app/commands/capslock/main.go` shim is retained as the binary entrypoint, importing the new package.

Standalone apps move to a sibling top-level **`apps/<name>/`**: initially `imztop`, `capdemo`, `capinspector`. The carousel-embedded demo apps (`thestack/imzero2/egui2/demo/apps/*`) and `spinnaker/hmi/play` stay at their current locations — disentangling them from the imzero2 host topology is more churn than payoff.

ImZero2 and FFFI2 stay at `public/thestack/`. `public/storage/` and `public/db/` stay put — the data pillar covers the *runtime-side* ClickHouse glue, not the CLI-side data factories.

Repository name and Go module path stay `github.com/stergiotis/boxer`. Keelson is a **directory boundary, not a module boundary**.
...
The migration ships as 7 commits behind one ADR, executable autonomously. Build-tag rule: `tags=$(cat tags | tr -d $'\n')`; every `go build` invocation uses `-tags "$tags"`. After each step: `go build -tags "$tags" ./... && bash scripts/ci/lint.sh`. Steps marked `(+test)` additionally run `bash scripts/ci/gotest.sh`. Steps marked `(+capslock)` additionally run `bash scripts/ci/capslock.sh`.

### Step 0 — Pre-flight baseline

`go build -tags "$tags" ./... && bash scripts/ci/lint.sh && bash scripts/ci/gotest.sh`. Capture `go vet -tags "$tags" ./...` output for delta comparison. Do not proceed if the baseline is dirty.

### Step 1 — Skeleton and ADR (this commit + EXPLANATION.md)

- `mkdir -p public/keelson/{runtime,data,security/capslock} apps`.
- Seed `public/keelson/EXPLANATION.md` via `scripts/new-doc.sh explanation`; fill the three-pillars rationale (substantive content, not a placeholder — Step 7 then has no re-flesh to do).
...
- Gate: `bash scripts/ci/doclint.sh public/keelson/ doc/adr/0035-keelson-namespace-introduction.md`.

### Step 2 — Security pillar: `capslock-check` → `keelson/security/capslock`

- `git mv public/app/commands/capslock public/keelson/security/capslock`.
...
- Create a new `public/app/commands/capslock/main.go` (~10 lines) that imports `keelson/security/capslock` and calls `capslock.Run(os.Args)`.
- Leave `trustBoundaryPackages` constants pointing at `thestack/runtime/...` for now; Step 3 sweeps them.
- Gate: `go build -tags "$tags" ./... && bash scripts/ci/lint.sh && bash scripts/ci/capslock.sh`.
...
- `git mv public/thestack/runtime public/keelson/runtime`.
- Mechanical sweep (the sed itself self-edits these very commands when run; re-run is safe because it becomes a no-op):
  ```
  find public apps -name '*.go' -print0 | xargs -0 sed -i \
    -e 's|github.com/stergiotis/boxer/public/thestack/runtime|github.com/stergiotis/boxer/public/keelson/runtime|g'
  find doc scripts -type f \( -name '*.md' -o -name '*.sh' \) -print0 | xargs -0 sed -i \
    -e 's|github.com/stergiotis/boxer/public/thestack/runtime|github.com/stergiotis/boxer/public/keelson/runtime|g' \
    -e 's|public/thestack/runtime|public/keelson/runtime|g'
  ```
- `goimports -l ./...` must return empty.
- Regenerate `runtime/factsschema/dml/runtime_facts_dml.out.go`: `go run -tags "$tags" ./public/app/commands/runtimecodegen`.
...
- Gate: `go build -tags "$tags" ./... && bash scripts/ci/lint.sh && bash scripts/ci/gotest.sh && bash scripts/ci/capslock.sh`.

### Step 4 — Data pillar: lift `chclient`, `chlocalbroker`, `chlocalpool` out of `runtime/`

- `git mv public/keelson/runtime/chclient public/keelson/data/chclient` (and `chlocalbroker`, `chlocalpool`). The destination `keelson/data/` must exist before the moves; if it has been recursively removed (e.g., by an earlier `rmdir`), recreate it first with `mkdir -p public/keelson/data`.
- Sweep (the sed itself self-edits the lines below; re-run is a no-op):
  ```
  find public apps doc scripts -type f \( -name '*.go' -o -name '*.md' -o -name '*.sh' \) -print0 | xargs -0 sed -i \
...
- `git mv public/thestack/<app> apps/<app>`.
- Sweep:
  ```
  find public apps doc scripts -type f \( -name '*.go' -o -name '*.md' -o -name '*.sh' \) -print0 | xargs -0 sed -i \
    -e "s|github.com/stergiotis/boxer/public/thestack/<app>|github.com/stergiotis/boxer/apps/<app>|g" \
    -e "s|public/thestack/<app>|apps/<app>|g"
  ```
...
- Gate: `go build -tags "$tags" ./... && bash scripts/ci/lint.sh && bash scripts/ci/gotest.sh && bash scripts/ci/capslock.sh`.
- Smoke test: `bash rust/imzero2/hmi.sh imzero2 demo --launch imztop` runs to first paint without crashing.

### Step 6 — Regenerate generated headers (cosmetic)

`./generate.sh` and `./egui2gen.sh` refresh the generator-path comment headers in `.out.go` and `rust/imzero2/src/imzero2/*.rs`. Bodies unaffected. Gate: build + lint clean (no semantic change expected).

### Step 7 — Docs

- Flesh out `public/keelson/EXPLANATION.md` with the three-pillars rationale, what moved and from where, what didn't and why.
- Update root `CLAUDE.md` to add a "keelson namespace" item under the *project-local supplement*: keelson is the platform namespace (data + runtime + security); `apps/` is the top-level home for standalone AppIs; `thestack/` continues to host imzero2/fffi2.
...
- Move imzero2 demo apps and `spinnaker/hmi/play` into `apps/`.
- Home the remaining `spinnaker/` content.
- Decide `adversarial-review` and the CLI placement (currently `public/app/commands/` and `public/app/` respectively — both out of keelson scope for this round).
...
- `ls public/keelson/` displays the platform spine in one view: `runtime/`, `data/`, `security/`. `ls apps/` displays the standalone apps. `ls public/thestack/` collapses to the GUI framework + carousel-coupled apps awaiting their own move.
- The three-pillar layout survived the imzero2 / FFFI2 consolidation into boxer without reshuffle: `thestack/` and `keelson/` coexist cleanly, and imzero2 churn does not ripple into keelson.
- `public/app/commands/capslock/main.go` is now a 10-line shim, freeing the security package to grow library API (`Run`, `Check`, etc.) without coupling to `os.Args` / `os.Exit`.
...
- Wire-incompatible AppId migration: facts persisted under old `Manifest.Id` strings (`github.com/stergiotis/boxer/apps/imztop` etc.) are orphaned. The runtime is pre-stable; no live operators retain history; documented here so the choice is auditable. Mitigation (a legacy-alias table in `keelson/runtime/app/legacy.go`) is explicitly **not** taken — the indirection cost outweighs the value.
...
- The carousel host (`thestack/imzero2/egui2/demo/carousel/`) ends up importing `keelson/runtime/...` from `thestack/`, creating a thestack → keelson cross-tree dependency. This is acceptable (and inevitable while imzero2 lives at `thestack/`) but noted.

### Neutral

- `storage/`, `db/`, `observability/`, `identity/`, and the various small utility packages stay at their current locations. Reclassifying them is out of scope.
...
- `utfsafe` (consumed only by `spinnaker/hmi/play`) stays put.
