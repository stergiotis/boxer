...
- **FFFI2 rules.** Generated files under `components/*.out.go` and [`src/rust/src/imzero2/interpreter.rs`](../../src/rust/src/imzero2/interpreter.rs) are off-limits; all changes land in hand-written definition files and the generator. FFFI2 reserves argument names matching `[a-zA-Z][0-9]*` ([`fffi2/ir/idl/fffi2_ir_idl_arguments.go:23`](../../src/go/public/thestack/fffi2/ir/idl/fffi2_ir_idl_arguments.go)). Documented in [`doc/skills/fffi2/SKILLS.md`](../skills/fffi2/SKILLS.md).
...
- **Unified color type.** [ADR-0052](./0003-imzero2-unified-color-type.md) established `egui2.Color` as the Go type for single-argument colors, with `.AsColor()` annotation on IDL args. Bulk pixel buffers (arrays of per-pixel colors) are a distinct surface not addressed by that ADR.
...
We introduce `scrollingTexture` as a purpose-built widget that owns a per-widget-id `egui::TextureHandle` in a hand-written Rust module `src/rust/src/imzero2/scrolling_texture.rs`, accepts a pre-packed `u32[]` RGBA column payload per frame plus a caller-supplied head cursor, and renders via a two-call split-UV `Painter::image` draw. The widget is domain-neutral; it has no knowledge of spectrograms, frequencies, intensities, or colormaps.

All `f32 → RGBA` mapping — colormap lookup, intensity scaling (Linear / Log / Db), bad / underflow / overflow substitution — lives in a new Go-side package `src/go/public/thestack/imzero2/egui2/widgets/colormap/`. A thin wrapper package `src/go/public/thestack/imzero2/egui2/widgets/heatmapscroll/` composes the colormap with the `scrollingTexture` widget for the common "scientific heatmap from an `f32` stream" caller.
...
Accepted — 2026-04-23, reviewed by `p@stergiotis`. Design frozen; implementation begins with the IDL definition, the hand-written `src/rust/src/imzero2/scrolling_texture.rs` module skeleton, and `./generate.sh` seam verification (milestone 1 of the planning thread).
...
- [`doc/adr/0003-imzero2-unified-color-type.md`](./0003-imzero2-unified-color-type.md) — `egui2.Color` unified type; SD9 of this ADR documents the narrow exception for bulk pixel buffers.
- [`doc/adr/0005-regex-explorer-offset-authority.md`](./0005-regex-explorer-offset-authority.md), [`doc/adr/0006-adopt-boxer-standards.md`](./0006-adopt-boxer-standards.md), [`doc/adr/0007-walkers-map-h3-binding.md`](./0007-walkers-map-h3-binding.md) — prior ADRs; template shape followed here.
- [`boxer/doc/DOCUMENTATION_STANDARD.md`](../../../boxer/doc/DOCUMENTATION_STANDARD.md) — Diátaxis + ADR conventions followed by this document (canonical copy lives in boxer per [`CLAUDE.md`](../../CLAUDE.md)).
...
- [`src/go/public/thestack/imzero2/egui2/definition/`](../../src/go/public/thestack/imzero2/egui2/definition/) — hand-written IDL definition files; new definition lands here.
- [`src/go/public/thestack/imzero2/egui2/definition/egui2_definition_d_plot.go`](../../src/go/public/thestack/imzero2/egui2/definition/egui2_definition_d_plot.go) — structural precedent (homogeneous-array payload + `.WithApplyCodeClientRust(...)` apply snippets).
- [`src/rust/src/imzero2/interpreter.rs`](../../src/rust/src/imzero2/interpreter.rs) — generated dispatch; `scrollingTexture` apply snippets land here after `./generate.sh`, calling into the new hand-written `scrolling_texture.rs` module.
