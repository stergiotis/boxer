...
against a draft nested how-to (since merged into
[the marshalling how-to](../howto/leeway-marshalling.md), per D2) without
...
## Update (2026-07-19): D1 enacted

D1 landed (f654ae30, −511 net lines): `,explode` is removed from the shared
flag grammar — both front-ends reject it naming the nested `[]Attr`
replacement — taking the whole N×1 shape axis with it (`ClassifyBegin` shrinks
from five begin shapes to three, the explode write/read arms go from both
codecs, and the element-wise `[]marshalltypes.X` slice carrier goes with the
flag: carriers are scalar-only, matching D5's single-attribute posture).

Enactment precision on `,highCardVerbatim`: what is removed is the
**value-field** (and `_`-const) spelling, rejected at the shared builder. The
token remains the channel selector on tuple `@membership` fields and nested
memberships — those are the D2-frozen escalation surface, where the choice is
load-bearing — and the channel keeps its DML methods and wire presence, as
decided. Every in-tree `.out.go` regenerated byte-identical, confirming the
census's zero-consumer claim at the wire; both how-tos dropped the culled
spellings.

## Update (2026-07-19): D2's how-to merge done; Review fallout cleared

The two recipes are one document
([doc/howto/leeway-marshalling.md](../howto/leeway-marshalling.md)): simple
subset first, the nested attribute model as the escalation surface, the flat
`:column` / `@membership` spellings in a marked *frozen* section, carriers on
their live flat spelling. The per-front-end capability matrix is gone — the
deliberate asymmetries live as a prose *Deferred surfaces* list, and the parity
corpus is their mechanical record. `lw.U8Array` is settled by **striking** it:
it added no capability (`,ct=u8h` reaches the u8 lane from flat fields and
nested sub-columns alike) and D5's authoring policy adds grammar only with a
named consumer; the lane set stays IPv4 / IPv6 / the two CIDR prefixes.

The remaining Review-fallout items landed (51f90ddd): `Validate` now checks
scalar-section `BeginAttribute` arity and `BeginAttributeSingle`'s one-arg
contract (a scalar/container shape mix is reported instead of resolving to an
impossible signature); `ReadRowSupported` rejects const-only kinds (presence
...
- [doc/howto/leeway-marshalling.md](../howto/leeway-marshalling.md) — the one recipe D2 converged the former flat + nested pair into
