...
- **Scope.** Packages, dies/clusters, L1/L2/L3 caches, cores, SMT threads (PUs), NUMA-node grouping, and per-node memory size (`MemTotal`). Live per-node usage, PCI/GPU/NIC locality, and physical DIMM inventory (DMI/EDAC) — the rest of full hwloc parity — are deliberately deferred.
