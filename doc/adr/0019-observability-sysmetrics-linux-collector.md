...
[btop](https://github.com/aristocratos/btop) is the obvious reference: its Linux collectors are well-tested, cover the gotcha matrix (cgroup-effective CPU sets, hwmon label discovery, `rx_bytes` rollover, ZFS ARC accounting, RAPL energy_uj deltas, AMD ROCm-SMI v5/v6 ABI fork) that any from-scratch implementation would re-discover one outage at a time. The license — verified at [its LICENSE](https://github.com/aristocratos/btop/blob/main/LICENSE) — is **Apache 2.0**, identical-spirit to this repo, so we can mirror structure and cite line:file freely.
...
- **SD4 — Variable-length sequences are `iter.Seq2[T, error]`.** Process table, GPU device list, mounted-disk list, network-interface list. Per [CODINGSTANDARDS.md](../../CODINGSTANDARDS.md) the standard names are `All` / `Values` / `Keys` / `Backward`. Callers that want a slice call `slices.Collect2`. Per-row errors do not abort iteration unless the caller breaks; the iterator yields `(zero, err)` and the next row.
...
- [`btop/LICENSE`](https://github.com/aristocratos/btop/blob/main/LICENSE) — Apache 2.0; structural mirroring with attribution is unencumbered.
- [`btop/src/linux/btop_collect.cpp`](https://github.com/aristocratos/btop/blob/main/src/linux/btop_collect.cpp) — Linux collector implementations; primary provenance source.
- [`btop/src/btop_shared.hpp`](https://github.com/aristocratos/btop/blob/main/src/btop_shared.hpp) — upstream Snapshot struct shapes.
