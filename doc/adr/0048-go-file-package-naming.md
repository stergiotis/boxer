...
- **2026-08-17 — N2/N3 enforcement shipped; scope turned out wider than the
  two files `scripts/ci/lint.sh`'s gofmt step already named.** Rules N2
  ...
  single `generated-suffix` finding since content alone cannot tell which of
  the two a generator intended.
  A `_test.go` file is exempt on principle, not as a grandfather: Go's test
  discovery requires the literal `_test.go` suffix, which a generated test
  file cannot trade for `.out.go`/`.gen.go` and still run under `go test`.
  Turning the check on found 23 pre-existing violations, not the two the
  gofmt-step comment named (that comment is about gofmt drift specifically,
  a narrower thing) — spanning four subsystems: the vendored/generated
  design-token tree (`styletokens/palette_generated.go` and 17 files under
  the already-baselined `data_encoding` package), one `go:generate` output
  named with an underscore instead of a dot
  (`help/search/chaliases_gen.go`), and a "Code generated by copy paste;
  DO NOT EDIT." family with no generator to repoint
  (`science/units`, `leeway/encodingaspects`, `leeway/useaspects`,
  `leeway/valueaspects`). All 23 were seeded into `naming-baseline.txt`
  rather than renamed in the same change, per this ADR's existing Migration
  policy (no dedicated rename PR; migrate opportunistically when a file is
  otherwise touched) — renaming still requires repointing each one's
  generator (or, for the copy-paste family, is a bare rename with no
