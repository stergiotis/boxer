...
- **Patches are identity-by-hash.** A patch's `PatchHash` is the keyed BLAKE3-256 digest of a deterministic-CBOR item over its canonicalised dependency set plus its changes ([ADR-0209](./0209-pushout-cbor-identity-and-wire.md); `Patch.ComputeHash`) — dependencies are inside the hash, so identity chains transitively. Every `NodeID` introduced by a patch carries that hash. Dependent patches reference it in their `Dependencies` slice and in their changes' context fields. The hash is the patch and cannot be changed without orphaning everything downstream. Deliberately *outside* the hash: `Author` / `Description` (patch level) and `Producer` / `Timestamp` (envelope level) are provenance, not identity — see inventory items 1–2 for what that buys.
- **Apply is monotonic and append-only.** The `PushoutGraph` data structure tombstones deleted nodes rather than removing them; pseudo-edges bridge over deleted regions to keep the live subgraph connected. The current `Unrecord` (`pijul_pushout_backend.go:Unrecord`) explicitly preserves the patch envelope in `MetaByHash` so the patch can be reapplied after a Pull from a peer that still holds it. The whole correctness story (commutativity, associativity, frictionless cherry-pick) depends on patches being permanent objects. The pushoutgraph ships a storage-limitation mechanism: `PushoutGraph.SweepTombstones(now, horizon)` destroys the content bytes of tombstoned nodes past a retention horizon, `NodeContentStatusE` (`Missing` / `Present` / `Purged`) lets data-protection audits distinguish never-recorded from deliberately destroyed, and `Unapply` of an affected patch fails closed past the horizon ("patch is permanent past retention"). The sweep operates on pushoutgraph state only — patch envelopes retain their `Change.Content` — so it discharges Art 5(1)(e), not Art 17.
...
| `SweepTombstones` *(pushoutgraph-level)* | Destroys tombstoned nodes' content past a retention horizon; marks them `Purged`; returns purged IDs for audit. | Reduces ghost residue (R9). **Not erasure** — envelopes keep `Change.Content`. |
...
| `State` | Materialises live cells from the pushoutgraph plus the applied-patch log. | Reads everything currently visible. |

The pushoutgraph data structure underneath (`public/algebraicarch/pushout/pushoutgraph/store/pushoutgraph.go`) holds two node sets (live + tombstoned), a content map, forward / backward edge lists, and pseudo-edge bookkeeping. Tombstoned nodes retain their content because `Unapply` may resurrect them.
...
1. **Patch metadata** — `Patch.Author` and `Patch.Description` (`pushoutgraph/patch/patch.go`). Plain UTF-8 strings recorded into every envelope. Author is typically a name or email; description is free-form. Propagated verbatim via Push/Pull. Both fields sit deliberately *outside* the patch hash (`ComputeHash` doc comment): local envelopes can be rewritten without breaking patch identity, so provenance is rectifiable in place on systems the controller reaches — a lever the hashed payload does not have (peer copies are unaffected, so this is rectification, not erasure). Under the A decision the default policy (SD10) routes these fields through the vault like any other PII-bearing field.
...
3. **Patch change content** — `Change.Content` for `ChangeKindNewNode` (`pushoutgraph/patch/patch.go`). The bytes the user recorded — the substantive payload of the VCS. Cardinality unbounded; could contain anything. Unlike items 1–2, this field is *inside* the patch hash: whatever enters it is identity-bearing and structurally un-erasable, which is why Architecture A keeps identifiable data out of it entirely.

4. **PushoutGraph node content** — `PushoutGraph.contents[NodeID]` (`pushoutgraph/store/pushoutgraph.go`). Persists for both live and tombstoned nodes. `DeleteNode` does not remove content; `SweepTombstones` destroys tombstoned content past a retention horizon and records the fact in `contentPurged` (see R9, SD8).
...
8. **Rendered output** — the text file produced by `PushoutGraph.Render` (`pushoutgraph/store/render.go`). Includes all live node content and conflict markers; this is the form most consumers see.

The first three travel in patch envelopes; the fourth lives in pushoutgraph state; the fifth is structural; the sixth is local-only by default but could be persisted; the seventh and eighth are derived. Erasure that targets only one of these (e.g., the envelope on disk) leaves residue in the others.
...
The decision is shaped by five primary instruments and two pieces of jurisprudence:
...
**CJEU *EDPS v SRB*, C-413/23 P (4 September 2025).** On appeal from T-557/20, the Court of Justice held that identifiability is assessed from the perspective of the party holding the data: pseudonymised data transferred to a recipient is not personal data *for that recipient* where the recipient has no means reasonably likely to be used to re-identify the subjects — the same record can be personal data in the controller's hands and non-personal in the recipient's (the "relative approach", converging with the Swiss reading in ADR-0027's landscape). The judgment construes Regulation (EU) 2018/1725 (the EU-institutions twin of the GDPR; the concepts are read in parallel) and leaves the disclosing controller's own position untouched — the data remain personal for the controller, and transparency duties are assessed at collection from the controller's perspective. Bearing on this ADR: peers receive only carrier tokens `(version, vaultRef, C)` — never values, nonces, or vault access — so the position that peers hold non-personal data from the moment of recording, previously argued on the Swiss axis only (HG190107-O), is now arguable on the GDPR axis as well; the controller-side analysis (vault, OQ1) is unchanged.
...
- **SD5 — Vault persistence (Architecture A).** The vault is per-controller, multi-actor, realised as a **Leeway facts table in ClickHouse**. Fact shape (Leeway encoding per [ADR-0018](./0018-leeway-card-json-canonical-format.md)): `(vaultRef, subjectID, actorID, fieldPath, valueBytes, nonce, recordedAt, retentionExpiry)`. `vaultRef` is random (≥128-bit), never derived from the value or the subject. `subjectID` and `actorID` are distinct dimensions on purpose: the actor is whoever *recorded* the patch, the subject is whom the data is *about*, and erasure/access requests (GDPR Art 17 / Art 15; FADP Art 32(2)(c) / Art 25) arrive keyed by subject — a vault keyed only by recording actor cannot answer them. Primary lookup by `vaultRef` (the `State` / `Render` path); secondary index on `subjectID` (access + erasure); secondary index on `retentionExpiry` to drive the storage-limitation sweep (Art 5(1)(e) / FADP Art 6(4)) — value changes mint new rows (SD6), so superseded rows are exactly the sweep's targets. Backups of the vault must follow the same retention/erasure policy as the live store; with SD12 adopted, restored backups are dead ciphertext for shredded subjects, which discharges the EDPB CEF 2025 Issue 6 erasure-on-restore demand by construction. The CH ENGINE choice (`ReplicatedMergeTree` vs. `MergeTree`) determines whether a `DROP TABLE … SYNC` clears replicated copies at once or relies on Keeper-coordinated mutation propagation — see SD11.
  ...
  **Shape convergence with ADR-0171 §SD5 (recorded 2026-08-14, not yet decided here).** [ADR-0171](./0171-leeway-sql-read-surface.md) §SD5 proposes an *exploded companion* to a packed leeway table — one row per attribute, `(doc, section, path, value…)`, generated from one schema, in ClickHouse — for query reasons entirely unrelated to erasure. That is the same explosion this sub-decision performs, reached from the other side: SD6's kill-reason 3 wants per-attribute rows because the erasure unit must equal the vault row. Three things follow for whoever fills this decision in.

  *The trial numbers are available but do not describe a vault.* The [second-substrate trial](../trials/leeway-second-substrate/README.md) priced the shape at 100M documents / 1.2 billion attributes converted in 50.8 s, exploded footprint 0.6978× the packed form, peak memory bounded rather than proportional to row count. Those are one-shot conversion figures. A vault mints a fresh row on every value change (SD6) and holds superseded rows until the SD5 retention sweep collects them, so the footprint ratio understates it by whatever the change rate and the sweep interval multiply to. The trial's memory figures transfer as a caution instead — up to 69.8× the packed form's peak on one value-predicate query, against a store this ADR expects to be queried by `subjectID`.

  *A second representation is a second erasure target.* ADR-0171 §SD5 leaves open how the redundant copy is maintained, and records that batch reconversion leaves it stale between runs. Where the rows carry personal data, staleness is not a freshness property: a derived copy refreshed after an erasure can restore what SD9 deleted. Any convergence must put both representations under SD11's finality contract, or keep vaulted sections out of the derived one entirely.

  *The columns pull apart.* SD12 envelope-encrypts `valueBytes` and `nonce` per subject and SD6 rules out equality joins over committed fields; ADR-0171 §SD5 exists precisely so that the path becomes a sort-key prefix and values become queryable. One table cannot serve both. The shape that survives is a split rather than a merge — this vault for the sections holding personal data, the companion for the rest — which would make SD10's routing question (`FieldPolicyI`, Go-side today) partly answerable from the leeway schema itself: `useaspects` carries a section-level `privacy` aspect and `valueaspects` carries `anonymized` / `pseudonymized` / `secret` / `application-level-encryption`. Whether routing should read those, or stay explicit in Go, is SD10's to settle; it is noted here because the schema already carries the signal.
...
- **SD8 — Tombstone garbage collection. Shipped.** `PushoutGraph.SweepTombstones(now, horizon)` (`pushoutgraph/store/pushoutgraph.go`) destroys tombstoned nodes' content past a retention horizon, marks them `Purged` (distinguishable from `Missing` for data-protection audits via `NodeContentStatus`), fails `Unapply` closed past the horizon, and returns the purged `NodeID`s in deterministic order for structured audit logging. Its doc comment carries the same dual framing as this ADR (GDPR Art 5(1)(e) / FADP Art 6(4): mechanism here, legal framing the consumer's concern). Residuals: the tombstone clock is session-local (pushoutgraphs rebuilt from envelopes reset it — applications needing persistent retention must layer it), and envelopes retain `Change.Content`, so the sweep discharges storage limitation for pushoutgraph state only; the vault remains the Art 17 / FADP Art 32(2)(c) mechanism. R9 reflects this.
...
- **SD13 — Value-equality and snapshot/diff semantics under vaulting.** The per-occurrence nonce (SD6) buys unlinkability at a deliberate cost on the *consumer* side: a vaulted field's patch-level content is its carrier token `(version, vaultRef, C)`, not its value, and two occurrences of the *same value* carry *different* tokens (a fresh `vaultRef` and nonce each). So **value equality no longer drives patch identity** for vaulted fields — with two consequences that surface in ordinary use, not only at erasure time:

  1. *Convergence by value-equality is lost for vaulted fields.* The base engine's content-addressing — two actors recording the identical edit converge on one patch — does **not** hold once a field is vaulted: identical values authored as separate occurrences are distinct patches and surface through the normal conflict machinery rather than collapsing. This is the unavoidable flip side of SD6's rejected per-actor-salt variant (kill-reason 1, "determinism leaks equality"): one cannot have both unlinkable commitments *and* value-equality convergence, and A chooses unlinkability. The Architecture-A diff-stability rule reuses a token byte-for-byte only while a *single* actor's value is *unchanged*; it confers no cross-occurrence or cross-actor equality.

  2. *Snapshot / diff APIs cannot diff values without the vault.* A git-like consumer that builds snapshots or diffs from patch content alone is diffing opaque tokens: it reports a *change* wherever the token differs (including where the underlying values are equal) and **cannot decide value equality at all** without resolving each token through the vault — which requires vault access (defeating the peer-side non-personal-data property of SD6) and is *impossible* for forgotten values (the row is gone; `State` / `Render` materialises the `Missing` / `Purged` placeholder, SD5/SD8). A snapshot spanning an erasure therefore carries the token but can neither render nor compare the value.

  Guidance for consumers: a value-level diff / snapshot API over vaulted fields must either (a) operate *structurally* over the commitments — occurrence identity, not value identity; (b) run controller-side with vault access, for non-forgotten values only; or (c) confine value diffs to non-vaulted fields. This bounds how "git-like" a pushout consumer can be on PII columns, and is the consumer-facing reason SD6's accepted "no equality joins over committed fields" cost is load-bearing beyond the privacy argument. The verification tree models the same property: the per-occurrence `recorded`-once guard in `erasure_vault.qnt` is exactly why identical values do not collapse there.

## Open questions for counsel

These are the questions engineering cannot resolve and that materially shape the architecture choice. The Decision section records engineering's selection (Architecture A) ahead of their answers (noted 2026-09-25): acceptance waits on OQ1 and OQ5, and an answer to either that goes against the engineering position reopens the Decision rather than amending it.

- **OQ1 — Do destroyed-nonce per-occurrence commitments clear WP216's anonymisation bar?** WP216 equates keyed-hash-with-key-deletion to "selecting a random number as a pseudonym … and then deleting the correspondence table" and files it under pseudonymisation. That passage addresses *deterministic* schemes, where one key spans many records and equality structure (linkability) survives key deletion. The SD6 scheme deletes a *per-occurrence* correspondence: after `Forget*`, each residue is a 32-byte string independent of every other commitment and of the value — no singling out, no linkability, no inference — and coincides with the EDPB CEF 2025 noted best practice of replacing erased data with "strings of random characters". The FADP axis is not the open part: BGE 136 II 508's effort test and the Walder Wyss deletion standard are cleared comfortably (ADR-0027 landscape). The engineering position is that the per-occurrence distinction meets the GDPR bar too; counsel should validate, since no DPA has opined on it explicitly. *EDPS v SRB* (C-413/23 P) supports the relative, holder-perspective reading of identifiability at EU level — but it addresses recipient-side classification, not the controller-side anonymisation claim at issue here, so it informs rather than resolves this question.
...
- Under the Swiss relative approach — and, since CJEU *EDPS v SRB* (C-413/23 P, 4 September 2025), arguably under the GDPR's recipient-perspective reading as well — peers hold non-personal data from the moment of recording (they never receive vault rows or nonces); the cross-border posture (FADP Art 16; GDPR Art 44–49) for patch propagation improves correspondingly, subject to counsel confirming the residue classification (OQ1).
...
- Render-time degradation: pushoutgraphs whose vault rows are missing render with placeholders. Consumers must handle this gracefully; UIs must distinguish "redacted" from "missing" from "unauthorized".
...
- [CJEU, *EDPS v SRB*, C-413/23 P, judgment of 4 September 2025 (EUR-Lex CELEX 62023CJ0413)](https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=celex:62023CJ0413) — [curia press release No 107/25](https://curia.europa.eu/site/upload/docs/application/pdf/2025-09/cp250107en.pdf)
