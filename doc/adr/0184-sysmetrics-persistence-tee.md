...
`gen.Input` gains an `ExternallyProvisioned` option. Under it the emitted store
**omits `EnsureTable` entirely** and keeps `VerifySchema`; the embedded DDL is
still emitted as the schema of record for review and diffing, but nothing in
the generated surface can apply it. `chstore.Store.SetupTable` stays the sole
DDL author for `boxer.facts`.
...
| --- | --- | --- |
| `recordstore/gen` `Input` (generated-code input) | +`ExternallyProvisioned`; emitted store omits `EnsureTable` (SD2) | emitter goldens; a store-shape test asserting the verb is absent |
