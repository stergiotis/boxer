...
- **SD16 — Streaming iter.Seq2 inputs (resolves Q-W1).** `Handle.LatLngsIterToCellsE` accepts `iter.Seq2[int, LatLng]` + an explicit `n` (iterators have no length). The iterator must cover every index in `[0, n)` exactly once; duplicate or out-of-range indices surface as an error. Implementation drains the iterator into handle-local reusable Go slices (`iterLats`, `iterLngs`) then performs the normal single batch write into wasm scratch — there is one unavoidable copy AoS→linear-memory regardless of input shape, so the iter variant is an ergonomic convenience for AoS sources (Arrow columns, channels, event streams), not a throughput optimisation. Rejected `iter.Seq[LatLng]` (no index, forces caller to maintain ordering state); rejected per-element `mem.Write` streaming (N bounds-checked wasm writes vs. one batch write). Iter variants for the other bulk methods are not added until a real consumer exposes AoS input; the one-call flagship keeps the pattern concrete without doubling API surface.
...
SD16 adds no new ABI export; it reuses `h3_latlng_to_cell`. The ADR's earlier open question Q-W1 ("should the bridge take `iter.Seq/iter.Seq2` inputs?") is resolved in the affirmative for the flagship entry point only.
