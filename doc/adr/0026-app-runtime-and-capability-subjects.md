...
The gate therefore adds a second condition: the capability-classified function must be called **by the app's own code** — the second-to-last function on the path is in the originating package. That drops 17 of 27 pairs, and each of the 10 survivors names a real call site (`adrboard.isDir -> os.Stat`, `play.newExecOptions -> os.Getpid`). Both rules are lower bounds; this is the tighter and more trustworthy one. What it gives up is capabilities reached through a higher-order stdlib wrapper (`io.Copy`, `io.ReadAll`) — precisely the cases where VTA cannot tell an `*os.File` from a `net.Conn`, so the verdict was a coin flip in either direction.
...
That retires four of the six findings (`adrboard`, `godepview`, `play`, `sccmap`), which the gate now evaluates and allows rather than dropping. Two remain, both real filesystem access from app code: `adrboard` stats the ADR corpus directly instead of going through the `SD7` picker substrate (and declares no caps at all), and the widgets demo's screenshot TestDriver creates its output directory. The four are worth remembering as the shape of a bad baseline entry — an accepted finding that no app could honestly act on is a defect in the table, not debt in the app.
