...
- **CGO-free Go build.**
- **Incremental migration.** Seven graphical apps and ~20 CLI subcommands cannot move atomically; old and new paths must coexist during the transition (as in [ADR-0057](./0008-demo-registry-and-drivers.md)).
...
Apps register via `init()`, mirroring the screenshot-tour pattern (`SD6` of [ADR-0057](./0008-demo-registry-and-drivers.md)). Two registration paths cover singleton vs multi-instance lifecycles:
...
Runtime state, capability grants, and audit records all live in a single CH+leeway table named `runtime.facts`, modelled on the existing `spinnaker.facts` schema at [`src/go/public/boxerstaging/spinnaker/schema/spinnaker_schema.go`](../../src/go/public/boxerstaging/spinnaker/schema/spinnaker_schema.go) and emitted by [`spinnaker_sql_ch.go`](../../src/go/public/boxerstaging/spinnaker/sql/spinnaker_sql_ch.go). A naive single-`payload`-string column is rejected because it abandons every property the leeway encoding provides — typed columnar query, dictionary compression on low-cardinality values, membership-as-ACL, co-section streaming, subset-projection for hot readers. The vocabulary used here (plain values, tagged value sections, memberships, streaming groups, co-sections) is defined in [`doc/skills/leeway-advanced/SKILLS.md`](../skills/leeway-advanced/SKILLS.md).
...
- **Migration spans several releases.** M1 alone touches seven existing apps; M3 reworks the host model. The two-path-coexistence discipline of [ADR-0057](./0008-demo-registry-and-drivers.md) applies throughout.
...
**Inotify backend** ([`fsbroker/watcher.go`](../../src/go/public/keelson/runtime/fsbroker/watcher.go)).
...
**Test coverage** ([`fsbroker/watcher_test.go`](../../src/go/public/keelson/runtime/fsbroker/watcher_test.go)):
...
**Cap-direction wrinkle.** `Service.Resolve` now grants `CapDirectionBoth` on `fs.handle.{uuid}.>` for watch-mode handles only — the app needs Sub on `.event`. Read / write / bundle handles continue to receive Pub-only grants. The conditional lives at [`fsbroker/service.go`'s Resolve path](../../src/go/public/keelson/runtime/fsbroker/service.go).

**Backends** ([`runtime/fsbroker/watcher.go`](../../src/go/public/keelson/runtime/fsbroker/watcher.go)):
...
- [ADR-0057](./0008-demo-registry-and-drivers.md) — Demo registry pattern that `app.Registry` generalises.
...
- [ADR-0005](./0015-streaming-persisted-kafka-from-connect.md) — Kafka package whose API the `kafka.*` subject family wraps.
...
- [`src/go/public/boxerstaging/spinnaker/schema/spinnaker_schema.go`](../../src/go/public/boxerstaging/spinnaker/schema/spinnaker_schema.go) — `spinnaker.facts` mapping; the structural precedent `runtime.facts` follows.
- [`src/go/public/boxerstaging/spinnaker/sql/spinnaker_sql_ch.go`](../../src/go/public/boxerstaging/spinnaker/sql/spinnaker_sql_ch.go) — Leeway → ClickHouse DDL emission used by `SD6`.
...
- [`src/go/public/thestack/imzero2/egui2/widgets/filepicker/`](../../src/go/public/thestack/imzero2/egui2/widgets/filepicker/) — Powerbox substrate (`SD7`).
