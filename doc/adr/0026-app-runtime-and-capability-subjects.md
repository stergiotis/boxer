...
**The baseline is 6 findings, and 4 of them are one problem in this table.** They are `READ_SYSTEM_STATE` incurred by `os.Getwd` or `os.Getpid` (`adrboard`, `godepview`, `play`, `sccmap`). The table maps `READ_SYSTEM_STATE` onto `sysmetrics.*`, a row written for `imztop` reading machine metrics over the bus. capslock's `READ_SYSTEM_STATE` is wider: it also covers ambient process facts (`os.Getwd`, `os.Getpid`, `os.Environ`, `os.Executable`). Declaring a `sysmetrics.*` subject in order to call `os.Getwd` would be nonsense, so these four are not apps failing to declare a capability — they are the row being too coarse to separate "read the machine's state" from "ask the runtime where I am". Splitting it is the next `SD10` decision, and it, not four manifest edits, is what should retire those entries. The remaining two are real filesystem access from app code: `adrboard` stats the ADR corpus directly instead of going through the `SD7` picker substrate (and declares no caps at all), and the widgets demo's screenshot TestDriver creates its output directory.
