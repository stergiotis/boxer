...
- **Per-app CH tables for state.** Rejected (`SD6`). Per-app schemas impose a schema-migration discipline on every new app and fragment the data-centricity property the leeway membership model preserves. Apps with structured-payload needs project a materialised view over `boxer.facts` rather than owning a separate base table.
- **Single polymorphic `payload` string column on `boxer.facts`** (the original draft of this ADR). Rejected (`SD6`): abandons the leeway columnar advantages (dictionary compression, typed query, membership ACL, subset projection) for a flat blob, and would require an opaque per-`kind` decoder on every read. The leeway-shaped table modelled on `spinnaker.facts` preserves these properties at the same row-count cost.
...
- **Leeway tooling is a hard prerequisite for queries.** Direct SQL over `boxer.facts` without the leeway DSL is awkward (physical column names are encoded). `play` and the boxer clickhouse DSL are the natural query paths; anyone reaching for `clickhouse-client` ad hoc will have a worse time.
...
**Three consumers become durable by this one change, one of them a correction rather than a feature.** play's editor buffers ([ADR-0148](./0148-app-workingsets.md)) survive a restart. The runtime applet store ([ADR-0132](./0132-sqlapplet-sql-defined-applets.md) O4) stops silently emptying its library at process exit — that ADR describes its documents as "`boxer.facts`-backed where ClickHouse is up", which the wired memory backend did not meet, and it carries a dated correction saying so. Any future persist user, including the dock-layout persistence this ADR lists as a follow-up, inherits the durability without its own work.

**The backend is chosen by the facts store's own verdict, and the choice is stated rather than inferred.** The carousel wires `FactsBackend` only when `chstore.NewWithFallback` reached ClickHouse; on the in-memory fallback both backends are process-scoped and the append-only trail would only grow, so `MemoryBackend` stays. `runtimestatus` reports the resolved backend in its existing `PersistBackend` segment ("facts" / "mem") beside the `FactsBackend` segment, because the two answer different questions: which code path app state took, and whether that path reaches ClickHouse. A `FactsBackend` construction failure degrades to memory rather than leaving `runtime.persist` unbound — losing durability is a smaller regression than losing the subject family — and the label says which one ran.

**`StorageBackendI` now receives a `StorageRef`, not a bare alias.** The alias stays authoritative for addressing and keying: the subject family is alias-addressed by construction, and that is what backends key on. The ref adds the requesting app's durable `AppId`, attributed by the service from the bus envelope exactly as launch and audit rows are attributed. A fact-recording backend stamps the id on the row, so a state fact joins that app's grant, audit, and launch facts on one column — the cross-kind join property §SD6 argues the single-table design exists to preserve. Recording the alias instead would have been silently correct for every app whose id equals its alias (all of boxer's) and silently wrong for an adopter whose id is a path, which is the shape that bites late.

Attribution is gated on the existing sender/subject alias check: a request addressing a namespace its sender does not own gets no id, since stamping the sender on a row in another app's namespace would misattribute it. The backend then promotes the alias to an id so reads and writes still agree. This deliberately does *not* resolve alias → AppId through the app registry: the applet store's `runtime.appletstore` is a synthetic service identity that is never registered as an app, so a registry lookup would miss precisely the consumer whose non-durability prompted the work.

**What did not change.** The subject grammar, the cap pattern, `Manifest.PersistedKeys`, `app.StorageI`, and `persist.Client` are all untouched — apps see no difference. Keys remain single subject tokens and values remain opaque `[]byte`; the typed-payload critique recorded against `StorageI` is still open and still declined. `MemoryBackend` keeps its alias-only keying and ignores the new field, so its behaviour is unmoved. With ClickHouse down, persist degrades to process-lifetime exactly as the audit trail does, and for the same reason.

### 2026-07-20 — `nats-server` ships as vendored source for airgapped builds; onbox unit; §SD4 unchanged

**The airgapped bundle ([ADR-0095](./0095-airgapped-build-bundle.md)) had no way to obtain a NATS server.** `SD4` names two provenance paths — contributors run `nats-server -js` locally; "for production, a deployment provisions it" — both of which assume the network or package access a sealed target lacks. The metric plane ([ADR-0090](./0090-sysmetrics-pubsub-data-plane.md)) is the first cross-process NATS consumer, and the topology layer ([ADR-0126](./0126-appliance-topology-as-data.md)) already declares the `nats` component with `sysmetricsd` depending on it — but no server could be built or run on an airgapped box.

**Mechanism: a Go `tool` directive, not a library import.** `github.com/nats-io/nats-server/v2` is added to `go.mod`'s `tool` block — the same mechanism already carrying staticcheck, govulncheck, and seven other build tools. `nats-server` is itself a Go program, so it rides the `go mod vendor` step the bundle already runs and the Go toolchain the bundle already ships; the target compiles the standard `nats-server` binary from vendored source. The repo cost is a `require` line plus one `tool` entry; the multi-MB server source materialises only inside the generated bundle, never in git (`vendor/` is bundle-time-only). `scripts/dev/airgap-bundle.sh` builds it in the offline-from-`vendor/` verify block, so an incomplete vendor is caught at bundle time.

**This does not reopen the embedded-server decision.** `SD4` and the *Alternatives rejected* both rule out an embedded `nats-server` **library** dependency — the monolith importing `.../v2/server` and inheriting NATS's lifecycle. A tool directive is the opposite: no boxer package imports the server, `./public/app` does not link it, and it runs as its own process. All three `SD4` properties hold — external binary, monolith neither launches nor supervises it, no library coupling. What changes is only the server's *provenance on an airgapped box*: from "a deployment provisions it" to "built from bundled source, run as a hardened loopback unit." `showcase/onbox/nats.service` supplies that unit — loopback-bound NATS core, no JetStream (per [ADR-0090](./0090-sysmetrics-pubsub-data-plane.md)), `Environment=BOXER_COMPONENT=nats` so the topology collector attributes it, sandbox modelled on `queryrunsd.service`. systemd is the supervisor, consistent with "not launched or supervised by the monolith."

### 2026-07-15 — `SD10` rewired: capslock in-process, own-code attribution, `compare` mode

**The integration described in `SD10` never ran, so its defects had never been observed.** It reads a JSON report produced by a `capslock` binary that is not installed and not vendored; the wrapper's own doc comments name `scripts/ci/capslock-check.sh` and a `cmd/capslock-check` binary, neither of which exists; and `capslock` appears in no lint script and no workflow. `SD10`'s *Limitations* asserts that "the JSON-output + wrapper script is the canonical integration shape". That sentence is withdrawn: the shape was never exercised, and three of the defects below are ones the shape itself invited.

**Shape.** The analysis now runs in-process against the source tree, importing `github.com/google/capslock/analyzer`. This costs one module and no transitive dependencies (all nine of capslock's were already in the build list), needs no external toolchain installed, keeps an airgapped build self-contained, and is type-checked against the pinned version instead of parsed out of a JSON contract that can drift. The repository already hosts `x/tools`-based analyzers in-process (`codelint`, `callsites`, `godep`), so this is the house shape rather than a new one. It does not route through `extbin` (ADR-0118): a library import resolves no external binary. Package loading is ours, not capslock's `analyzer.LoadPackages`, which sets no `packages.Config.Dir` and loads from the process working directory — a library may not `chdir`. Build tags come from `godepcollect.ResolveTags`; a load that omits them analyses a tree that does not exist.

**The gate was reporting on the wrong axis.** The JSON decoder dropped `capabilityType`, so transitive reachability was read as direct. Reachability saturates absolutely — all 19 app packages reach all 10 capabilities capslock reports, 190 of 190 pairs — because the standard library and the runtime reach everything. Every app was damned by everything, which is why the gate could not be promoted. This also retires `SD10`'s stated reason for preferring capslock over an import linter ("catches transitive escapes that a hand-rolled import-grep would miss"): transitive escape is real, but it is not a discriminator, because everything escapes transitively.

**Granularity is a trap worth recording.** `GranularityPackage` looks like the natural setting for a per-package question, and it is wrong here. capslock sorts by capability then function and keeps only the **first** record per (capability, package) pair, so the surviving record's `capabilityType` is one arbitrary representative's rather than the package's strongest. Filtering direct at package granularity finds 4 (package, capability) pairs where aggregating per function finds 27. The gate uses `GranularityFunction` and unions: a pair is the package's as soon as any one of its functions incurs it.

**Direct is necessary but not sufficient, and this is the substantive change to what `SD10` gates on.** capslock demotes a path to transitive only for a hop through a *non-stdlib* package other than the originator, so a stdlib hop never demotes and "direct" means only "no third-party package on the path". The capability may still be incurred deep inside the standard library, over an edge VTA guessed rather than proved:

```
imztop.formatPercent -> strconv.FormatFloat -> … -> internal/strconv.float32bits
  => direct UNSAFE_POINTER: formatting a float hard-fails the gate.

godepview.Mount -> context.WithCancel$1 -> (*context.cancelCtx).cancel
  -> (*context.afterFuncCtx).cancel$1 -> (*net.netFD).connect$1
  => direct NETWORK: VTA links every func() ever handed to context.AfterFunc
     anywhere in the program, so cancelling a context reaches the network.
```

The gate therefore adds a second condition: the capability-classified function must be called **by the app's own code** — the second-to-last function on the path is in the originating package. That drops 17 of 27 pairs, and each of the 10 survivors names a real call site (`play.newExecOptions -> os.Getpid`; the widgets TestDriver's `os.MkdirAll`). Both rules are lower bounds; this is the tighter and more trustworthy one. What it gives up is capabilities reached through a higher-order stdlib wrapper (`io.Copy`, `io.ReadAll`) — precisely the cases where VTA cannot tell an `*os.File` from a `net.Conn`, so the verdict was a coin flip in either direction.

**The `CAPABILITY_UNANALYZED` row is dropped, not implemented.** It reads "Capslock could not analyse the package; treat as a build-system bug, not an allowed default." That premise is wrong. `UNANALYZED` is capslock's marker for 39 named higher-order standard-library functions — `sort.Slice`, `io.Copy`, `sync.Once.Do`, `errors.Is`, the `bufio` `Reader`/`Writer` methods — whose behaviour depends on a value the caller supplies. It signals nothing about the build. Reporting it also *costs* findings: each such function becomes a sink that stops the walk and reports `UNANALYZED` instead of continuing into the concrete callee (measured over the apps: 27 direct pairs with them excluded, 26 plus `UNANALYZED` on 3 packages with them included). The classifier is configured to exclude them — capslock's own CLI default, adopted here deliberately rather than inherited. An `UNANALYZED` that somehow arrives is treated as an unknown capability, which the table already hard-fails.

**The trust-boundary carve-out is retired.** `SD10` and ADR-0028 `SD7` name a package list (`fsbroker`, `inprocbus`, `chlocalbroker`, …) whose capabilities the broker absorbs. Own-code attribution subsumes it: a capability an app reaches by calling a broker is reached through a non-stdlib package, so it is transitive and never raises a finding. The explicit list was only necessary because the previous implementation could not tell direct from transitive at all.

**The gate saw 8 of the 17 packages that register an app.** `capdemo`, `capinspector`, `godepview`, `imzrt`, `splashscreen`, `taskdemo`, `idsshowcase`, `logdemo` and `sccmap` were silently unchecked — not because they lack manifests, but because the check's app set is a hardcoded list of side-effect imports that had drifted. The list must stay static (Go imports are), so it is now asserted against the tree by a test: an app that registers but is not imported does not error, it simply never appears, and an invisible failure mode needs an explicit check rather than a convention.

**Adoption: M2.7 advisory → M3 `compare`.** `SD10`'s phased plan reaches its second step. The check is a plain Go test rather than a script, so it cannot drift from the code it checks, and `scripts/ci/lint.sh` gates on it — which is where this ADR says capability discipline belongs. `baseline.go` records the findings accepted today, each with the call site that incurs it; a finding outside the baseline fails, and a baseline entry that no longer reproduces fails as stale, so the list cannot quietly rot. The test is excluded from the default `-short` test gate: it costs ~15s and ~4.4GB of RSS building SSA over the apps' dependency cones. Hard-fail post-M4 is unchanged and still ahead.

**`READ_SYSTEM_STATE` is split by the function that incurs it.** The table maps the whole capability onto `sysmetrics.*` — a row written for `imztop` reading machine metrics over the bus. capslock's category is far wider: 36 functions spanning network topology (`net.Interfaces`), the process environment (`os.Getenv`), user and host identity (`os/user.Current`, `os.Hostname`, `os.Getuid`), well-known directories (`os.UserHomeDir`), and ambient facts about the process itself (`os.Getwd`, `os.Getpid`). One row cannot answer for four concerns, and as written it could not honestly answer for any of them: it demanded a `sysmetrics.*` subject to justify `os.Getwd`. The case it was written for cannot even reach it — `imztop` reads `/proc` through the sysmetrics packages, a non-stdlib hop, so its access is transitive and invisible here. The row had never fired for its intended purpose and, under own-code attribution, could not.

The gate now refines the capability using the last function on the path, which capslock already reports:

- **Ambient process facts** (`os.Getwd`, `os.Getpid`, `os.Getppid`, `os.Executable`, `os.Getpagesize`) are **always allowed**. They confer no effect and disclose nothing beyond the process, and every use of them that matters is classified on its own — a path built from `os.Getwd` is `FILES` when it is opened. Gating the discovery *as well* counts one act twice, and no subject would honestly justify it.
- **Environment access** (`os.Getenv`, `os.LookupEnv`, `os.Environ`, `os.ExpandEnv`, `syscall.Getenv`) is **not this table's business**. codelint CS011 bans it outright outside `public/config/env` (ADR-0009) — a stricter rule, over the whole tree rather than just apps. Deferring stops `SD10` issuing the wrong remedy, since "declare a `sysmetrics.*` subject" is not the fix for reading an env var. It cannot reach the gate today in any case: apps read configuration through `public/config/env`, and that hop is transitive.
- **The remainder** — network topology, user and host identity — keeps the `sysmetrics.*` requirement. No app triggers it today. The row now stands for an app that reads the machine directly, which is what it was always meant to catch.

That retires four of the six findings (`adrboard`, `godepview`, `play`, `sccmap`), which the gate now evaluates and allows rather than dropping — and they are worth remembering as the shape of a bad baseline entry: an accepted finding that no app could honestly act on is a defect in the table, not debt in the app.

**The baseline stands at one entry**, and how the other five left is the more useful record. Four went the way above, by correcting the table. The fifth left because its app did: `adrboard` stat-ed the ADR corpus directly instead of going through the `SD7` picker substrate, and it was removed the same day — its board is now a query against `keelson.adr` in play's kanban pane (ADR-0122). Debt can leave because the code left; that is not a fix, and nothing was made safer by it. What remains is the widgets demo's screenshot TestDriver creating its capture directory (`os.MkdirAll`) — harness code compiled into a demo app rather than a capability the demo exercises. **None has yet left the way the list wants: an app declaring the subject it needs, or reading through the broker.** Until one does, `compare` mode is holding a line that has never actually been walked back.

**What stands.** The threat model is unchanged and correct as written: hygiene, not security. Verdicts are a lower bound — VTA links a call only where a concrete type demonstrably flows, and reflection, unsafe, cgo and linkname carry no static edges — so an absent capability is the absence of a finding, not proof of absence. capslock is a detector and a drift alarm, which is what `compare` mode uses it as; it is never an authority for what code *cannot* do.
...
**Carousel wiring.** `imzero2_demo_cli.go` constructs `persist.NewService(bus, log.Logger, persist.NewMemoryBackend())` immediately after the fsbroker service. The in-memory backend matches Phase C scope; a `boxer.facts`-backed backend lands in a follow-up so state writes appear as auditable rows alongside grants + lifecycle events. The service is closed at process exit via a `defer` on the carousel's Action return.
...
**What this enables.** A future Rust-side audit-write path (screenshot capture, FFFI desync diagnostic, etc.) pulls the run_id from `runinfo::run_id()` and writes to the same `boxer.facts` table — a single `LifecyclesByRun` / `LookupRunStart` query reads both Go-originated and Rust-originated audit rows under one run. The amendment landed today is the prerequisite identity-flow piece; no audit writer crosses the FFFI boundary in this commit.
...
**Two new fact kinds** in `boxer.facts`, via two new `FactsStoreI` methods (`WriteRuntimeStart`, `WriteAppLifecycle`):
...
- `public/spinnaker/schema/spinnaker_schema.go` — `spinnaker.facts` mapping; the structural precedent `boxer.facts` follows.
...
- `public/db/clickhouse/dsl` — DSL package available for cap-broker query-shape validation and for `play`-side fact querying.
- `public/streaming/persisted/kafka` — Kafka client wrapper backing the `kafka.*` subject family.
