...
- **SD3 — `github.com/stergiotis/boxer/public/semistructured/cbor` is untouched.** Only a downstream consumer's `alpha/cbor` and the derived `public/semistructured/cbor` indexing layer retire; the upstream boxer CBOR encoder remains a supported dependency for low-level CBOR encoding needs unrelated to semi-structured ingestion.
...
- **`github.com/stergiotis/boxer/public/semistructured/cbor` continues as an upstream dependency for low-level CBOR encoding.** Code that needs to emit CBOR bytes (e.g., content-addressable hashing, wire-level test fixtures) keeps the boxer API; only the consumer-local parser stack retires.
