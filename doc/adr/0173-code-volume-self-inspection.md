...
One column, `generators`, on `go_packages` beside the existing generated
counts, parsed from the marker line the §SD3 pass already reads:
`Code generated by <tool>` yields the tool, and ANTLR's variant additionally
yields the input grammar. Free at acquisition, and lenient by necessity — the
tool field is free-form prose that nothing validates, so the values range from
an import path through a shell invocation to `copy paste` (and, in four files,
`copy pase`). The column groups strings somebody typed; where the marker
carries no tool at all, it reports `unknown` rather than failing the pass.

It is a **list**, not a single string: a quarter of this repository's
generated packages carry more than one tool and the leeway packages carry
three, so one string would silently drop the rest.
...
- **M3 — the `generators` column (§SD10).** ✓ One regex on a pass that already runs.
...
### 2026-08-06 — M3 landed; the vocabulary is larger than first-party suggested

`generators` on `go_packages`, `codevol.GeneratorOf` behind it. Three things
the implementation established:

- **A list, not a string.** §SD10 was drafted around one value per package.
  Measured: 15 of 59 generated packages in this repository carry more than one
  distinct tool, and `leeway/anchor/ecsdemo/stage2` carries three. A single
  column would have dropped a quarter of the answer, so the column is
  `Array(String)` and grouping goes through `arrayJoin`.
- **One normalisation is safe; a second is not.** Dropping the trailing caller
  parenthetical — several generators append the package that invoked them —
  takes the first-party vocabulary from 36 strings to 17, and is the
  difference between "Leeway DML" being one bucket and thirteen. Stripping
  invocation *arguments* looked like the same move and is not: it would
  collapse `./boxer.sh designsystem colors vendor` and
  `./boxer.sh designsystem colors gen` into one generator. So `stringer
  -type=Foo` and `stringer -type=Bar` stay distinct, and the vocabulary over
  the whole closure is ~130 strings rather than the 17 the first-party sample
  implied. That is the closure being larger, not the parse being wrong:
  stdlib and third-party markers carry whole command lines where first-party
  ones carry tool names.
- **The gate is the inverse of the normalisation, not a cardinality bound.** A
  bound on the vocabulary size was written first and failed against the real
  closure for the reason above. What can be asserted exactly is that no tool
  ends in the parenthetical the normalisation exists to remove.

Also fixed in passing: `providersgodep`'s integration gate had not compiled
since M2 changed `packagesTable`'s signature, and the default lane never
