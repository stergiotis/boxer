...
Two operating modes follow from this, not one. The **autonomous** mode is the steady state above — the timer polls tags and the box updates itself. But an operator occasionally needs to deploy *now*, off the tag cadence: an emergency hotfix that is not yet a release, or a pin back to a specific known-good revision. That **operator** mode has to be first-class rather than improvised around the autonomous loop — improvised, it degrades into "flip the signing flag and hand-run the binary," which is at once unsafe (a standing config hole, easily left on) and unergonomic (the toolchain env the systemd unit supplies is lost). The decision below covers both modes under one trust gate.
...
- An **operator can deploy a specific revision out-of-cadence** (`deploy --ref <commitish>`) through that same build → gate → swap → rollback pipeline — an emergency hotfix or a pin-back — under the *same* provenance gate (the commit's signature, against the same trusted keys as tags), with an audited, self-reverting **break-glass** for the genuinely unsignable case.
...
- **SD10 — Two deploy modes under one pipeline: autonomous (tag) and operator (ref).** The timer path (SD1) selects the newest *release tag*; the operator path adds `deploy --ref <commitish>`, which deploys *exactly* the named revision — a commit, a branch tip, or a specific tag — bypassing tag-selection and the newest-wins comparison, through the identical checkout → build → `ws_probe` gate → atomic swap → rollback pipeline (SD3–SD6). It covers emergency-forward (a hotfix not yet released) and emergency-back (pin to a known-good revision without waiting for a re-tag). The operator path runs in the *same execution context* as the timer — the `imzero2-deploy` systemd unit (a templated instance, or a `systemd-run` that inherits the unit's `Environment=` drop-in), not a bare hand-run of the binary — so the toolchain `PATH`/`RUSTUP_HOME` the build needs cannot diverge from the autonomous path. Hand-reconstructing that env is the failure mode this closes.

- **SD11 — Operator provenance: verify the commit; break-glass, not a flag-flip, as the last resort.** The operator path keeps SD8's gate, lowered one level: it verifies the *commit* signature (`git verify-commit <ref>`) against the **same `allowed_signers`** the box already trusts for tags — so a *signed* emergency commit preserves the full provenance guarantee with no release tag and no new trust material (it is exactly the payoff of signing commits, not only release tags). The irreducible case — a revision that cannot be signed in the moment — is an explicit `--break-glass`, defined so the trust downgrade stays bounded: (a) **per-invocation only** — an argument to one run, never a change to the unit's standing `IMZERO2_DEPLOY_REQUIRE_SIGNED_TAGS=true`, so the next autonomous cycle is fail-closed again with nothing to revert; (b) **audited** — it emits the SD7 `inprocbus` `Request` (the audited path, not `Publish`) recording who/what/when and logs a loud `WARN`; (c) **observable in the artifact** — it writes a `BREAKGLASS` marker into `releases/<ref>/`, so the running release self-identifies as unverified in `ls`, in the logs, and in the demo's `runinfo`. This supersedes SD8's bare "unsigned acceptance is a dev convenience" escape for operator use: the dev/loopback box stays a *standing-config* posture, but on a hostile box the only way past the gate is this deliberate, audited, self-closing *action* — not an env var set once and forgotten.

- **SD12 — Supersession by version floor, not string identity.** The autonomous skip test is "deploy `selectNewestTag()` unless it equals the tag `current` points at" — a *string equality* on the highest tag, a sound proxy for "nothing newer to run" only while operation is monotonic and `current` is itself that highest tag. An operator or break-glass deploy breaks that invariant: `current` becomes a non-tag revision, so the next poll finds the pre-existing highest tag ≠ `current` and would **redeploy it, reverting the operator's change on the very next tick.** The fix makes "newer" a *version* comparison rather than an identity check: an operator deploy records an **effective version floor** — the release tag the revision descends from (`git describe --tags`), or the highest tag at deploy time — and the autonomous path deploys the newest signed tag only when its version **strictly exceeds that floor**. An equal-or-older tag can no longer clobber an emergency deploy; only a genuinely newer signed release supersedes it — the intended "the hotfix holds until a real release rolls past it." The same change hardens the autonomous path, replacing the string-equality stand-in with the version comparison it was always approximating.
...
- **Operator path: flip the signing config and hand-run the binary** (what the tooling forced before SD10–SD12). Rejected: it conflates a *standing posture* (is signing required on this box?) with a *momentary action* (do one emergency deploy), so the insecure state persists and is forgotten; it loses the unit's toolchain env (the `cargo: command not found` hand-reconstruction of `PATH`/`RUSTUP_HOME`); and it leaves no audit of who bypassed the gate or what shipped. SD11's per-invocation, audited, self-reverting break-glass is the same capability without those three failure modes.
- **Operator path: cut a throwaway release tag.** Tag the hotfix `vX.Y.Z`, push, let the timer take it. Rejected as the *primary* emergency path: it forces version-sort reasoning under pressure (the tag must out-rank `current` to deploy yet not block the next real release), pollutes the public tag namespace with throwaway versions, and still cannot deploy an *unsigned* hotfix without the gate escape. Fine for a *planned* release; SD10's `--ref` is the unplanned one.
- **Operator path: globally lower the gate** (set the box to never require signatures). Rejected outright: it trades a one-deploy provenance reduction for a permanent one. The whole of SD11 exists to keep the downgrade scoped to a single audited action.
...
- **Emergency deploys are operable under pressure and still verified:** one command (`deploy --ref <sha>`) puts an exact revision through the full gate + rollback pipeline; a *signed* hotfix keeps the SD8 provenance guarantee with no release tag (SD10–SD11), and the version-floor rule stops the timer reverting it (SD12).
...
- **Break-glass is a real, if bounded, provenance reduction.** A `--break-glass` deploy runs code the box did not verify, and it stays live until a newer signed release rolls past it (SD12). The design makes that window deliberate, audited (SD7), observable (the `BREAKGLASS` marker), and self-closing on the gate side (SD11) — it does not pretend the window is zero.
...
- [How to sign and verify boxer releases](../howto/release-signing.md) — the SSH `allowed_signers` trust root SD8 verifies *tags* against and SD11 reuses to verify *commits*; adopting commit signing is what lets the signed-emergency path (SD10–SD11) be verified without cutting a release tag.
