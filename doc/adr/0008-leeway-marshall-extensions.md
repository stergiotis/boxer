...
`naturalKey` so its `SetId` matches the facts 2-arg form. Other in-tree
facts DTOs that still declare an `int64` `ts` (the `task*` / `watch*` /
`grant*` / `dialogreply` / `persistreply` / `inflightsnapshotreply`
codecs) keep compiling from their checked-in `.out.go` — they are not
regenerated by any test — but will need the same `int64`→`time.Time` (and
`naturalKey`) migration the next time their generator runs.
