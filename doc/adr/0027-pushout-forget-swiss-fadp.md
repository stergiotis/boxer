...
> **Status: superseded by [ADR-0025](./0025-pushout-forget-architecture.md) on 2026-05-17.** Architecture A — the GDPR-grade vault-by-design selection in ADR-0025 — also clears the FADP axis under BGE 136 II 508's effort test and Art 32(4)'s anonymisation route, so the S2-tier optimisation this ADR proposed for Swiss-only scope is no longer load-bearing for the active deployment. This document is compacted (2026-05-17) to retain only the reference material that's hard to reconstruct — Swiss legal landscape, FADP↔GDPR comparison, taxonomy mapping — for any future deployment that revisits Swiss-only scope.

> **Earlier status (pre-2026-05-17):** proposed — pre-human-review; engineering recommendation S2 (split-tier with local sidecar), deferred pending counsel review. Pre-compaction history is available in git via the file at commit `baaebb1f` and earlier.
...
The substantive proposal (**S2** — split-tier patch with local-sidecar cleartext, per-actor HMAC commitment, salt-destruction-as-anonymisation) is no longer the active path: ADR-0025 selected Architecture A on 2026-05-17, and A's destroyed-salt-HMAC story clears Swiss compliance as a side effect of clearing the stricter GDPR bar. What remains useful in this ADR is the Swiss legal landscape and the FADP↔GDPR comparison, both retained below.
...
| **Kettiger 2021 §28** | "From a purely (data protection) legal perspective, anonymisation of decisions and judgments is **only a pseudonymisation as long as the decision number or case number remains**." | Inverse reading: when the linkage key (in our case, the per-actor salt) is destroyed, what remains is anonymisation *in the legal sense*. The split-tier proposal's salt-destruction step crosses from pseudonymisation into anonymisation under Swiss law. |
...
| ADR-0025 Architecture A *(adopted 2026-05-17)* | (a) HMAC | per-actor | vault (Leeway/CH) | none | FADP + GDPR |
| ADR-0025 Architecture B | (a) / plaintext + propagation | n/a | none | cooperative purge | FADP + GDPR — by-design weak |
| ADR-0025 Alternative D | (b) symmetric | per-patch | sidecar | unilateral | rejected under WP216; **defensible under FADP — see SQ8** |
...
Superseded by [ADR-0025](./0025-pushout-forget-architecture.md) on 2026-05-17. The earlier engineering recommendation in this ADR (S2 — split-tier with local sidecar) is preserved in the Variants table above and in git history; it is no longer pursued.

## Alternatives

Pre-supersession this section enumerated S1 / S3 / S4 (and the rejection of ADR-0025 Architectures A and B for Swiss-only scope). After supersession the active alternative set is owned by [ADR-0025 § Alternatives](./0025-pushout-forget-architecture.md#alternatives); the Variants-mapped-onto-the-taxonomy table above carries the comparative inventory forward. The pre-compaction enumeration is available in git pre-2026-05-17.

## Open questions for counsel

SQ1–SQ7 were the Swiss-specific decision-shaping questions for S2 vs S4; under the A decision they are historical and have been dropped from this compacted version (available in git pre-compaction). The one open question worth carrying forward:
...
Pre-supersession this section enumerated the Positive / Negative / Neutral consequences of adopting S2. After supersession the active consequence set is owned by [ADR-0025 § Consequences](./0025-pushout-forget-architecture.md#consequences). The single Swiss-specific consequence worth carrying forward is the enforcement-risk axis: Architecture A's overspend (vault apparatus, mutation-finality contract) is materially cheaper-than-justified under FADP-only sanctions (CHF 250k cap on natural persons, wilful + on complaint) than under GDPR Art 83 fines, but the overspend is paid anyway because A is the chosen path. The pre-compaction enumeration is available in git pre-2026-05-17.

## Status

Superseded by [ADR-0025](./0025-pushout-forget-architecture.md) (Architecture A
decision, 2026-05-17). Retained per the append-only ADR discipline as a
comparative reference for any future Swiss-only-scope deployment that
might revisit the S2-tier optimisation.

Status lifecycle: `Proposed → Accepted → (Deprecated | Superseded by ADR-XXXX)`.

## Updates

### 2026-05-17 — compacted (post-supersession)

Compacted alongside the supersession to drop content that became
non-load-bearing once Architecture A was selected:

- Personal-data inventory revisited (redundant with ADR-0025).
- Architectural-fit / split-tier S2 proposal (the design body — superseded).
- Three operations section (canonical home is `doc/explanation/erasure-design-space.md`).
- Design space (QOC) — Question / Options / Criteria / Matrix / Reading.
- Detailed treatment of S2 / S3 / S4.
- Adequacy considerations.
- Alternatives.
- Consequences.
- Open questions SQ1–SQ7 (S2-specific). SQ8 (family-(b) under FADP) kept.

Preserved: Swiss authorities table, FADP vs GDPR comparison table,
Variants-mapped-onto-the-taxonomy inventory, References, Status, Updates.
Pre-compaction content is available in git (the file at any commit before
the compaction commit).

### 2026-05-17 — superseded by ADR-0025

ADR-0025 selected **Architecture A** (PII-segregation by design; vault as a
Leeway facts table in ClickHouse; per-actor HMAC salts) for the active
greenfield deployment. Architecture A's legal posture under FADP rests on
BGE 136 II 508's effort test plus Art 32(4)'s anonymisation route — both
of which are already satisfied by destroyed-salt HMAC commitments. The
S2-tier optimisation this ADR proposed (split-tier with local sidecar,
right-sized for Swiss-only scope) is therefore no longer load-bearing:
Architecture A delivers Swiss compliance as a side effect of clearing the
stricter GDPR bar, at the cost of the vault apparatus this ADR judged
"over-built" for Swiss-only scope.

If a future deployment scopes itself to Swiss-only data subjects and the
controller wishes to revisit the S2-tier optimisation, a follow-up ADR
should reopen the analysis rather than amend this one.

### 2026-05-12

[ADR-0039 — Antiquing Architecture for the Pushout VCS](./0039-pushout-antiquing.md)
formalised the antiquing prerequisite that the S2 → S4 upgrade path
inherited from ADR-0025 SD4. (Superseded by 2026-05-17: ADR-0025 selected
Architecture A; SD4 is now *inapplicable*; ADR-0039's compliance-blocker
status is dissolved.)
...
- [ADR-0025 — Right-to-Erasure Architecture for the Pushout VCS (GDPR-scope)](0025-pushout-forget-architecture.md). This ADR is the (now-superseded) FADP-scope variant.
...
- `../boxer/public/algebraicarch/pushout/envelope/envelope.go` — envelope codec.
