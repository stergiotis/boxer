...
4. **No dedicated "throttle when hidden" option.** Under `vsync` the compositor already throttles occluded windows; an explicit option is deferred (SD6). *The premise holds for what is presented and not for what is burned — an occluded client spins a core (2026-08-18 entry) — and the defect is upstream, not in the cadence (2026-08-19 entries).*
...
- **SD6 — A "throttle when hidden" option is deferred, not designed out.** egui 0.34 exposes `ViewportInfo.occluded` (eframe sets it from winit's `WindowEvent::Occluded`), so a visibility-aware mode is implementable. It is deferred because under `vsync` the compositor already throttles occluded windows, making the option largely redundant today. *Reopened: the throttle governs presentation, not CPU, and the option would not have helped anyway — the spin is an eframe control-flow defect, fixed upstream by #8398 (2026-08-18 and 2026-08-19 entries). Also, winit never emits `WindowEvent::Occluded` on Wayland, so the mechanism this SD names is inert on the platform it was measured on.* Revisit if `vsync`-off / non-blocking present modes become common, or if a deeper-than-compositor throttle (near-pause when hidden) is wanted. Occlusion is known only Rust-side; a visibility-aware mode would make the Rust client the authority and have the Go decorator defer to it.
...
- Reactive mode is available for power-sensitive use, cutting a visible-but-idle window from vsync rate to a few fps at ~0.2 % of a core. It is **not** a fix for a *backgrounded* window: where the compositor withholds frame callbacks entirely, a reactive client spins exactly as a continuous one does (2026-08-23 entry).
...
- **`vsync` induces the beat, where there is a beat.** With `-vsync on` a present that lands just past the refresh deadline slips a whole refresh (60→30); `-vsync off` (the compositor composites, so no tearing) markedly reduces the doubling on this session, and does nothing measurable on a session that has no doubling to begin with (2026-08-23 entry). This is the one app-side lever, and it reinforces the SD6 argument that a hidden-throttle option is largely redundant under `vsync`.
...
This does not change the continuous default, and the ~1 fps itself remains expected and documented ([triage janky rendering](../howto/imzero2-render-troubleshooting.md)). What changes is SD6's cost basis: the compositor throttles what is *presented*, not what the client *burns*, so a hidden-throttle option is not redundant — it is worth roughly one core on any backgrounded window. SD6 already names the mechanism (`ViewportInfo.occluded`, set by eframe from winit's `WindowEvent::Occluded`) and the ownership question (occlusion is known only Rust-side, so the client would become the authority and the Go decorator defer to it); both stand. `IMZERO2_RENDER_CADENCE=reactive` avoids the spin on a compositor that still delivers throttled frame callbacks — not on one that withholds them, where it spins too (2026-08-23 entry) — and either way it is not the answer SD6 wants — it also throttles a *visible*-but-idle window, which decision 1 and the O2 rejection deliberately declined to impose.
...
Two readings. **Under throttling both cadences deliver the same ~1 fps, so the spin buys nothing** — continuous burns a whole core for zero additional frames, and reactive removes it 55× *here*. That last part does not generalise: this probe keeps delivering frame callbacks at 1 Hz, and on a compositor that stops delivering them altogether reactive spins as hard as continuous (2026-08-23 entry). **But reactive still cannot be the default**, because on a *visible* window it drops 60 fps to ~1: exactly the O2 rejection in decision 1. `IMZERO2_RENDER_CADENCE=reactive` is therefore a real workaround for a backgrounded dev window and not a fix.
...
No decision change. Continuous stays the default and SD6 stays open, now blocked on work outside this tree: filed as [emilk/egui#8434](https://github.com/emilk/egui/issues/8434), proposing that eframe re-key its existing backstop from "the window is invisible" to "a redraw has been requested but unserviced for > N ms" — observable on every platform, and free here because the frames it suppresses are frames the compositor was never going to deliver. Until that lands there is no reliable in-tree mitigation: `IMZERO2_RENDER_CADENCE=reactive` helps only where the compositor still delivers throttled callbacks, and on GNOME/mutter — which withholds them from a covered surface — the reactive client burns 92–100 % of a core at zero frames (2026-08-23 entry).
