...
- **SD6 — Self-module exclusion: `pkg:golang/github.com/stergiotis/boxer` purl prefix.** The post-processor filters components whose purl begins with the boxer module path. This matches the current `--ignore github.com/stergiotis/boxer` flag. Sibling repositories owned by the same author (`capmap`, `pebble2impl`, `pushout-algo`, `stylometrics`) are *not* filtered: they are independent modules and the gate evaluates them like any other dependency. Their LICENSE-file absence surfaces in the `unknown` advisory and is tracked there.
