...
> **Amendment 2026-07-25 — transport, tab inventory, the known-difference ledger, and the debounce that never shipped.**
> Five parts of this ADR no longer describe the code. Recorded here rather than edited in place, per the append-only rule.
>
> **1. SD7 is superseded — the transport is the chlocalbroker, not `os/exec`.** SD7 describes spawning a fresh `clickhouse local --query … --format ArrowStream` per query through `os/exec`. The app now publishes on the capability subject `ch.local.exec.regex_explorer` and a pre-spawned pooled worker answers (ADR-0028 §SD9); no subprocess management remains in the package. The consequences SD7 argued for still hold — no server, no auth, no network, wall-clock-only timing — and cold-start cost is now amortised across the pool rather than paid per query. The `Format: "ArrowStream"` request travels with the publish, so the SQL builders emit no `FORMAT` clause.
>
> **2. The four-tab table is wrong in two rows.** As shipped: **Test** (renamed **Preview (Go)**) is a Go-only surface that highlights matches and breaks out capture groups, and issues no SQL at all; **List** runs `extractAll`, plus `extractAllGroups` when the pattern captures; **Replace** runs `replaceRegexpAll`; **Multi** is not a tab — it renders inline beneath the multi-pattern input, because unlike the other three it owns its own input and a tab would separate the input from its results. That asymmetry is deliberate, not an oversight.
>
> **3. SD1 gains a known-difference ledger, because a binary tripwire could not work.** `tripwireCase` now carries a `KnownDrift` note; a case that declares one is still evaluated and logged, but its disagreement is counted as *known* rather than lighting the DRIFT indicator. This was forced by discovering that the original ten-case corpus already contained a permanent divergence — `group-capture` — so the indicator had read DRIFT since the day it was written and could never signal anything else. Two differences are now documented:
>
> - **`extractAll` returns capture group 1, not the full match, whenever the pattern has a capture group.** `extractAll('(x) (yz)', '\(([^)]+)\)')` is `['x','yz']` where Go's `FindAllString` gives `["(x)","(yz)"]`. This is the most load-bearing difference for this app: the List tab renders `extractAll` output directly while the Preview tab highlights full matches, so the List tab now states the caveat and shows the `extractAllGroups` breakdown alongside.
> - **Empty-match enumeration.** `extractAll('xyz','a*')` is `[]`; Go reports one zero-width match per position. RE2 specifies what matches, not how a caller enumerates repeated empty matches. The Go-side preview sides with ClickHouse (zero-width matches are filtered) because predicting ClickHouse is the product; the tripwire compares the raw engines so the difference stays visible.
>
> SD1 coverage also extends past `extractAll`: every corpus pattern is probed against VectorScan via `multiMatchAllIndices`, because the Multi tab decides which lines to send using Go's `regexp` — a *different engine's* opinion. A pattern Go accepts and VectorScan rejects fails the whole set's query, so that mismatch is now reported as drift in its own right.
>
> **4. SD5 is now testable.** Capture-group display was specified across this ADR, the package doc, and the cheatsheet, but never implemented — the painter only ever called `FindAllStringIndex`, so group offsets were computed nowhere and SD5's numbering-parity assumption had nothing to compare. The Preview tab now uses `FindAllStringSubmatchIndex` and paints per-group tints with byte ranges, which is what C2's "engine fidelity" rating assumed all along.
>
> **5. Criterion C1's 300 ms debounce never existed, and is no longer wanted.** No timer was ever implemented; every keystroke dispatched up to four queries. Rather than adding one, dispatch became *level-triggered*: each frame states the input fingerprint each lane should hold results for, and a lane converges. At most one run per lane is in flight, so the query rate is bounded by query latency instead of keystroke rate — twenty characters typed during one round-trip produce exactly one follow-up query, for the twentieth. This also fixes the defect the edge-triggered design carried: an edit arriving while a query was in flight was dropped and never re-fired, so the UI could report `CH: match=true` for a pattern that no longer matched. A debounce would have added a fixed latency floor to every edit without fixing that.
...
- **SD2 — Parameter binding via [`marshalling.EscapeString`](../../public/db/clickhouse/dsl/marshalling/ch_marshalling_marshall.go) inlined into SQL.** `clickhouse local` does not offer an HTTP `?param_<name>=…` channel, and the SETTINGS-clause form was falsified under both transports via `clickhouse local` at Phase 1: `SELECT match({hay:String}, {pat:String}) SETTINGS param_hay='…', param_pat='…'` returns *Code 456: Substitution `hay` is not set (UNKNOWN_QUERY_PARAMETER)*. ClickHouse's SETTINGS clause is for query-level server settings (`max_execution_time` etc.); it is not a parameter-binding mechanism. Multi-statement `SET param_<name> = '…'; SELECT … {<name>:Type} …` does work, and could be layered in via `clickhouse local`'s implicit multi-statement handling, but gains nothing over inline escaping given this app's SQL shape (single `SELECT` per dispatch). The app therefore inlines escaped literals via [`marshalling.EscapeString`](../../public/db/clickhouse/dsl/marshalling/ch_marshalling_marshall.go). Example: user pattern `it's a test` becomes the SQL literal `'it\'s a test'` (boxer-tested escape of `'`, `\`, `\n`, `\t`, `\r`, `\0`), interpolated directly into the SELECT. [`passes.WriteSettings`](../../public/db/clickhouse/dsl/nanopass/passes/nanopass_passes_manipulate_setting.go) remains available for actual server settings (query caps like `max_execution_time` / `max_result_rows`) but is not used for parameter binding. The decision's core premise — that Go sees the same `pat` / `hay` strings it sends to ClickHouse — holds: both Go's `regexp` compile and ClickHouse's parse see the same pre-escape source string, so `regexp.MustCompile(pat).FindAllStringSubmatchIndex(hay, -1)` remains an accurate local mirror.
...
- **Boxer reuse.** [`passes.WriteSettings`](../../public/db/clickhouse/dsl/nanopass/passes/nanopass_passes_manipulate_setting.go) + `ExecuteArrowStream` cover the round-trip; no new SQL-construction code in the demo beyond template strings.
...
- [`public/db/clickhouse/dsl/marshalling/ch_marshalling_marshall.go`](../../public/db/clickhouse/dsl/marshalling/ch_marshalling_marshall.go) — `EscapeString` / `MarshalGoValueToSQL`, the fallback if SD2 verification fails.
