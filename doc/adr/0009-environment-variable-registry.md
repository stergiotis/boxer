...
- **2026-05-21 — §4 doc generator migrated to `boxer env gen-docs`.** The standalone `internal/cmd/envgen` main is removed. Its renderer call site moves to a new `envdoc.NewGenDocsCommand` registered under `boxer env` via the variadic `extraSubcommands` parameter newly added to `env.NewCliCommand`. The indirection breaks what would otherwise be an `env → envdoc → env` import cycle (envdoc imports env to read `Snapshot()`). The blank-import set that envgen owned moves to `public/app/main.go`: `dev`/`docgen`/`logging`/`tracing` were already in boxer's graph; `llm/openaichat` and `semistructured/leeway/ddl/clickhouse` are added explicitly. Side effect — `boxer env list` now sees those two packages' Specs at runtime, narrowing the "doc generator vs runtime list" gap noted in earlier entries. The `//go:generate` directive in `doc_gen.go` switches from `go run … internal/cmd/envgen` to `go run … public/app env gen-docs`; regenerated `doc/env-vars.md` differs only in the generator metadata header (21 variables × 6 categories, unchanged). Motivation is CODINGSTANDARDS.md "Entry Points" — the four legacy mains audit baseline becomes three after this migration. Core decision (single registry, declared-once Specs, single doc surface) stands.

- **2026-08-28 — Resolution tiers made explicit: empty means unset, in-process `Override`, empty categorial default.** The first downstream wrapper command (a downstream `gui`, which hosts boxer's carousel in-process and must seed its app's specs before its `Mount` reads them) exposed three gaps. (1) `AsCliFlag`'s chained Action cached whatever urfave/cli parsed, and urfave treats a set-but-empty variable — or a bare `--flag=` — as a set flag, so an exported empty value was cached as `""` and shadowed every later read, while `Get()` treated the same empty variable as unset; the two now agree, on every typed var: an empty parsed value leaves the cache alone and runs no user action, and `Spec` documents the rule (empty is never a distinct value). (2) Only `SetForTest` could write a var's cache, so the wrapper exported values with `os.Setenv` to reach an in-process reader. Every typed var gains `Override(v)` / `ClearOverride()` — highest precedence, process-local, never written to the environment — and `ValueSource()` reporting the tier in use (`ValueSourceE`: override > flag > env > default; the flag tier includes values urfave took from the bound variable). The per-var cache and mutex moved into one generic `resolved[T]` helper shared by the seven types. (3) `NewCategorialString` required a non-empty `Default`; a variable whose absence means "idle" therefore needed a sentinel member or a plain string with hand validation. An empty `Default` is now allowed and reads as unset (`Get()` returns `""` until a member is supplied); the empty string can never be a member. Core decision (single registry, declared-once specs, one read path) stands; §3's "the flag's Action writes the parsed value back" is refined, not replaced.

- **2026-09-25 — A downstream project no longer named.** Mentions of a downstream repository were replaced with a generic description. That repository is private, and this one is public. The decision is unchanged.
