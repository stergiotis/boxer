...
The ImZero2 Go↔egui binding at [`src/go/public/thestack/imzero2/`](../../src/go/public/thestack/imzero2/) currently surfaces colors to Go callers through two unrelated, incompatible transports:

- **`uint32` PlainArg path** — used by painter, plot, and graph widgets. IDL declares `PlainArg("rgba", ctabb.U32)`; Rust decodes via `color32_from_rgba_u32()` at [`src/rust/src/imzero2/interpreter.rs:27-32`](../../src/rust/src/imzero2/interpreter.rs) with premultiplication. Surfaces in Go as `uint32`.
- **`Color32S` EvaluatedArg path** — used by widgets (Frame, ProgressBar, CodeView, Label). IDL declares `EvaluatedArg("color", structColor32())`; the retained holder emits Color32-construction opcodes that leave the value in the Rust-side register `r11_color32` ([`interpreter.rs:1227`](../../src/rust/src/imzero2/interpreter.rs)). Surfaces in Go as `Color32S`.

An audit across [`src/go/public/thestack/imzero2/egui2/definition/egui2_definition_d_*.go`](../../src/go/public/thestack/imzero2/egui2/definition/) counted ~28 color-bearing argument sites. Concrete consequences of the split:
...
- **Round-trip cruft.** [`src/go/public/thestack/imzero2/egui2/widgets/treemap/coloring.go:48-53`](../../src/go/public/thestack/imzero2/egui2/widgets/treemap/coloring.go) packs `uint32` → `(r,g,b,a)` bytes → `Color().FromRgbaUnmultiplied(...).Keep()` to feed widget-style APIs that only accept `Color32S`.
...
- **FFFI2 rules.** Generated files under `components/*.out.go` and `src/rust/src/imzero2/enums_out.rs` are off-limits; all changes must land in the hand-written definition files and the generator under [`src/go/public/thestack/fffi2/`](../../src/go/public/thestack/fffi2/). FFFI2 reserves argument names matching `[a-zA-Z][0-9]*` ([`fffi2/ir/idl/fffi2_ir_idl_arguments.go:23`](../../src/go/public/thestack/fffi2/ir/idl/fffi2_ir_idl_arguments.go)).
...
- **Treemap round-trip goes away.** [`treemap/coloring.go:48-53`](../../src/go/public/thestack/imzero2/egui2/widgets/treemap/coloring.go) collapses from `splitRGBA` + `retainedFromRGBA` to a single `egui2.Hex(rgba)`.
...
Accepted — 2026-04-22. Amended 2026-04-24 with SD9 (color arrays as `egui2.Colors`, literal-only). Design frozen; implementation begins with the `egui2.Color` / `egui2.Colors` Go types, the `.AsColor()` / `.AsColors()` IDL annotations, and the per-transport encoder paths in [`fffi2/compiletime/goserver/`](../../src/go/public/thestack/fffi2/compiletime/goserver/) and [`fffi2/compiletime/rustclient/`](../../src/go/public/thestack/fffi2/compiletime/rustclient/).
...
- [`doc/adr/0001-clickhouse-observability-pipeline.md`](./0001-clickhouse-observability-pipeline.md) — prior ADR in this repository.
- [`doc/adr/0002-query-categorization-provenance.md`](./0002-query-categorization-provenance.md) — prior ADR; template shape followed here.
...
- [`src/go/public/thestack/imzero2/egui2/definition/`](../../src/go/public/thestack/imzero2/egui2/definition/) — hand-written IDL definition files targeted by the refactor.
- [`src/go/public/thestack/fffi2/ir/idl/fffi2_ir_idl_arguments.go`](../../src/go/public/thestack/fffi2/ir/idl/fffi2_ir_idl_arguments.go) — IDL primitive declarations (`PlainArg`, `EvaluatedArg`).
- [`src/go/public/thestack/fffi2/compiletime/goserver/fffi2_compiletime_go_server.go`](../../src/go/public/thestack/fffi2/compiletime/goserver/fffi2_compiletime_go_server.go) — Go emitter.
- [`src/go/public/thestack/fffi2/compiletime/rustclient/fffi2_compiletime_rust_client.go`](../../src/go/public/thestack/fffi2/compiletime/rustclient/fffi2_compiletime_rust_client.go) — Rust emitter; existing per-type switching precedent at lines 67–81.
- [`src/rust/src/imzero2/interpreter.rs`](../../src/rust/src/imzero2/interpreter.rs) — runtime decoder; `color32_from_rgba_u32` at line 27, `r11_color32` at line 1227.
