...
--   LEEWAY_LU_MEMB_IDX_TO_VAL_IDX(card)
--       the membership-index -> attribute-index map, from `lmrcard`.
--   coGather(lane, sel) / raggedStarts(card)      [ADR-0162 chpack]
...
--       coGather(vals, raggedStarts(len)) re-aligns it by taking each
