...
--       rides `lr`), so the lanes are NOT aligned and the pack's LW_CO_LOOKUP —
--       which is `lane[indexOf(keys, k)]` — silently returns the wrong
--       attribute for every row.
--
--   LW_LIST_BY_TAG_EQUAL(flat, len, tags, tag, m2v)   [ADR-0066]
...
-- LW_CO_GATHER(vals, LW_RAGGED_STARTS(len)) dropped every element after the first
-- without saying so.
--
